...
p.loadOptions = function(hOpts) {
    ...
    if (oSelf.hClasses.App && oSelf.hClasses.App.hProperties)
        ...
        oSelf.hClasses.App = {
            ...
            ,hProperties:{
                id:{bPrimary:true,sType:'String',nLength:10,sMySqlType:'CHAR(64)'}
                ,sid:{bUnique:false,sType:'String',sMySqlType:'CHAR(64)'}
                ,root:{sType:'Boolean'}
                ,app_id:{sType:'Number'}
...
p.init = function (hOpts,fnCallback) {
    ...
    // Set up file watchers on the configuration file and update apiary file.
    if (hOpts && hOpts.NORDIS_ENV_CONF)
        fs.watch(hOpts.NORDIS_ENV_CONF, function (curr, prev) {
            console.log('Config file changed. Updating apiary docs in project root ('+hOpts.NORDIS_ENV_ROOT_DIR+').');
            oSelf.writeApiaryDocs(hOpts.NORDIS_ENV_ROOT_DIR,function(err){
                if (err)
                    oSelf.error(err);
                else
                    console.log('Done updating apiary docs.');
            });
        });
    if (fnCallback)
        fnCallback();
