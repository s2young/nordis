...
p.broadcast = function(txid,message) {
    var oSelf = this;
    var broadcast = function() {
        ...
        oSelf.dbPub.publish(txid,message);
    };
    if (!oSelf.dbPub)
...
p.subscribe = function(txid,messageHandler) {
    var oSelf = this;
    var subscribe = function() {
        console.log('SUBSCRIBE TO '+txid);
...
p.unsubscribe = function(txid,messageHandler) {
    ...
    var unsubscribe = function() {
        console.log('UNSUBSCRIBE FROM '+txid);
        oSelf.dbSub.unsubscribe(txid);
        oSelf.dbSub.removeListener('message',messageHandler);
    };
    if (!oSelf.dbSub)
