 ...
 * NORDIS_ENV_CONF_OVERRIDE - The full path for an environment-specifc config file (for dev, staging, localhost).
  */
function AppConfig() {
    console.log('NORDIS_ENV_CONF: '+process.env.NORDIS_ENV_CONF);
    if (NORDIS_ENV_CONF_OVERRIDE)
        console.log('NORDIS_ENV_CONF_OVERRIDE: '+process.env.NORDIS_ENV_CONF_OVERRIDE);
...
p.getEnvDescription = function(hOpts){
    ...
    return '\n NORDIS_ENV: ' + process.env.NORDIS_ENV+sApp+sNordis+sPort+sViews+sWorkerId+sApiConsumer+sRedisConf+sMySqlConf;
};
