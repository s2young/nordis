...
p.loadOptions = function(hOpts) {
    ...
    if (oSelf.hClasses.App)
        ...
        oSelf.hClasses.App = {
            ...
            ,bSingleton:true
            ,hProperties:{
                sid:{bPrimary:true,sType:'String',sValue:'App',sMySqlType:'CHAR(100)'}
            }
            ,hExtras:{}
        };
        ...
        if (oSelf.hStats[sStat].fnQuery || oSelf.hStats[sStat].fnValidate) {
            if (oSelf.hClasses[sStat])
                ...
                oSelf.hClasses[sStat] = {
                    sTable: '_AutoClassTbl', sDbAlias: oSelf.hStats[sStat].sDbAlias || oSelf.hStats.sDbAlias, sSource: oSelf.hStats[sStat].sSource || null, bSingleton: true, hProperties: {
                        sid: {bPrimary: true, sType: 'String', sValue: sStat, sMySqlType: 'CHAR(100)'}, notes: {sType: 'String'}
                    }, hExtras: {}
                };

                // Add extras for each granularity.
                ['hour', 'day', 'month', 'year', 'all'].forEach(function (sGrain) {
...
//    }
    ...
    else if (hSettings.bSingleton && !hSettings.hProperties[hSettings.sKeyProperty].sValue)
        oSelf.warn('Singleton class ('+sClass+') not properly configured. Must set default sValue on the primary key property ('+hSettings.sKeyProperty+').');
...
p.init = function (hOpts,fnCallback) {
    ...
    if (!oSelf.oApp) {
        ...
        oSelf.oApp = Base.lookup({sClass:'App'});
    }
 ...
 */
p.trackStat = function(sStat,Params,fnCallback,dDate,nFakeCount){
    var oSelf = this;
    dDate = (dDate) ? moment(dDate).utc() : moment.utc();
    ...
    if (!oSelf.hStats[sStat])
        ...
        oSelf.hStats[sStat].fnValidate(Params,function(err,sKey){
            if (err)
                ...
                if (aKeys.length) {
                    if (sKey)
                        oSelf.Redis.hincrby(aKeys,sKey,nFakeCount||1,fnCallback,oSelf.hStats.sDbAlias);
                    else {
                        oSelf.Redis.incrby(aKeys,nFakeCount||1,fnCallback,oSelf.hStats.sDbAlias);
                    }
                }
            }
        });
...
p.processStats = function(hOpts,fnCallback) {
    ...
    var Base = require('./Base');
    if (hOpts instanceof Function) fnCallback = hOpts;
    if (!oSelf.oApp) oSelf.oApp = Base.lookup({sClass:'App'}); // Retrieve app singleton.
    ...
    async.series([
        ...
        function(callback) {
            for (var sStat in oSelf.hStats) {
                ...
                else if (oSelf.hStats[sStat].fnQuery) // Mysql-based stats provide a query to retrieve the stat with a passed-in date range.
                    aDbStats.push(sStat);
            }
            callback();
        }
            ...
            if (!aRedisStats || !aRedisStats.length)
                ...
                async.forEach(aRedisStats,function(sStat,cb) {
                    async.waterfall([
                        ...
                        // primary redis box.
                        function(cback) {
                            oSelf.Redis.keys(sStat+',*',cback,oSelf.hStats.sDbAlias);
                        }
                                ...
                                async.forEach(aKeys,function(sKey,cback2){
                                    ...
                                    async.waterfall([
                                        ...
                                        if (err || !res)
                                            ...
                                            var dDate = moment({year:aParts[1],month:aParts[2],day:aParts[3],hour:aParts[4]});
                ...
                async.forEach(aDbStats,function(sStat,cb){
                    ...
                    var processMySqlStat = function(hOpts,cb2) {
                        async.parallel([
                            ...
                            function(cb3) {
                                Base.lookup({sClass:'Stat',hQuery:{name:sStat,year:hOpts.year,month:hOpts.month,day:hOpts.day,hour:hOpts.hour}},cb3);
                            }
                            // Look the stat up directly from the model data, using the fnQuery from the stat configuration.
                            ,function(cb3) {
                                var Collection = require('./Collection');
                                new Collection({sClass:hSettings.sClass,sSource:'MySql',nSize:1,hQuery:hSettings.fnQuery({dStart:hOpts.dStart,dEnd:hOpts.dEnd},oSelf)},cb3);
                            }
                            ...
                            if (err)
                                ...
                                oStat.set('count',cColl.nTotal);
                                oStat.set('name',sStat);
                                oStat.set('year',hOpts.year);
                                oStat.set('month',hOpts.month);
                                oStat.set('day',hOpts.day);
                                oStat.set('hour',hOpts.hour);
                                if (hOpts.year)
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        ...
                        var aHours = [{year:null,month:null,hour:null,day:null}];
                        while (dNow < dEnd) {
                            aHours.push({year:dNow.year(),month:dNow.month(),hour:dNow.hour(),day:dNow.date()});
                            dNow.add('hour',1);
                        }
                        async.forEach(aHours,processMySqlStat,cb);
                    } else
...
p.flushStats = function(fnCallback){
    ...
    // We're actually going to call this function twice, in case there are any discrepancies between Redis and MySql. The first
    // will flush Redis and the second MySql.
    var flushStats = function(cb){
        var q = async.queue(function(hOpts,cback){
            if (hOpts.sStat && hOpts.sGrain) {
                ...
                if (!oSelf.oApp) oSelf.oApp = Base.lookup({sClass:'App'});
