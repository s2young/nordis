        ...
        if (oSelf.hStats[sStat].fnQuery || oSelf.hStats[sStat].fnValidate) {
            if (oSelf.hClasses[sStat])
                ...
                oSelf.hClasses[sStat] = {
                    sTable:'_AutoClassTbl'
                    ,sDbAlias:oSelf.hStats.sDbAlias
                    ,bSingleton:true
                    ,hProperties:{
...
//    }
    ...
    if (!hSettings.hProperties)
        throw new Error('Class improperly configured. Missing hProperties hash for '+sClass);
    ...
    if (!hSettings.sKeyProperty)
        throw new Error('No bPrimary property set for '+sClass+'. At least one is required per class so we cannot continue.');
    else if (hSettings.bSingleton && !hSettings.hProperties[hSettings.sKeyProperty].sValue)
        throw new Error('Singleton class ('+sClass+') not properly configured. Must set default sValue on the primary key property ('+hSettings.sKeyProperty+').');
 ...
 * @param fnCallback
 */
p.processStats = function(fnCallback) {
    var oSelf = this;
