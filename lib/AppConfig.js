        ...
        if (oSelf.hClasses[sClass].hImages) {
            ...
            for (var sType in oSelf.hClasses[sClass].hImages) {
                ...
                for (var i = 0; i < oSelf.hClasses[sClass].hImages[sType].aSizes.length; i++) {
                    aImages.push('s'+sType+oSelf.hClasses[sClass].hImages[sType].aSizes[i].nSize);
                }
            }
            oSelf.hClasses[sClass].aProperties = oSelf.hClasses[sClass].aProperties.concat(aImages).concat(['nID','nCreated','nUpdated']);
