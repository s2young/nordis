...
p.processClass = function(sClass,bBuildMySql) {
    ...
    if (hSettings.sKeyProperty) {
        ...
        // an endpoint that follows this pattern: /{{sClass}}/metric/{{sAlias}}
        if (hSettings.hMetrics) {
            if (!oSelf.hEndpointMap) oSelf.hEndpointMap = {};
            for (var sMetric in hSettings.hMetrics) {
                var hMetric = hSettings.hMetrics[sMetric];
                if (hMetric.sAlias) {
                    var sEndpoint = '/metric/'+sClass.toLowerCase()+'/'+hMetric.sAlias;
                    if (!oSelf.hEndpointKeywords['metric']) oSelf.hEndpointKeywords['metric'] = [];
                    oSelf.hEndpointKeywords['metric'].push({sMetric:sMetric,sEndpoint:sEndpoint,sClass:sClass});
                }
            }
        }
    }
...
p.broadcast = function(txid,message,fnCallback) {
    ...
    var broadcast = function() {
        if (txid) {
            ...
            oSelf.error('CANNOT PUBLISH (no txid)',message);
    };
    if (!oSelf.dbPub)
