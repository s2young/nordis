...
p.loadOptions = function(hOpts) {
    ...
    if (!this.hClassMap) this.hClassMap = {};
    for (var sClass in this.hClasses) {
        ...
        if (!this.hClasses[sClass].nClass) {
            throw new Error('Class improperly configured. Missing nClass value for '+sClass);
        } else {
            this.hClassMap[this.hClasses[sClass].nClass] = sClass;
            this.hClassMap[sClass.toLowerCase()] = sClass;
        }
    }
    this.debug('----- APP CLASS MAP -----');
    this.debug(this.hClassMap);
    this.debug('----- END APP CLASS MAP -----');
};
