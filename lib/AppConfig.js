    ...
    if (!oSelf.hStats[sStat])
        ...
        oSelf.hStats[sStat].fnValidate(aParams,function(err,sKey){
            if (err)
                ...
                if (aKeys.length) {
                    if (sKey)
                        oSelf.Redis.hincrby(aKeys,sKey,1,fnCallback,oSelf.hStats.sDbAlias);
                    else {
                        oSelf.Redis.incr(aKeys,fnCallback,oSelf.hStats.sDbAlias);
                    }
                }
            }
        });
                                    ...
                                    async.parallel([
                                        function(cback3){
                                            ...
                                            oSelf.hStats[sStat].fnValidate(null,function(err){
                                                if (err)
                                                    oSelf.Redis.hlen(sKey,cback3,oSelf.hStats.sDbAlias);
                                                else
