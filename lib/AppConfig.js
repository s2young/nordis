...
p.trackStat = function(hOpts,fnCallback){
    ...
    if (!sStat)
        ...
        if (!oApp) {
            ...
            oApp = Base.lookup({sClass:'App'}); // Default App object has sid of 'app.'  Submit oApp option to this method to assign to a tenant app.
        }
        ...
        oSelf.hStats[sStat].fnValidate(Params, function (err, sKey) {
            if (err)
                ...
                if (aKeys.length) {
                    if (sKey)
                        oSelf.Redis.hincrby(aKeys, sKey, hOpts.nFakeCount || 1, fnCallback, oSelf.hStats.sDbAlias);
                    else {
...
p.processStats = function(hOpts,fnCallback) {
    ...
    var oApp = (hOpts && hOpts.oApp) ? hOpts.oApp : Base.lookup({sClass:'App'}); // Will create an App object with sid of 'app' by default. Can pass in custom app object with tenant id if desired.
                                    ...
                                    async.waterfall([
                                        ...
                                        if (err || !res)
                                            ...
                                            var processRedisStat = function(hOpts,callback) {
                                                ...
                                                Base.lookup({sClass:'Stat',hQuery:{app_id:oApp.getKey(),name:hOpts.name,year:hOpts.year,month:hOpts.month,day:hOpts.day,hour:hOpts.hour}},function(err,oStat){
                                                    if (err)
                                                        callback(err);
                                                    else {
                ...
                async.forEach(aDbStats,function(sStat,cb){
                    ...
                    var processMySqlStat = function(hOpts,cb2) {
                        async.parallel([
                            ...
                            function(cb3) {
                                Base.lookup({sClass:'Stat',hQuery:{app_id:oApp.getKey(),name:sStat,year:hOpts.year,month:hOpts.month,day:hOpts.day,hour:hOpts.hour}},cb3);
                            }
                            ...
                            ,function(cb3) {
                                ...
                                new Collection({sClass:hSettings.sClass,sSource:'MySql',nSize:1,hQuery:hSettings.fnQuery(hSubOpts,oSelf)},cb3);
                            }
...
p.flushStats = function(hOpts,fnCallback){
    ...
    var oApp = (hOpts && hOpts.oApp) ? hOpts.oApp : Base.lookup({sClass:'App'});
    ...
    var flushStats = function(cb){
        var q = async.queue(function(hOpts,cback){
            if (hOpts.sStat && hOpts.sGrain) {
                ...
                oApp.loadExtras(hExtras,function(err){
                    if (err)
                        cback(err);
                    else if (oApp[hOpts.sStat] && oApp[hOpts.sStat][hOpts.sGrain]) {
                        oApp[hOpts.sStat][hOpts.sGrain].delete(cback);
                    } else
                        cback();
                });
            } else
