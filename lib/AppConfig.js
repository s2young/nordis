...
p.loadOptions = function(hOpts) {
    ...
    if (oSelf.hClasses.App)
        ...
        oSelf.hClasses.App = {
            nClass:0.1
            ,hProperties:{
                id:{bUnique:true,sType:'Number',nValue:(oSelf.nSeedID-1)}
            }
            ,hExtras:{}
        };
    ...
    for (var sStat in oSelf.hStats) {
        if (oSelf.hStats[sStat].fnQuery || oSelf.hStats[sStat].fnValidate) {
            ['year','month','day','hour'].forEach(function(sGrain){
                oSelf.hClasses.App.hExtras[sStat+'_'+sGrain] = {
                    sType:'Collection'
                    ,sClass:'Stat'
                    ,sOrderBy:'date'
                    ,fnQuery:function(oSelf,App,sProperty) {
                        var aParts = sProperty.split('_');
                        var sGrain = aParts[aParts.length-1];
                        var hResult = {name:sProperty}
    ...
    for (var sProp in hSettings.hProperties) {
        ...
        for (var sExtra in hSettings.hExtras) {
            if (hSettings.hExtras[sExtra].sType.match(/(Object|Collection)/))
                oSelf.hClassMap[sClass.toLowerCase()+'.'+sExtra.toLowerCase()] = hSettings.hExtras[sExtra].sClass;
        }
    }
    ...
    if (!oSelf.hStats[sStat])
        ...
        oSelf.hStats[sStat].fnValidate(aParams,function(err,sKey){
            if (err)
                ...
                // Create name key for redis for each granularity.
                var aKeys = [
...
p.processStats = function(dStart,dEnd,fnCallback) {
    ...
    oSelf.warn('Processing stats for '+dStart.toString()+' thru '+dEnd.toString());
    ...
    var processStat = function(hOpts,callback) {
        ...
        Base.lookup({sClass:'Stat',hQuery:{name:hOpts.name,year:hOpts.year,month:hOpts.month,day:hOpts.day,hour:hOpts.hour}},function(err,oStat){
            if (err)
                ...
                var nOldCount = oStat.get('count')||0;
                oStat.setData(hOpts);
                oStat.set('count',nOldCount+hOpts.count);
                // Calling setExtra on the App singleton will both save the stat record to the stat table
                // and place the stat into the appropriate collection.
                oApp.setExtra(hOpts.name+'_'+hOpts.sGrain,oStat,function(err){
                    // Once we've counted the stat, flush the value from Redis - saves space and confirms that it's been counted.
                    if (err)
                        callback(err);
                    else
                        oSelf.Redis.del(hOpts.sKey,callback);
                });
            }
        })
    };
