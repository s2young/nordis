            ...
            async.waterfall([
                ...
                ,function(oObj,callback) {
                    if (!AppConfig.MySql.hOpts.bSkip && (oSelf.sSource == 'MySql' || !oSelf.getKey())) {
                        ...
                        AppConfig.debug('AppConfig.MySql.hOpts.bSkip:'+AppConfig.MySql.hOpts.bSkip);
                        callback(null,oSelf);
                    }
                }
                ,function(oObj,callback) {
    ...
    var loadExtra = function(hOpts,callback){
        ...
        if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty]) {
                ...
                AppConfig.info('Load '+sProperty+' ('+(sClass||'')+' '+hSettings.hExtras[sProperty].sType+') on '+oParent.sClass+' ('+oParent.getKey()||oParent.getStrKey()+')');
...
p.get = function(sProperty){
    ...
    if (oSelf.hData && sProperty && oSelf.getSettings().hProperties[sProperty]) {
        ...
        AppConfig.debug('NOT FOUND: '+sProperty);
        AppConfig.debug(oSelf.hData);
        AppConfig.debug(oSelf.getSettings().hProperties);
        AppConfig.deug(new Error().stack());
        return null;
    }
};
/**
