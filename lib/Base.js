...
p.loadExtras = function(hExtras,fnCallback,oClient) {
    ...
    if (hExtras) {
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/))
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                                ...
                                async.series([
                                    function(cback) {
                                        if (!hQuery && hSettings.hExtras[sProperty].fnQueryOverride) {
                                            //Config.debug(oParent.sClass+' ('+oSelf.getKey()+').'+sProperty+' fnQueryOverride');
                                    ...
                                    ,function(cback) {
                                        ...
                                        if (hQuery && !oParent[sProperty].getKey()) {
                                            //Config.debug(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' Redis.loadObject');
                                    ...
                                    ,function(cback){
                                        if (hQuery && !oParent[sProperty].getKey()) {
                                            //Config.debug(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' MySql.loadObject');
                                    ...
                                    ,function(cback) {
                                        if (oParent[sProperty].getKey() && hOpts.hExtras) {
                                            ...
                                            //if (!oParent[sProperty].getKey()) //Config.debug('EXTRA NOT FOUND: '+sProperty+' ON '+oParent.sClass+' ('+oParent.getKey()+')');
                                            cback();
                                        }
                                    }
                                ],callback);
                            ...
                            case 'Collection':
                                ...
                                var sOrderBy = (hOpts.hExtras && hOpts.hExtras.sOrderBy) ? (hOpts.hExtras.sOrderBy) : (hOpts.sOrderBy) ? hOpts.sOrderBy : (hSettings.hExtras[sProperty].sOrderBy) ? (hSettings.hExtras[sProperty].sOrderBy instanceof Function) ? hSettings.hExtras[sProperty].sOrderBy(oSelf) : hSettings.hExtras[sProperty].sOrderBy : '';
                                var sExtraSource = (hOpts.hExtras && hOpts.hExtras.sSource) ? hOpts.hExtras.sSource : (hSettings.sSource) ? hSettings.sSource : hOpts.sSource;
                                ...
                                if (hSettings.hExtras[sProperty].fnQueryOverride) {
                                    var hCustom = hSettings.hExtras[sProperty].fnQueryOverride(oParent,Config);
                                    if (hCustom.hQuery) {
                                ...
                                async.series([
                                    ...
                                    ,function(cback){
                                        if (!hOpts || !hOpts.hExtras || !oParent[sProperty].aObjects || !oParent[sProperty].aObjects.length)
                                            cback();
                                        else {

                                            var n = -1;
                                            async.forEach(oParent[sProperty].aObjects,function(oItem,cb){
                                                n++;
                                                oParent[sProperty].aObjects[n] = (oItem instanceof Base) ? oItem : Base.lookup({sClass:oParent[sProperty].sClass,hData:oItem});
                            ...
                            case 'Hash':case 'Array':
                                oParent[sProperty] = (hSettings.hExtras[sProperty].sType=='Hash') ? {} : [];
                                if (hSettings.hExtras[sProperty].fnQuery) {
                                    if (hSettings.hExtras[sProperty].bCallback)
                                        hSettings.hExtras[sProperty].fnQuery(oParent,Config,callback);
                                    else {
        ...
        async.forEachLimit(Object.keys(hExtras),10,function(sProperty,cb){
...
p.deleteHashKey = function(sProperty,sKey) {
    if (this.getHash(sProperty)[sKey])
        delete this.getHash(sProperty)[sKey];
    this.set(sProperty,JSON.stringify(this.getHash(sProperty)));
};
/**
                                ...
                                async.waterfall([
                                    function(callback) {
                                        ...
                                        if (Value instanceof Base) {
                                            var sOrderBy = (hSettings.sOrderBy instanceof Function) ? hSettings.sOrderBy(Value) : hSettings.sOrderBy;
                                            async.parallel([
                                                function(cb){
    ...
    if (sProperty && Value && Value instanceof Base) {
        async.series([
            function(callback) {
                ...
                var hSettings = oSelf.getSettings().hExtras[sProperty];
                var sKey = oSelf.getClass() + ':' + oSelf.getKey() + ':' + sProperty;
                var sOrderBy = (hSettings.sOrderBy instanceof Function) ? hSettings.sOrderBy(oSelf) : hSettings.sOrderBy;
                switch (hSettings.sType) {
                    case 'Collection':
                        // Don't save it again if it doesn't need saving.
                        async.parallel([
