                    ...
                    function(cb){
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            ...
                            case 'Object':
                                ...
                                break;
                            case 'Collection':
                                if (sSource != 'MySql' && oParent[sProperty].hQuery && oParent[sProperty].hQuery.sWhere)
                                    sSource = 'MySql';
...
p.set = function(sProperty,value,bIgnoreDelta){
    ...
    switch (this.hSettings().hProperties[sProperty]) {
        case 'Boolean':
    ...
    App.debug('oldVal: '+oldVal);
    App.debug('newVal: '+this.hData[sProperty]);
    App.debug('bIgnoreDelta: '+bIgnoreDelta);
    App.debug('nID:'+this.get('nID'));
    App.debug(oldVal != this.hData[sProperty]);
    App.debug(sProperty.match(/^(nID|nUpdated|nCreated)$/));

    if (!bIgnoreDelta && this.get('nID') && oldVal != this.hData[sProperty] && !sProperty.match(/^(nID|nUpdated|nCreated)$/)) {
        if (!this.hDelta) this.hDelta = {};
        this.hDelta[sProperty] = {
            old:oldVal,
            new:this.hData[sProperty]
        };
        App.debug(this.hDelta);
    }
};
/**
