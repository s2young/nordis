...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        ...
        var lookup = function(cb){
            oSelf.sSource = (hOpts.sSource) ? hOpts.sSource : (oSelf.getSettings().sDb) ? oSelf.getSettings().sDb : (!AppConfig.Redis.hOpts.bSkip) ? 'Redis' : 'MySql';
...
p.loadExtras = function(hExtras,fnCallback) {
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty]) {
                ...
                if (!sClass) {
                    ...
                    var loadClient = function(sSource,callback){
                        if (sSource == 'Redis' && !AppConfig.Redis.dbPool[sDbAlias])

                            AppConfig.Redis.acquire(callback,sDbAlias);

                        else if (sSource == 'MySql' && (!hClients.MySql || !hClients.MySql[sDbAlias])) {
                            if (!hClients.MySql) hClients.MySql = {};
                            AppConfig.MySql.acquire(function(err,oClient){
                                if (err) {
                                    AppConfig.error(err);
                                    callback(err);
                                } else {
                                    hClients.MySql[sDbAlias] = oClient;
                                    callback();
                                }
                            },sDbAlias);
                        } else {
                                ...
                                case 'Increment':
                                    loadClient('Redis',function(err){
                                        if (err)
                                            ...
                                            AppConfig.Redis.dbPool[sDbAlias].get(sKey,function(err,res){
                                            ...
                                            if (oObj)
                                                ...
                                                loadClient('Redis',function(err){
                                                    if (err)
                                                        ...
                                                        AppConfig.Redis.loadObject({hQuery:hQuery},oParent[sProperty],cback);
                                                });
                                            else
                                                cback(null,null);
                                        }
                                    ...
                                    async.waterfall([
                                        ...
                                        ,function(callback){
                                            if (sSource == 'MySql' || AppConfig.Redis.hOpts.bSkip)
                                                ...
                                                if (hOpts && hOpts.nMin)
                                                    AppConfig.Redis.dbPool[sDbAlias].zcount(sKey,hOpts.nMin,726081834000,callback);
                                                else
                                                    AppConfig.Redis.dbPool[sDbAlias].zcard(sKey,callback);
                                            }
                                        }
                                                ...
                                                async.waterfall([
                                                    function(cb){
                                                        ...
                                                        if (!oParent[sProperty].nFirstID) {
                                                            ...
                                                            AppConfig.Redis.dbPool[sDbAlias].zrevrank(sKey,oParent[sProperty].getClass()+':'+oParent[sProperty].nFirstID,cb);
                                                        } else {
                                                            AppConfig.Redis.dbPool[sDbAlias].zrank(sKey,oParent[sProperty].getClass()+':'+oParent[sProperty].nFirstID,cb);
                                                        }
                                                    }
                                                    ,function(nStart,cb){
                                                        ...
                                                        var handleResult = function(err,aResult) {
                                                            ...
                                                            var loadItem = function(sKey,cback){
                                                                AppConfig.Redis.dbPool[sDbAlias].hgetall(sKey,function(err,res){
                                                                    if (err)
                                                                        cback(err);
                                                        ...
                                                        if (oParent[sProperty].bReverse)
                                                            AppConfig.Redis.dbPool[sDbAlias].zrevrange(sKey,oParent[sProperty].nStart,oParent[sProperty].nEnd,handleResult);
                                                        else
                                                            AppConfig.Redis.dbPool[sDbAlias].zrange(sKey,oParent[sProperty].nStart,oParent[sProperty].nEnd,handleResult);
                                                    }
                                                ],callback);
    ...
    async.forEach(aProps,loadExtra,function(err){
        if (hClients && hClients.MySql)
            for (var sDbAlias in hClients.MySql) {
                AppConfig.MySql.release(hClients.MySql[sDbAlias]);
                delete hClients.MySql[sDbAlias];
            }
