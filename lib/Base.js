...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        ...
        var lookup = function(cb){
            oSelf.sSource = (hOpts.sSource) ? hOpts.sSource : (hOpts && hOpts.sSource=='Redis' && hOpts.sSource!='MySql') ? 'Redis' : (!AppConfig.Redis.hOpts.bSkip) ? 'Redis' : 'MySql';
            async.waterfall([
                function(callback) {
                ...
                ,function(oObj,callback) {
                    if (!oObj.getKey() && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty] && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty].sValue) {
                        if (fnCallback) {
                            oSelf.set(oSelf.getSettings().sKeyProperty,AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty].sValue);
                            callback(null,oSelf);
                        } else
        ...
        if (hOpts.hQuery) {
        ...
        } else if (AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty] && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty].sValue) {
            ...
            hOpts.hQuery = {};
            hOpts.hQuery[AppConfig.hClasses[oSelf.sClass].sKeyProperty] = AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sKeyProperty].sValue;
            lookup(fnCallback);
        } else {
        ...
        if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty]) {
                ...
                if (!sClass) {
                    ...
                    var sDbAlias = AppConfig.hClasses[sClass].sDbAlias || 'default';
                    console.log('Load '+sProperty+' ('+(sClass||'')+' '+hSettings.hExtras[sProperty].sType+') on '+oParent.sClass+' ('+(oParent.getKey()||oParent.getStrKey())+') using db alias '+sDbAlias);
                    ...
                    var sKey = (oParent.getKey()) ? oParent.getClass()+':'+oParent.getKey()+':'+sProperty : null;

                    // We'll load a redis & mysql connection, but only when needed and only once.
                    var loadClient = function(sSource,callback){
                        // TODO: Remove conditionals once Redis supports sDbAlias.
                        if (!hClients[sSource] || (sSource == 'MySql' && !hClients[sSource][sDbAlias])) {
                            if (sSource == 'MySql')
                                if (!hClients[sSource]) hClients[sSource] = {};

                            console.log('loadClient '+sSource+'.'+sDbAlias);
                            AppConfig[sSource].acquire(function(err,oClient){
                                console.log('loadClient back'+sSource+'.'+sDbAlias);
                                if (err) {
                                    AppConfig.error(err);
                                    callback(err);
                                } else {
                                    if (sSource == 'MySql')
                                        hClients[sSource][sDbAlias] = oClient;
                                    else
                                        hClients[sSource] = oClient;
                                    console.log('loaded Client '+sSource+'.'+sDbAlias);
                                    callback();
                                }
                            },sDbAlias);
                        } else {
                            console.log('existingClient '+sSource+'.'+sDbAlias);
                            callback();
                        }
                    };
                            ...
                            switch (hSettings.hExtras[sProperty].sType) {
                                            ...
                                            if (!res && hQuery && !AppConfig.MySql.hOpts.bSkip)
                                                loadClient('MySql',function(err){
                                                    if (err)
                                                        ...
                                                        AppConfig.MySql.loadObject({hQuery:hQuery},oParent[sProperty],cback,hClients.MySql[sDbAlias]);
                                                    }
                                                });
                                            else
                                                cback(null,res);
                                ...
                                case 'Collection':
                                    ...
                                    if (!oParent[sProperty].hQuery)
                                        ...
                                        if (sSource != 'MySql' && !AppConfig.MySql.hOpts.bSkip && oParent[sProperty].hQuery)
                                            sSource = 'MySql';
                                                ...
                                                async.waterfall([
                                                    function(cb){
                                                        ...
                                                        if (!oParent[sProperty].nFirstID) {
                                                            ...
                                                            hClients.Redis.zrevrank(sKey,oParent[sProperty].getClass()+':'+oParent[sProperty].nFirstID,cb);
                                                        } else {
                                                            hClients.Redis.zrank(sKey,oParent[sProperty].getClass()+':'+oParent[sProperty].nFirstID,cb);
                                                        }
                                                    }
                                                    ,function(nStart,cb){
                                                ...
                                                loadClient('MySql',function(err){
                                                    ...
                                                    if (err)
                                                        ...
                                                        AppConfig.MySql.loadCollection({
                                                            ...
                                                            console.log('loadCollection done.');
    ...
    async.forEachLimit(aProps,3,loadExtra,function(err){
        if (hClients)
            for (var sSource in hClients) {
                if (sSource == 'MySql') {
                    for (var sDbAlias in hClients[sSource]) {
                        AppConfig[sSource].release(hClients[sSource][sDbAlias]);
                        delete hClients[sSource][sDbAlias];
                    }
                } else if (AppConfig[sSource] && hClients[sSource]) {
                    AppConfig[sSource].release(hClients[sSource]);
                    delete hClients[sSource];
                }
            }
