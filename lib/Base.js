    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                ...
                Config.debug(oSelf.sClass,hOpts);
                async.series([
                    function(callback) {
                        Config.debug('Redis');
                    ...
                    Config.debug('sSource',oSelf.sSource);
...
p.save = function(hOpts,fnCallback) {
    ...
    async.series([
        ...
        ,function(callback) {
            async.parallel([
                function(cb) {
                    if (!Config.Redis.hOpts.bSkip && (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce))) {
                        Config.Redis.saveObject(hOpts,oSelf,cb);
                    } else
                        cb();
                }
                ,function(cb) {
                    if (!Config.MySql.hOpts.bSkip && (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce))) {
...
p.publish = function() {
    ...
    if (Config.getClasses(oSelf.sClass).sAdapterPath && (bNew || hDelta || bRemoved))
        oSelf.handle(bNew,hDelta,bRemoved,oSelf);
};
...
p.handle = function(bNew,hDelta,bRemoved,sCustomPath,fnCallback){
    ...
    Config.debug('sAdapterPath',sAdapterPath);
    ...
    if (sAdapterPath && !Handler) {
        ...
        try {
            ...
            Config.error('Error when attempting to require adapter for class '+oSelf.sClass+' at path '+sAdapterPath,sAdapterPath);
        } finally {
