...
//                    ,function(callback) {
//                        // Track hit rates? Total hits for each db source?
                ...
                if (!sClass) {
                    ...
                    switch (hSettings.hExtras[sProperty].sType) {
                            ...
                            async.series([
                                ...
                                ,function(cback) {
                                    ...
                                    if (hQuery && !oParent[sProperty].getKey())
                                        Config.Redis.loadObject({hQuery:hQuery,sSource:sSource},oParent[sProperty],cback);
                                    else
                                        cback();
                                }
                                ...
                                ,function(cback){
                                    if (hQuery && !oParent[sProperty].getKey())
                                        Config.MySql.loadObject({hQuery:hQuery},oParent[sProperty],function(err,oResult,oDbClient){
                        ...
                        case 'Collection':
                            ...
                            var sOrderBy = (hOpts.hExtras && (hOpts.hExtras.sScore||hOpts.hExtras.sOrderBy)) ? (hOpts.hExtras.sScore||hOpts.hExtras.sOrderBy) : (hOpts.sScore||hOpts.sOrderBy) ? (hOpts.sScore||hOpts.sOrderBy) : (hSettings.hExtras[sProperty].sScore||hSettings.hExtras[sProperty].sOrderBy) ? (hSettings.hExtras[sProperty].sScore||hSettings.hExtras[sProperty].sOrderBy) : '';
                            ...
                            async.waterfall([
                                function(callback) {
                                    ...
                                    if (Value instanceof Base) {
                                        async.parallel([
                                            function(cb){
                                                if (!Config.Redis.hOpts.bSkip)
                                                    Config.Redis.addToSet({
                                                    sKey:sKey,
                                                    nTTL:nTTL,
                                                    sOrderBy:(hSettings.sScore||hSettings.sOrderBy)
                                                }, Value, cb);
                                                else
                                                    cb();
                                            }
                                            ,function(cb){
                                                if (!Config.MySql.hOpts.bSkip)
    ...
    async.series([
        ...
        ,function(callback) {
            if (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce)) {
                async.parallel([
                    function(cb) {
                        if (!Config.Redis.hOpts.bSkip && oSelf.getSettings().sDb != 'MySql') {
                            Config.Redis.saveObject(hOpts,oSelf,cb);
                        } else
                            cb();
                    }
                    ,function(cb) {
                        if (!Config.MySql.hOpts.bSkip && oSelf.getSettings().sDb != 'Redis') {
        ...
        oSelf.processExtras(oSelf,oSelf.bNew,oSelf.hDelta,oSelf.bRemoved);
        oSelf.publish();
...
p.delete = function(fnCallback) {
    ...
    if (oSelf.getKey()) {
        async.parallel([
            ...
            oSelf.bRemoved = true;
            oSelf.processExtras(oSelf,null,null,oSelf.bRemoved);
            oSelf.publish();
...
p.processExtras = function(bNew,hDelta,bRemoved) {
    var oSelf = this;
    // See if this item belongs in an 'extra' on another-related class.
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEachLimit(Config.hExtrasByClass[oSelf.sClass],1,function(hOpts,callback) {
            //process.env.sExtraBusiness += ',1';
            ...
            if (data) {
                if (data instanceof Array) {
                    async.forEachLimit(data,1,function(hItem,cb){
                        var oParent = Base.lookup({sClass:hOpts.sParent,hData:hItem});
                        if (oSelf.bRemoved) {
//                            console.log(hOpts.sParent+'.deleteExtra('+hOpts.sExtra+')');
                            oParent.deleteExtra(hOpts.sExtra,oSelf,cb);
                        } else {
//                            console.log(hOpts.sParent+'.setExtra('+hOpts.sExtra+')');
...
p.handle = function(sCustomPath,fnCallback){
    ...
    if (sCustomPath) sAdapterPath = sCustomPath;
