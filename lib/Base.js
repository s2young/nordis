                ...
                async.waterfall([
                    function(callback) {
                        if (oSelf.sSource == 'Redis')
                            App.Redis.load(hOpts,oSelf,callback);
                        else
                            callback(null,null);
                    }
                                        ...
                                        if (sSource == 'Redis' && hSettings.hExtras[sProperty].aKey && oParent.get(hSettings.hExtras[sProperty].aKey[0]))
                                            loadClient('Redis',function(err){
                                                if (err)
                                                    ...
                                                    App.Redis.load({hQuery:{nID:oParent.get(hSettings.hExtras[sProperty].aKey[0])}},oParent[sProperty],cback,hClients.Redis);
                                            });
                                        else
                                           cback(null,null);
                                    }
                                ...
                                async.waterfall([
                                    ...
                                    ,function(nTotal,callback){
                                        ...
                                        if (nTotal) {
                                            ...
                                            oParent[sProperty].nIndex = (hOpts) ? hOpts.nIndex || 0 : 0;
                                            ...
                                            if (!oParent[sProperty].nFirstID) {
                                                ...
                                                var handleResult = function(err,aResult) {
                                                    ...
                                                    if (oParent[sProperty].nSize && aResult[oParent[sProperty].nSize]) {
                                                        oParent[sProperty].nNextID = aResult[oParent[sProperty].nSize].toString().replace(/^[^:]*\:/,'');
                                                        aResult.splice(-1,1);
                                            ...
                                            loadClient('MySql',function(err){
                                                if (err)
                                                    ...
                                                    App.MySql.loadCollection({hQuery:hSettings.hExtras[sProperty].fnQuery(oParent,App)},oParent[sProperty],function(err){
