    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                async.waterfall([
                    function(callback) {
                       if ((hOpts && hOpts.sSource=='Redis' && hOpts.sSource!='MySql') || ((!hOpts || !hOpts.hQuery.sWhere) && !App.Redis.hSettings.bSkip)) {
                    ...
                    ,function(oObj,callback) {
                        if ((hOpts && hOpts.sSource=='MySql' && hOpts.sSource!='Redis') || (!oSelf.get('nID') && !App.MySql.hSettings.bSkip)) {
...
p.loadExtras = function(hExtras,fnCallback) {
    ...
    var oRedisClient = (hExtras.oRedisClient) ? hExtras.oRedisClient : null;
    delete hExtras.oRedisClient;
            ...
            if (!hSettings.hExtras[sProperty]) {
                ...
                async.series([
                    ...
                    ,function(cb){
                        ...
                        switch (sProperty.substring(0,1)) {
                            case 'n':
                                oRedisClient.get(sKey,function(err,res){
                            ...
                            case 'o':
                                async.waterfall([
                                    function(cback) {
                                        if (hSettings.hExtras[sProperty].aKey && oParent.get(hSettings.hExtras[sProperty].aKey[0]))
                                            oRedisClient.hgetall(oParent[sProperty].nClass+':'+oParent.get(hSettings.hExtras[sProperty].aKey[0]),cback);
                                        else
                                           cback(null,null);
                                    }
                                    ,function(res,cback) {
                                        oParent[sProperty] = Base.lookup({sClass:hSettings.hExtras[sProperty].sClass,hData:res});
                                        if (res && hOpts.hExtras) {
                                            hOpts.hExtras.oRedisClient = oRedisClient;
                            ...
                            case 'c':
                                ...
                                async.waterfall([
                                    function(callback){
                                        if (hOpts && hOpts.nMin)
                                            oRedisClient.zcount(sKey,hOpts.nMin,726081834000,callback);
                                        else
                                            oRedisClient.zcard(sKey,callback);
                                    }
                                        ...
                                        if (!oParent[sProperty].nFirstID) {
                                            ...
                                            var handleResult = function(err,aResult) {
                                                ...
                                                var loadItem = function(sKey,cback){
                                                    oRedisClient.hgetall(sKey,function(err,res){
                                                        if (err)
                                                            cback(err);
                                            ...
                                            if (oParent[sProperty].bReverse)
                                                oRedisClient.zrevrange(sKey,oParent[sProperty].nStart,oParent[sProperty].nEnd,handleResult);
                                            else
                                                oRedisClient.zrange(sKey,oParent[sProperty].nStart,oParent[sProperty].nEnd,handleResult);
                                        }
                                    }
                                ],cb);
                                break;
...
//                                aMySqlProps.forEach(function(sProperty,n){
        ...
        if (!Value) {
            ...
            if (oSelf[sProperty]) {
                while (oSelf[sProperty].next()) {
                    if (parseInt(oSelf[sProperty].getItem().get('nID')) == parseInt(Value.get('nID'))) {
