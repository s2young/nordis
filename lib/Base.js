...
p.publish = function() {
    ...
    if (App.hClasses[oSelf.sClass].sAdapterPath && (oSelf.bNew || oSelf.hDelta || oSelf.bRemoved)) {
        ...
        console.log(sAdapterPath);
        var Handler = require.cache[sAdapterPath];
        if (!Handler) {
            if (fs.existsSync(sAdapterPath))
                Handler = require(sAdapterPath);
            else
                App.error('Adapter path not found: '+sAdapterPath);
        }
        if (Handler)
...
module.exports.lookup = function(hOpts,fnCallback) {
    ...
    if (App.hClasses[hOpts.sClass] && App.hClasses[hOpts.sClass].sPath) {
        var oClass = require(App.NORDIS_ENV_ROOT_DIR+App.hClasses[hOpts.sClass].sPath);
