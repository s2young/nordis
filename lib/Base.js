    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                ...
                async.series([
                    function(callback) {
                        process.stdout.write(oSelf.sClass+' Redis.loadObject');
...
p.loadExtras = function(hExtras,fnCallback) {
    ...
    if (hExtras && hExtras instanceof Object && Object.keys(hExtras) && Object.keys(hExtras).length) {
                                    ...
                                    async.series([
                                        ...
                                        ,function(cback) {
                                            ...
                                            if (hQuery && !oSelf[sProperty].getKey()) {
                                                //Config.debug
                                                process.stdout.write(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' Redis.loadObject');
        ...
        async.forEachOf(Object.keys(hExtras),function(sProperty,ind,cb){
...
p.deleteExtra = function(sProperty,Value,fnCallback) {
    ...
    if (oSelf && oSelf.getKey() && sProperty && Value && Value instanceof Base) {
        ...
        console.log('NO deleteExtra callback on '+oSelf.sClass+'.'+sProperty);
};
/**
...
p.processExtras = function(bNew,hDelta,bRemoved,fnCallback) {
    ...
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEachOf(Config.hExtrasByClass[oSelf.sClass],function(hOpts,ind,callback) {
            ...
            Config.hClasses[hOpts.sParent].hExtras[hOpts.sExtra].fnCreate(oSelf,Config,bNew,hDelta,bRemoved,function(err,data){
                if (data instanceof Array) {
                    async.forEachOf(data,function(hItem,ind,cb){
