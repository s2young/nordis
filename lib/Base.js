        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                ...
                async.series([
                    ...
                    ,function(callback) {
                        if (Config.Mongo)
                            Config.Mongo.loadObject(hOpts,oSelf,callback);
                        else
                            callback();
                    }
                    ,function(callback) {
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/))
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            ...
                            case 'Collection':
                                ...
                                var hQuery;
                                var hConf = (hSettings.hExtras[sProperty].fnQuery) ?  hSettings.hExtras[sProperty].fnQuery(oSelf,Config,sProperty) : null;
                                if (hConf.hQuery) {
                                    hQuery = hConf.hQuery;
                                    delete hConf.hQuery;
                                    if (nMax==undefined && hConf.nMax != undefined) nMax = hConf.nMax;
                                    if (nMin==undefined && hConf.nMin != undefined) nMin = hConf.nMin;
                                    if (bReverse==undefined && hConf.bReverse != undefined) bReverse = hConf.bReverse;
                                }
...
p.save = function(hOpts,fnCallback,fnAdapterCallback) {
    ...
    var bCallDb = (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce));
    ...
    async.series([
        ...
        ,function(callback) {
            if (hDelta && Object.keys(hDelta) && Object.keys(hDelta).length) oSelf.setUpdateTime();
            callback();
        }
        ,function(callback) {
            if (bCallDb)
        ...
        ,function(callback) {
            if (Config.Mongo && bCallDb)
                Config.Mongo.saveObject(hOpts,oSelf,callback);
            else
                callback();
        }
        ,function(callback) {
 ...
 * @param fnCallback
 */
p.delete = function(fnCallback,bSkipMySql) {
    var oSelf = this;
    if (oSelf.getKey()) {
        async.parallel([
            function(callback) {
            ...
            ,function(callback) {
                if (Config.Mongo) {
                    console.log('Call Mongo.deleteObject');
                    Config.Mongo.deleteObject(oSelf,callback);
                } else
                    callback();
            }
            ,function(callback) {
