...
p.getRedisKey = function(sKeyValue,sProperty) {
    var oSelf = this;
    if (!oSelf._sRedisKey) {
        oSelf._sRedisKey = oSelf.nClass || oSelf.getSettings().nClass || oSelf.sClass;
        if (sProperty=='cMsgPlatforms') {
            console.log(oSelf.getSettings().sTenantProperty,oSelf.hData);
            console.log(oSelf.getSettings().sTenantProperty,oSelf.get(oSelf.getSettings().sTenantProperty));
        }

        if (oSelf.getSettings().sTenantProperty && oSelf.get(oSelf.getSettings().sTenantProperty)) {
            oSelf._sRedisKey += ':'+oSelf.get(oSelf.getSettings().sTenantProperty);
            if (sProperty=='cMsgPlatforms') {
                console.log('CI',oSelf._sRedisKey);
            }
        }


        if (sKeyValue)
            oSelf._sRedisKey += ':'+sKeyValue
        else
            oSelf._sRedisKey += ':'+oSelf.getKey();

    }
    return oSelf._sRedisKey;
};
/**
...
p.processExtras = function(bNew,hDelta,bRemoved,fnCallback) {
    ...
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEach(Config.hExtrasByClass[oSelf.sClass],function(hOpts,callback) {
            ...
            if (hOpts.sExtra=='cMsgPlatforms') console.log(hOpts,data);
            if (data) {
