            ...
            if (fnCallback) {
                ...
                async.waterfall([
                    ...
                    ,function(oObj,callback) {
                        if (oSelf.sSource == 'MySql' || (!oSelf.getNumKey() && !App.MySql.hSettings.bSkip)) {
...
p.getImageHash = function() {
    var oSelf = this;
    if (oSelf.getSettings() && oSelf.getSettings().aImages) {
        var hResult = {};
        for (var i = 0; i <  oSelf.getSettings().aImages.length; i++) {
            hResult[oSelf.getSettings().aImages[i]] = oSelf.get(oSelf.getSettings().aImages[i]);
        }
        return hResult;
    }
};
