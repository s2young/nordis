                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            ...
                            case 'Collection':
                                ...
                                async.series([
                                    ...
                                    ,function(cback){
                                        if (sExtraSource != 'Redis' && oSelf.getKey() && (!oSelf[sProperty] || !oSelf[sProperty].nTotal) && hQuery) {
...
p.getRedisKey = function(sKeyValue) {
    ...
    if (oSelf._Tenant) _sRedisKey += ':'+oSelf._Tenant;
...
p.processExtras = function(bNew,hDelta,bRemoved,fnCallback) {
    ...
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEach(Config.hExtrasByClass[oSelf.sClass],function(hOpts,callback) {
            ...
            var data = Config.hClasses[hOpts.sParent].hExtras[hOpts.sExtra].fnCreate(oSelf,Config,bNew,hDelta,bRemoved,callback);
            if (data) {
                ...
                Config.debug(hOpts.sExtra+ ' processed, but no data returned. Make sure there is a callback handler!');
