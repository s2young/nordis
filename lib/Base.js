        ...
        async.each(Object.keys(hExtras),function(sProperty,cb){
            ...
            if (sProperty && !sProperty.match(/^(nFirstID|sFirstID|nMax|nMin|nSize|sView|sSource)$/)) {
                ...
                if (Config.logger.level.match(/^(debug|silly)$/)) {
                    ...
                    oSelf._Timers[oSelf.sClass+'['+oSelf.getKey()+'].'+sProperty] = setTimeout(function(){
                        Config.fatal('EXTRA NEVER LOADED! '+oSelf.sClass+'['+oSelf.getKey()+'].'+sProperty);
                    },5000);
                }
                ...
                loadExtra({
                    ...
                    if (Config.logger.level.match(/^(debug|silly)$/)) clearTimeout(oSelf._Timers[oSelf.sClass+'['+oSelf.getKey()+'].'+sProperty]);
                    cb();
                });
            } else
                cb();
