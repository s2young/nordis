            ...
            if (fnCallback) {
                ...
                async.waterfall([
                    ...
                    ,function(oObj,callback) {
                        AppConfig.MySql.loadObject(hOpts,oSelf,function(err,oResult){
                            if ((oResult instanceof Base)===false) oMySqlClient = oResult;
                            callback(err,null);
                        });
                    }
                    ,function(o,callback) {
                        if (hOpts && hOpts.hExtras)
                            oSelf.loadExtras(hOpts.hExtras,callback,oMySqlClient);
                        else
                            callback(null,oSelf);
                    }
                ],function(err){
                    if (oMySqlClient)
                        oMySqlClient.release();
                                    ...
                                    if (oParent.getKey() && (!oParent[sProperty] || !oParent[sProperty].nTotal)) {
                                        ...
                                        if (hQuery)
                                            AppConfig.MySql.loadCollection({
                                                ...
                                                ,nMax:nMax
                                                ,nMin:nMin
                                            },oParent[sProperty],cback,oClient);
                                        else
                                            cback();
                                    } else
                                        cback();
