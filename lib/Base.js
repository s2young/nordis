 ...
 * @param sClass
 */
module.exports.loadAppSingleton = function(sid,callback) {
    new Base({sClass:'App',hQuery:{sid:sid}},function(err,oApp){
        if (err)
            callback(err);
        else if (!oApp.getKey()) {
            oApp.set('sid',sid);
 ...
 */
module.exports.requestP = function(method,url,form) {
    AppConfig.debug(method+' - '+url);

    return new promise(function (resolve, reject) {
        request[method.toLowerCase()](url,{form:form}, function (err, res, body) {
            if (err) {
                return reject(err);
