...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        if (hOpts.sClass) {
            ...
            if (p.Config.getClasses(oSelf.sClass) && p.Config.getClasses(oSelf.sClass).nClass) oSelf.nClass = p.Config.getClasses(oSelf.sClass).nClass;
        } else if (hOpts.nClass) {
            oSelf.nClass = hOpts.nClass;
            oSelf.sClass = p.Config.getClassMap(oSelf.nClass).sClass;
        }
        if (!oSelf.sClass) {
            delete hOpts.hQuery;
            p.Config.error('Must supply either nClass or sClass to create an object, and set classes properly in configuration. '+JSON.stringify(hOpts));
        }
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                if (!hOpts.sSource)
                    hOpts.sSource = (oSelf.getSettings().sDb) ? oSelf.getSettings().sDb : (!p.Config.get('Redis').hOpts.bSkip) ? 'Redis' : 'MySql';

                async.waterfall([
                    function(callback) {
                        p.Config.get('Redis').loadObject(hOpts,oSelf,callback);
                    }
                    ,function(oObj,callback) {
                        p.Config.get('MySql').loadObject(hOpts,oSelf,function(err,oClient){
                            if (!err && hOpts && hOpts.hExtras)
                                oSelf.loadExtras(hOpts.hExtras,function(err){
                                    p.Config.get('MySql').release(oClient);
                                    callback(err,oSelf);
                                },oClient);
                            else {
                                p.Config.get('MySql').release(oClient);
...
p.loadExtras = function(hExtras,fnCallback,oClient) {
    ...
    var bFlushConn = (oClient==undefined);
    ...
    var loadExtra = function(hOpts,callback){
        ...
        if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty])
                ...
                if (!sClass) {
                    p.Config.warn('Skipping extra lookup of '+sProperty+' ('+(sClass||'')+' '+hSettings.hExtras[sProperty].sType+') on '+oParent.sClass+' ('+(oParent.getKey())+')');
                    callback();
                } else {
                    ...
                    switch (hSettings.hExtras[sProperty].sType) {
                        case 'Increment':
                            if (oParent.getKey())
                                p.Config.get('Redis').get(oParent.getClass()+':'+oParent.getKey()+':'+sProperty,function(err,res){
                        ...
                        case 'Object':
                            ...
                            if (hSettings.hExtras[sProperty].fnQuery)
                                hQuery = hSettings.hExtras[sProperty].fnQuery(oParent,p.Config,sProperty);
                            else {
                            ...
                            async.series([
                                ...
                                ,function(cback) {
                                    ...
                                    if (hQuery && !oParent[sProperty].getKey())
                                        p.Config.get('Redis').loadObject({hQuery:hQuery,sSource:sSource},oParent[sProperty],cback);
                                    else
                                        cback();
                                }
                                ...
                                ,function(cback){
                                    if (hQuery && !oParent[sProperty].getKey())
                                        p.Config.get('MySql').loadObject({hQuery:hQuery},oParent[sProperty],function(err,oResult){
                                            if (bFlushConn && oResult) oClient = oResult;
                                            cback(err);
                                        },oClient);
                                    else
                                        cback();
                        ...
                        case 'Collection':
                            ...
                            async.series([
                                function(cback) {
                                    if (oParent.getKey()) {
                                        p.Config.get('Redis').loadCollection({
                                ...
                                ,function(cback){
                                    if (oParent.getKey() && (!oParent[sProperty] || !oParent[sProperty].nTotal)) {
                                        var hQuery = (hSettings.hExtras[sProperty].fnQuery) ?  hSettings.hExtras[sProperty].fnQuery(oParent,p.Config,sProperty) : null;
                                        if (hQuery) {
                                            p.Config.get('MySql').loadCollection({
                                                hQuery: hQuery, bReverse: bReverse, sOrderBy: sOrderBy, nSize: nSize, sFirstID: sFirstID || nFirstID, nMax: nMax, nMin: nMin
                                            }, oParent[sProperty], function (err, oResult) {
                                                if (bFlushConn && oResult) oClient = oResult;
                                                cback(err);
                                            }, oClient);
                                        } else
                                    ...
                                    if (!oParent[sProperty].nTotal || !hOpts || !hOpts.hExtras)
                                        ...
                                        var loadItem = function(oItem,cback){
                                            if (oItem instanceof Base)
                                                oItem.loadExtras(hOpts.hExtras,cback,oClient);
                                            else
                                                cback();
                                        };

                                        var q = async.queue(loadItem,10);
                                        q.drain = cback;
            ...
            async.series([
                function(callback){
                    if (Value instanceof Base && (!Value.getKey() || Value.hDelta)) {
                        p.Config.debug('Calling save on '+Value.sClass+' ('+oSelf.sClass+'.'+sProperty+')');
                ...
                ,function(callback) {
                    ...
                    switch (hSettings.sType) {
                        case 'Collection':
                            ...
                            async.waterfall([
                                function(callback) {
                                    ...
                                    if (Value instanceof Base) {
                                        async.parallel([
                                            function(cb){
                                                if (!p.Config.get('Redis').hOpts.bSkip)
                                                    p.Config.get('Redis').addToSet({
                                                    sKey:sKey,
                                                    nTTL:nTTL,
                                            ...
                                            ,function(cb){
                                                if (!p.Config.get('MySql').hOpts.bSkip)
                                                    p.Config.get('MySql').addToSet({
                                                        sKey:sKey,
                                                        sOrderBy:hSettings.sOrderBy
                                                    }, Value, cb);
                                                else
                            ...
                            break;
                        case 'Increment':
                            p.Config.get('Redis').increment({oObj:oSelf,sProperty:sProperty, nTTL:nTTL}, Value, function (err, res) {
                                if (res)
                                    oSelf[sProperty] = res;
...
p.deleteExtra = function(sProperty,Value,fnCallback) {
    ...
    if (sProperty && Value && Value instanceof Base) {
        async.series([
            function(callback){
                if (!Value.bRemoved) {
                    p.Config.debug('deleteExtra: DELETE '+Value.sClass+' ('+Value.getKey()+')');
                    ...
                    switch (hSettings.sType) {
                        case 'Collection':
                            ...
                            async.parallel([
                                    ...
                                    if (oSelf[sProperty].nTotal) {
                                        ...
                                        while (oSelf[sProperty].next()) {
                                            if (oSelf[sProperty].getItem().getKey() == Value.getKey()) {
                                                p.Config.debug('deleteExtra: SPLICE ITEM '+n);
                                ...
                                ,function(cb){
                                    p.Config.get('Redis').removeFromSet({
                                        sKey:sKey,
                                        sOrderBy:hSettings.sOrderBy
                                    }, Value, cb);
                                }
                            ...
                            break;
                        case 'Increment':
                            p.Config.get('Redis').del(oSelf.getClass()+':'+oSelf.getKey()+':'+sProperty, function (err) {
                                callback(err,oSelf[sProperty]);
                            });
    ...
    async.series([
        ...
        ,function(callback) {
            if (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce)) {
                async.parallel([
                    function(cb) {
                        if (!p.Config.get('Redis').hOpts.bSkip && oSelf.getSettings().sDb != 'MySql')
                            p.Config.get('Redis').saveObject(hOpts,oSelf,cb);
                        else
                            cb();
                    }
                    ,function(cb) {
                        if (!p.Config.get('MySql').hOpts.bSkip && oSelf.getSettings().sDb != 'Redis') {
                            p.Config.get('MySql').saveObject(hOpts,oSelf,cb);
                        } else
                            cb();
                    }
                ],callback);
...
p.setID = function(fnCallback) {
    ...
    if (!oSelf.getKey()) {
        ...
        if (!oSelf.getSettings().bAutoIncrementKey)
            p.Config.get('Redis').getNextID(oSelf,fnCallback);
        else
...
p.getSettings = function() {
    return p.Config.getClasses(this.sClass) || {};
};
/**
...
p.delete = function(fnCallback) {
    ...
    if (oSelf.getKey()) {
        async.parallel([
            function(callback) {
                p.Config.get('Redis').deleteObject(oSelf,callback);
            }
            ,function(callback) {
                p.Config.get('MySql').deleteObject(oSelf,callback);
            }
        ],function(err){
            oSelf.bRemoved = true;
            oSelf.publish();
...
p.toSampleHash = function() {
    var oSelf = this;
    for (var sProp in p.Config.getClasses(oSelf.sClass).hProperties) {
        if (p.Config.getClasses(oSelf.sClass).hProperties[sProp].sSample != undefined)
            oSelf.set(sProp,p.Config.getClasses(oSelf.sClass).hProperties[sProp].sSample,true);
    }
    return oSelf.toHash();
};
...
p.publish = function() {
    ...
    if (p.Config.getClasses(oSelf.sClass).sAdapterPath && (oSelf.bNew || oSelf.hDelta || oSelf.bRemoved)) {
        oSelf.handle();
...
p.handle = function(sCustomPath){
    var oSelf = this;

    // Once the handler is loaded, we don't need to do it again. So see if it is loaded in the cache.
    var sAdapterPath = (p.Config.NORDIS_ENV_ROOT_DIR) ? p.Config.NORDIS_ENV_ROOT_DIR+p.Config.getClasses(oSelf.sClass).sAdapterPath   : p.Config.getClasses(oSelf.sClass).sAdapterPath;
    if (sCustomPath)
        sAdapterPath = sCustomPath;
...
p.toTimestamp = function(value) {
    if (validator.isInt(value))
        ...
        if (date)
            ...
            p.Config.warn('Cannot do anything with '+value+'. Tried converting to timestamp.');
    }
};
...
module.exports.lookup = function(hOpts,fnCallback) {
    if (!hOpts.sClass && hOpts.nClass)
        hOpts.sClass = p.Config.getClassMap(hOpts.nClass);

    if (p.Config.getClasses(hOpts.sClass) && p.Config.getClasses(hOpts.sClass).sClassPath) {
        var sClassPath = (p.Config.getEnv('NORDIS_ENV_ROOT_DIR')) ? p.Config.getEnv('NORDIS_ENV_ROOT_DIR') + p.Config.getClasses(hOpts.sClass).sClassPath  : p.Config.getClasses(hOpts.sClass).sClassPath;
