...
p.loadExtras = function(hExtras,fnCallback,oClient) {
    ...
    if (aProps.length)
        async.forEach(aProps,loadExtra,function(err){
            ...
            if (fnCallback)
                process.nextTick(function() { fnCallback(err,oSelf) })
        });
    else {
        if (bPreserve && oClient)
            Config.MySql.release(oClient);
...
p.set = function(sProperty,value,bIgnoreDelta){
    if (sProperty && this.hData && this.getSettings().hProperties[sProperty] && !this.getSettings().hProperties[sProperty].bReadOnly) {
        ...
        switch (this.getSettings().hProperties[sProperty].sType) {
            case 'Boolean':
                oldVal = validator.toBoolean(oldVal);
    ...
    if (sProperty && Value) {
        ...
        if (!hSettings)
            ...
            async.series([
                ...
                if (fnCallback) process.nextTick(function() { fnCallback(err,oSelf) })
            });
        }
    } else if (fnCallback)
        process.nextTick(function() { fnCallback(err,oSelf) })
        ...
        async.forEach(Config.hExtrasByClass[oSelf.sClass],function(hOpts,callback) {
            ...
            if (data) {
                if (data instanceof Array) {
                    async.forEach(data,1,function(hItem,cb){
            ...
            if (fnCallback) process.nextTick(function() { fnCallback(err) })
        });
    else if (fnCallback)
        fnCallback();
