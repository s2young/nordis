                ...
                if (!sClass) {
                    ...
                    switch (hSettings.hExtras[sProperty].sType) {
                        ...
                        case 'Collection':
                            ...
                            var sSource = (hOpts.hExtras && hOpts.hExtras.sSource) ? hOpts.hExtras.sSource : (hOpts.sSource) ? hOpts.sSource : null;
                                    ...
                                    if (!oParent[sProperty].nTotal || !hOpts || !hOpts.hExtras)
                                        ...
                                        async.forEachLimit(oParent[sProperty].aObjects,10,function(oItem,cb){
                                            if ((oItem instanceof Base)===false)
                                                oItem = Base.lookup({sClass:oParent[sProperty].sClass,hData:oItem})
            ...
            if (data) {
                if (data instanceof Array) {
                    async.forEachLimit(data,1,function(hItem,cb){
                        ...
                        if (oSelf.bRemoved) {
                            Config.debug(hOpts.sParent+'.deleteExtra('+hOpts.sExtra+')');
                            oParent.deleteExtra(hOpts.sExtra,oSelf,cb);
                        } else {
                            Config.debug(hOpts.sParent+'.setExtra('+hOpts.sExtra+')');
