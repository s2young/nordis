                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            ...
                            case 'Collection':
                                ...
                                async.series([
                                    ...
                                    ,function(cback){
                                        if (sExtraSource != 'Redis' && oSelf[sProperty].sSource != 'Redis' && oSelf.getKey() && (!oSelf[sProperty] || !oSelf[sProperty].nTotal) && hQuery) {
        ...
        async.forEachOf(Object.keys(hExtras),function(sProperty,ind,cb){
            ...
            if (sProperty && !sProperty.match(/^(nFirstID|sFirstID|nMax|nMin|nSize|sView)$/))
                ...
                loadExtra({
                    ...
                    if (oSelf[sProperty]) {
                        if (!oSelf[sProperty]._Extras) oSelf[sProperty]._Extras = '';
                        oSelf[sProperty]._Extras += oSelf.sClass;
                        if (oSelf.getKey()) oSelf[sProperty]._Extras += '('+oSelf.getKey()+')';
                        console.log(oSelf[sProperty]._Extras);
                    }
    ...
    if (sProperty && Value) {
        ...
        if (!hSettings)
            ...
            async.series([
                ...
                ,function(callback) {
                    if (oSelf[sProperty] && hSettings.sType=='Collection')
...
p.delete = function(fnCallback) {
    ...
    if (oSelf.getKey()) {
        async.parallel([
            ...
            ,function(callback) {
                oSelf.bRemoved = true;
                oSelf.processExtras(null,null,true,callback);
                //callback();
            }
        ],function(err){
            oSelf.publish();
