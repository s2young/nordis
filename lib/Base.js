...
p.loadExtras = function(hExtras,fnCallback) {
    ...
    if (hExtras && hExtras instanceof Object && Object.keys(hExtras) && Object.keys(hExtras).length) {
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/))
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            case 'Increment':
                                ...
                                if (oSelf.getKey())
                                    Config.Redis.get(oSelf.getRedisKey(null,sProperty)+':'+sProperty,function(err,res){
                                    ...
                                    function(cback) {
                                        if (oSelf.getKey()) {
                                            Config.Redis.loadCollection({
                                                sKey:oSelf.getRedisKey(null,sProperty)+':'+sProperty
        ...
        async.forEach(Object.keys(hExtras),function(sProperty,cb){
            ...
            //Config.debug(oSelf.sClass+'.'+sProperty+' START!');
            if (sProperty && !sProperty.match(/^(nFirstID|sFirstID|nMax|nMin|nSize|sView)$/))
                ...
                loadExtra({
                    ...
                    //Config.debug(oSelf.sClass+'.'+sProperty+' DONE!',err);
                    cb();
                });
    ...
    if (sProperty && Value) {
        ...
        if (!hSettings)
            ...
            async.series([
                ...
                ,function(callback) {
                    if (oSelf.getKey()) {
                        var sKey = oSelf.getRedisKey(null,sProperty) + ':' + sProperty;
                        console.log('setExtra: '+sKey);
            ...
            function(callback) {
                ...
                switch (hSettings.sType) {
                    case 'Collection':
                        ...
                        break;
                    case 'Increment':
                        Config.Redis.del(oSelf.getRedisKey(null,sProperty)+':'+sProperty, function (err) {
                            callback(err,oSelf[sProperty]);
                        });
...
p.save = function(hOpts,fnCallback,fnAdapterCallback) {
    ...
    async.series([
        ...
        ,function(callback) {
            if (hOpts && hOpts.bWaitForExtras)
                oSelf.processExtras(oSelf.bNew,oSelf.hDelta,oSelf.bRemoved,callback);
            else {
                if (!hOpts || !hOpts.bSkipExtras) oSelf.processExtras(oSelf.bNew,oSelf.hDelta,oSelf.bRemoved);
                callback();
            }
        }
        ,function(callback) {
            if (hOpts && hOpts.bWaitForAdapter)
                oSelf.publish(callback);
            else {
                if (!hOpts || !hOpts.bSkipAdapter) oSelf.publish();
                callback();
            }
        }
    ],function(err){
...
p.getRedisKey = function(sKeyValue,sProperty) {
    if (!this._sRedisKey) {
        this._sRedisKey = this.nClass || this.getSettings().nClass || this.sClass;
        if (this.getSettings().sTenantProperty && this.get(this.getSettings().sTenantProperty))
            this._sRedisKey += ':'+this.get(this.getSettings().sTenantProperty);

        if (sKeyValue)
            this._sRedisKey += ':'+sKeyValue
        else
            this._sRedisKey += ':'+this.getKey();

        console.log('HEY:'+this._sRedisKey,sProperty);
    }
    return this._sRedisKey;
};
/**
...
p.publish = function(fnAdapterCallback) {
    var oSelf = this;
    var bNew = oSelf.bNew;
    ...
    if (Config.hClasses[oSelf.sClass].sAdapterPath && (bNew || hDelta || bRemoved))
        oSelf.handle(bNew,hDelta,bRemoved,null,fnAdapterCallback);
