...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        if (hOpts.sClass) {
            ...
            oSelf.sClass = (Config.getClassMap(oSelf.nClass)) ? Config.getClassMap(oSelf.nClass).sClass : null;
        }
        if (!oSelf.sClass) {
            delete hOpts.hQuery;
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                ...
                async.series([
                    function(callback) {
                        //Config.debug(oSelf.sClass+' Redis.loadObject');
                        Config.Redis.loadObject(hOpts,oSelf,callback);
                    }
                    ,function(callback) {
                        //Config.debug(oSelf.sClass+' MySql.loadObject');
                        Config.MySql.loadObject(hOpts,oSelf,callback);
                    }
                    ,function(callback) {
                        if (oSelf.sSource == 'MySql' && hOpts.sSource != 'MySql') {
                            // Figure out how to track Redis MISSES in some way.  Maybe a queue.
                        }

                        if (hOpts && hOpts.hExtras) {
                            //Config.debug(oSelf.sClass+' loadExtras');
                            oSelf.loadExtras(hOpts.hExtras,callback);
                        } else {
                            //Config.debug(oSelf.sClass+' loadExtras SKIP');
                            callback();
                        }
                    }
                ],function(err){
 ...
 * @param fnCallback
 */
p.loadExtras = function(hExtras,fnCallback,oClient,bDebug) {
    var oSelf = this;
    ////Config.debug('Redis: loadExtras on '+oSelf.sClass+' ('+oSelf.getKey()+')');
    if (hExtras && Object.keys(hExtras) && Object.keys(hExtras).length) {
                ...
                if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty])
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/))
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            case 'Increment':
                                oSelf[sProperty] = null;
                                if (oSelf.getKey())
                                    Config.Redis.get(oSelf.getClass()+':'+oSelf.getKey()+':'+sProperty,function(err,res){
                                        oSelf[sProperty] = parseFloat(res);
                                        callback(err);
                                    },hSettings.hExtras[sProperty].sDbAlias||oSelf.getSettings().sDbAlias);
                                else
                                    callback();
                                break;
                            case 'Object':
                                if (oSelf[sProperty] && (oSelf[sProperty] instanceof Base) && oSelf[sProperty].getKey()) {
                                    ...
                                    if (hOpts.hExtras) {
                                        ...
                                        oSelf[sProperty].loadExtras(hOpts.hExtras,callback,oClient,bDebug);
                                    } else {
                                    ...
                                    async.series([
                                        ...
                                        ,function(cback) {
                                            ...
                                            if (hQuery && !oSelf[sProperty].getKey()) {
                                                //Config.debug(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' Redis.loadObject',hQuery);
                                                Config.Redis.loadObject({hQuery:hQuery,sSource:sExtraSource},oSelf[sProperty],cback);
                                            } else {
                                                //Config.debug(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' Redis.loadObject SKIP');
                                                cback();
                                            }
                                        }
                                        ...
                                        ,function(cback){
                                            if (hQuery && !oSelf[sProperty].getKey()) {
                                                ...
                                                Config.MySql.loadObject({hQuery:hQuery,sSource:sExtraSource},oSelf[sProperty],function(err,oResult,oDbClient){
                                        ...
                                        ,function(cback) {
                                            if (oSelf[sProperty].getKey() && hOpts.hExtras) {
                                                ...
                                                oSelf[sProperty].loadExtras(hOpts.hExtras,cback,oClient,bDebug);
                                            } else {
                                                //Config.debug(oSelf.sClass+' ('+oSelf.getKey()+').'+sProperty+' loadExtras SKIP');
                                                //if (!oSelf[sProperty].getKey()) //Config.debug('EXTRA NOT FOUND: '+sProperty+' ON '+oSelf.sClass+' ('+oSelf.getKey()+')');
                                                cback();
                            ...
                            case 'Collection':
                                ...
                                async.series([
                                    function(cback) {
                                        if (oSelf.getKey()) {
                                            Config.Redis.loadCollection({
                                                sKey:oSelf.getClass()+':'+oSelf.getKey()+':'+sProperty
                                    ...
                                    ,function(cback){
                                        if (oSelf.getKey() && (!oSelf[sProperty] || !oSelf[sProperty].nTotal) && hQuery) {
                                            Config.MySql.loadCollection({
                                                ...
                                                ,nMax:nMax
                                                ,nMin:nMin
                                            }, oSelf[sProperty], function (err, oResult, oDbClient) {
                                                if (bPreserve && oDbClient) oClient = oDbClient;
                                                cback(err);
                                            }, oClient, bPreserve);
                                    ...
                                    ,function(cback){
                                        if (!hOpts || !hOpts.hExtras || !oSelf[sProperty].aObjects || !oSelf[sProperty].aObjects.length)
                                            ...
                                            oSelf[sProperty].forEach(function(oItem,cb){
                                                ...
                                                oSelf[sProperty].aObjects[n].loadExtras(hOpts.hExtras,cb,oClient,bDebug);
                                            },cback);
                            ...
                            case 'Hash':case 'Array':
                                ...
                                if (hSettings.hExtras[sProperty].fnQuery) {
                                    if (hSettings.hExtras[sProperty].bCallback) {
                                        console.log(sProperty+'('+hSettings.hExtras[sProperty].sType+') 1');
                                        hSettings.hExtras[sProperty].fnQuery(oSelf,Config,function(){
                                            console.log(sProperty+'('+hSettings.hExtras[sProperty].sType+') 1 done');
                                            callback();
                                        });
                                    } else {
                                        console.log(sProperty+'('+hSettings.hExtras[sProperty].sType+') 2');
                                        hSettings.hExtras[sProperty].fnQuery(oSelf,Config);
                                        console.log(sProperty+'('+hSettings.hExtras[sProperty].sType+') 2 done');
                                        callback();
                                    }
                                } else
                                    callback();
        ...
        async.forEach(Object.keys(hExtras),function(sProperty,cb){

            //if (bDebug) console.log(oSelf.sClass+'.'+sProperty+' START!');
            if (sProperty && !sProperty.match(/^(nFirstID|sFirstID|nMax|nMin|nSize|sView)$/))
                ...
                loadExtra({
                    ...
                    //if (bDebug) console.log(oSelf.sClass+'.'+sProperty+' DONE!');
                    cb();
                });
            ...
            //if (bDebug) console.log(oSelf.sClass+' DONE!');
...
p.deleteHashKey = function(sProperty,sKey) {
    if (this.getHash(sProperty)[sKey]) delete this.getHash(sProperty)[sKey];
    this.set(sProperty,JSON.stringify(this.getHash(sProperty)));
};
/**
    ...
    if (sProperty && Value) {
        ...
        if (!hSettings)
            ...
            async.series([
                ...
                if (fnCallback) setImmediate(function() { fnCallback(err,oSelf) })
            });
        }
    } else if (fnCallback)
        setImmediate(function() { fnCallback(err,oSelf) })
};
/**
...
p.save = function(hOpts,fnCallback) {
    ...
    async.series([
        ...
        oSelf.processExtras(oSelf.bNew,oSelf.hDelta,oSelf.bRemoved,oSelf.fnExtrasCallback);
        oSelf.publish();
        if (fnCallback)
            fnCallback(err,oSelf);
    });
...
p.delete = function(fnCallback) {
    var oSelf = this;
    if (oSelf.getKey()) {
        Config.info('Deleting '+oSelf.sClass+': '+oSelf.getKey());
        async.parallel([
            function(callback) {
        ...
        Config.info('Skipping Delete of '+oSelf.sClass);
        fnCallback(null,oSelf);
    }
};
/**
...
p.publish = function() {
    var oSelf = this;
    var bNew = oSelf.bNew;
    var hDelta = oSelf.hDelta;
    var bRemoved = oSelf.bRemoved || oSelf.get('bRemoved');
    ...
    if (Config.hClasses[oSelf.sClass].sAdapterPath && (bNew || hDelta || bRemoved))
        oSelf.handle(bNew,hDelta,bRemoved);
};
...
p.processExtras = function(bNew,hDelta,bRemoved,fnCallback) {
    ...
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEach(Config.hExtrasByClass[oSelf.sClass],function(hOpts,callback) {
            ...
            if (fnCallback) setImmediate(function() { fnCallback(err) })
        });
    else if (fnCallback)
        fnCallback();
...
p.handle = function(bNew,hDelta,bRemoved,sCustomPath,fnCallback){
    var oSelf = this;
    ...
    if (Handler) {
        if (Handler.handle)
            Handler.handle(bNew,hDelta,bRemoved,oSelf,fnCallback);
        else if (Handler.exports.handle)
            Handler.exports.handle(bNew,hDelta,bRemoved,oSelf,fnCallback);
    }
};
