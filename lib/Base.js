...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                if (!hOpts.sSource)
                    hOpts.sSource = (oSelf.getSettings().sDb) ? oSelf.getSettings().sDb : 'Redis';
            ...
            if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
                ...
                if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty])
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/)) {
                        Config.debug('Skipping extra lookup of '+sProperty+' ('+(sClass||'')+' '+hSettings.hExtras[sProperty].sType+') on '+oParent.sClass+' ('+(oParent.getKey())+')');
                        callback();
                    } else {
