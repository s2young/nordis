...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        if (hOpts.sClass) {
            ...
            oSelf.nClass = AppConfig.hClasses[oSelf.sClass].nClass;
        ...
        var lookup = function(cb){
            oSelf.sSource = (hOpts.sSource) ? hOpts.sSource : (hOpts && hOpts.sSource=='Redis' && hOpts.sSource!='MySql') ? 'Redis' : ((!hOpts || !hOpts.hQuery.sWhere) && !AppConfig.Redis.hOpts.bSkip) ? 'Redis' : 'MySql';
            async.waterfall([
                function(callback) {
                ...
                ,function(oObj,callback) {
                    if (!AppConfig.MySql.hOpts.bSkip && (oSelf.sSource == 'MySql' || !oSelf.getKey()))
                        AppConfig.MySql.loadObject(hOpts,oSelf,callback);
                    else
                        callback(null,oSelf);
                }
                ,function(oObj,callback) {
                    if (!oObj.getKey() && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty] && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty].sValue) {
                        if (fnCallback) {
                            oSelf.set(oSelf.getSettings().sStrKeyProperty,AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty].sValue);
                            callback(null,oSelf);
                        } else
        ...
        if (hOpts.hQuery) {
        ...
        } else if (AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty] && AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty].sValue) {
            ...
            hOpts.hQuery = {};
            hOpts.hQuery[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty] = AppConfig.hClasses[oSelf.sClass].hProperties[AppConfig.hClasses[oSelf.sClass].sStrKeyProperty].sValue;
            lookup(fnCallback);
        } else {
        ...
        if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty]) {
                ...
                if (!sClass && hSettings.hExtras[sProperty].fnGetClass)
                    sClass = hSettings.hExtras[sProperty].fnGetClass(oParent,AppConfig);
                ...
                if (!sClass) {
                    ...
                    AppConfig.debug('Load '+sProperty+' ('+(sClass||'')+' '+hSettings.hExtras[sProperty].sType+') on '+oParent.sClass+' ('+(oParent.getKey()||oParent.getStrKey())+')');
                    ...
                    var sKey = (oParent.getKey()) ? oParent.nClass+':'+oParent.getKey()+':'+sProperty : null;

                    // We'll load a redis & mysql connection, but only when needed and only once.
                    var loadClient = function(sSource,callback){
                        if (!hClients[sSource])
                            AppConfig[sSource].acquire(function(err,oClient){
                            ...
                            switch (hSettings.hExtras[sProperty].sType) {
                                ...
                                case 'Object':
                                    ...
                                    if (hSettings.hExtras[sProperty].fnQuery)
                                        hQuery = hSettings.hExtras[sProperty].fnQuery(oParent,AppConfig);
                                    else {
                                    ...
                                    async.waterfall([
                                        ...
                                        ,function(oObj,cback) {
                                            if (oObj)
                                                cback(null,oObj);
                                            else if (sSource == 'Redis' && hQuery && !AppConfig.Redis.hOpts.bSkip)
                                                loadClient('Redis',function(err){
                                                    if (err)
                                                        cback(err);
                                                    else
                                                        AppConfig.Redis.loadObject({hQuery:hQuery},oParent[sProperty],cback,hClients.Redis);
                                                });
                                            else
                                                cback(null,null);
                                        }
                                        ,function(res,cback){
                                            if (!res && hQuery && !AppConfig.MySql.hOpts.bSkip)
                                                loadClient('MySql',function(err){
                                                    if (err)
                                                        cback(err);
                                                    else
                                                        AppConfig.MySql.loadObject({hQuery:hQuery},oParent[sProperty],cback,hClients.MySql);
                                                });
                                            else
                                                cback(null,res);
                                        }
                                ...
                                case 'Collection':
                                    ...
                                    if (!oParent[sProperty].hQuery)
                                        ...
                                        if (sSource != 'MySql' && !AppConfig.MySql.hOpts.bSkip && oParent[sProperty].hQuery && oParent[sProperty].hQuery.sWhere)
                                            sSource = 'MySql';
                                    ...
                                    async.waterfall([
                                        ...
                                        ,function(callback){
                                            if (sSource == 'MySql' || AppConfig.Redis.hOpts.bSkip)
                                                callback(null,0);
                                            else {
                                                if (hOpts && hOpts.nMin)
                                        ...
                                        ,function(nTotal,callback){
                                            ...
                                            if (nTotal || AppConfig.MySql.hOpts.bSkip) {
    ...
    async.forEachLimit(aProps,3,loadExtra,function(err){
        if (hClients)
            for (var sSource in hClients) {
                if (AppConfig[sSource] && hClients[sSource])
                    AppConfig[sSource].release(hClients[sSource]);
            }
...
p.set = function(sProperty,value,bIgnoreDelta){
    ...
    if (this.getSettings().hProperties[sProperty]) {
        ...
        if (!bIgnoreDelta && this.getKey() && oldVal != this.hData[sProperty] && sProperty != this.getSettings().sNumKeyProperty && sProperty != this.getSettings().sStrKeyProperty) {
 ...
 * the sNumKeyProperty property name as defined in config, or sStrKeyProperty if no num key is provided.
 */
p.getKey = function(){
    if (this.getSettings().sNumKeyProperty)
        return this.get(this.getSettings().sNumKeyProperty);
    else if (this.getSettings().sStrKeyProperty)
    ...
    if (sProperty && Value) {
        ...
        if (!hSettings)
            ...
            async.series([
                ...
                ,function(callback) {
                    var sKey = oSelf.nClass + ':' + oSelf.getKey() + ':' + sProperty;
                ...
                if (oSelf[sProperty]) {
                    ...
                    switch (hSettings.sType) {
                        case 'Collection':
                            ...
                            break;
                        case 'Increment':
                            AppConfig.Redis.del({oObj:oSelf,sProperty:sProperty, nTTL:nTTL}, Value, function (err) {
                                callback(err,oSelf[sProperty]);
                            });
    ...
    oSelf.setID(function() {
        if (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce)) {
            async.parallel([
                function(callback) {
                    if (!AppConfig.Redis.hOpts.bSkip) {
                        AppConfig.Redis.saveObject(hOpts,oSelf,callback);
                    }else
                        callback();
                }
                ,function(callback) {
                    if (!AppConfig.MySql.hOpts.bSkip) {
                        AppConfig.MySql.saveObject(hOpts,oSelf,callback);
                    } else
                        callback();
                }
            ],function(err){
...
p.toSampleHash = function() {
    var oSelf = this;
    for (var sProp in oSelf.hClasses[oSelf.sClass].hProperties) {
        if (oSelf.hClasses[oSelf.sClass].hProperties[sProp].sSample != undefined)
            oSelf.set(sProp,oSelf.hClasses[oSelf.sClass].hProperties[sProp].sSample,true);
    }
    return oSelf.toHash();
};
