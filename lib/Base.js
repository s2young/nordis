...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                oSelf.sSource = (hOpts && hOpts.sSource=='Redis' && hOpts.sSource!='MySql') ? 'Redis' : ((!hOpts || !hOpts.hQuery.sWhere) && !App.Redis.hSettings.bSkip) ? 'Redis' : 'MySql';
                async.waterfall([
                    function(callback) {
    ...
    var loadExtra = function(hOpts,callback){
        ...
        if (hOpts && hOpts.hPropertyOpts && hOpts.sProperty && hOpts.sProperty != 'sSource') {
            ...
            if (!hSettings.hExtras[sProperty]) {
                ...
                // Determine where we should look. A user can specify in the hExtras an sSource.
                var sSource = (hOpts.sSource) ? hOpts.sSource : oSelf.sSource;
                                        ...
                                        if (sSource == 'Redis' && hSettings.hExtras[sProperty].aKey && oParent.get(hSettings.hExtras[sProperty].aKey[0]))
                                            loadClient('Redis',function(err){
                                                if (err)
                                                    ...
                                                    hClients.Redis.hgetall(oParent[sProperty].nClass+':'+oParent.get(hSettings.hExtras[sProperty].aKey[0]),function(err,res){
                                                        oParent[sProperty].hData = res;
                                                        cback(err,res);
                                                    });
                                            });
                                        else
                                        ...
                                        if (!res && hSettings.hExtras[sProperty].aKey && oParent.get(hSettings.hExtras[sProperty].aKey[0]))
                                            loadClient('MySql',function(err){
                                                if (err)
                                                    ...
                                                    App.MySql.load({hQuery:{nID:oParent.get(hSettings.hExtras[sProperty].aKey[0])}},oParent[sProperty],cback,hClients.MySql);
                                            });
                                        else
                                            cback(null,res);
                                    }
                                        ...
                                        if (nTotal) {
                                            ...
                                            loadClient('MySql',function(err){
                                                if (err)
                                                    ...
                                                    hOpts.hQuery = hSettings.hExtras[sProperty].fnQuery(oParent,App);
                                                    App.MySql.loadCollection(hOpts,oParent[sProperty],function(err){
                                                        if (err || !oParent[sProperty].nTotal || !hOpts || !hOpts.hExtras)
                                                            ...
                                                            async.forEachLimit(oParent[sProperty].aObjects,100,loadItem,callback);
                                                        }
                                                    },hClients.MySql);
                                                }
                                            })
