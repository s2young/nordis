                ...
                if (!hSettings || !sProperty || !hSettings.hExtras || !hSettings.hExtras[sProperty])
                    ...
                    if (!sClass && hSettings.hExtras[sProperty].sType.match(/(Object|Collection)/))
                        ...
                        switch (hSettings.hExtras[sProperty].sType) {
                            case 'Object':
                                console.log(sProperty,hSettings.hExtras[sProperty]);
                                    ...
                                    async.series([
                                        function(cback) {
                                            if (hSettings.hExtras[sProperty].bCallback) {
                                                console.log(sProperty+' has a callback fnQuery!');
                                                hSettings.hExtras[sProperty].fnQuery(oSelf,Config,sProperty,function(err,hRes){
                                                    hQuery = hRes;
                                                    cback(err);
                                                });
                                            } else {
                            ...
                            case 'Collection':
                                ...
                                var hQuery;
                                async.series([
                                    // Pull the query required to load the Extra from the configuration file.
                                    function(cback) {

                                        var getQuery = function(err,hConf) {
                                            if (hConf) {
                                                if (hConf.hQuery) {
                                                    hQuery = hConf.hQuery;
                                                    delete hConf.hQuery;
                                        ...
                                        if (hSettings.hExtras[sProperty].bCallback)
                                            hSettings.hExtras[sProperty].fnQuery(oSelf,Config,sProperty,getQuery);
                                        else
                                           getQuery(null,hSettings.hExtras[sProperty].fnQuery(oSelf,Config,sProperty));
