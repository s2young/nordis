...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        ...
        if (hOpts.hQuery) {
            if (fnCallback) {
                if (!hOpts.sSource)
                    hOpts.sSource = (oSelf.getSettings().sDb) ? oSelf.getSettings().sDb : (!Config.get('Redis').hOpts.bSkip) ? 'Redis' : 'MySql';

                async.waterfall([
                    function(callback) {
                        Config.get('Redis').loadObject(hOpts,oSelf,callback);
                    }
                    ,function(oObj,callback) {
                        Config.get('MySql').loadObject(hOpts,oSelf,function(err,oClient){
            ...
            if (!hSettings || !hSettings.hExtras || !hSettings.hExtras[sProperty])
                ...
                if (!sClass) {
                    ...
                    switch (hSettings.hExtras[sProperty].sType) {
                        case 'Increment':
                            if (oParent.getKey())
                                Config.get('Redis').get(oParent.getClass()+':'+oParent.getKey()+':'+sProperty,function(err,res){
                            ...
                            async.series([
                                ...
                                ,function(cback) {
                                    ...
                                    if (hQuery && !oParent[sProperty].getKey())
                                        Config.get('Redis').loadObject({hQuery:hQuery,sSource:sSource},oParent[sProperty],cback);
                                    else
                                        cback();
                                }
                                ...
                                ,function(cback){
                                    if (hQuery && !oParent[sProperty].getKey())
                                        Config.get('MySql').loadObject({hQuery:hQuery},oParent[sProperty],function(err,oResult){
                        ...
                        case 'Collection':
                            ...
                            async.series([
                                function(cback) {
                                    if (oParent.getKey()) {
                                        Config.get('Redis').loadCollection({
            ...
            async.series([
                function(callback){
                    if (Value instanceof Base && (!Value.getKey() || Value.hDelta)) {
                        Config.debug('Calling save on '+Value.sClass+' ('+oSelf.sClass+'.'+sProperty+')');
                ...
                ,function(callback) {
                    ...
                    switch (hSettings.sType) {
                        case 'Collection':
                            ...
                            async.waterfall([
                                function(callback) {
                                    ...
                                    if (Value instanceof Base) {
                                        async.parallel([
                                            function(cb){
                                                if (!Config.get('Redis').hOpts.bSkip)
                                                    Config.get('Redis').addToSet({
                                                    sKey:sKey,
                                                    nTTL:nTTL,
                                            ...
                                            ,function(cb){
                                                if (!Config.get('MySql').hOpts.bSkip)
                                                    Config.get('MySql').addToSet({
                                                        sKey:sKey,
                                                        sOrderBy:hSettings.sOrderBy
                                                    }, Value, cb);
                                                else
                            ...
                            break;
                        case 'Increment':
                            Config.get('Redis').increment({oObj:oSelf,sProperty:sProperty, nTTL:nTTL}, Value, function (err, res) {
                                if (res)
                                    oSelf[sProperty] = res;
...
p.deleteExtra = function(sProperty,Value,fnCallback) {
    ...
    if (sProperty && Value && Value instanceof Base) {
        async.series([
            function(callback){
                if (!Value.bRemoved) {
                    Config.debug('deleteExtra: DELETE '+Value.sClass+' ('+Value.getKey()+')');
                    ...
                    switch (hSettings.sType) {
                        case 'Collection':
                            ...
                            async.parallel([
                                    ...
                                    if (oSelf[sProperty].nTotal) {
                                        ...
                                        while (oSelf[sProperty].next()) {
                                            if (oSelf[sProperty].getItem().getKey() == Value.getKey()) {
                                                Config.debug('deleteExtra: SPLICE ITEM '+n);
                                ...
                                ,function(cb){
                                    Config.get('Redis').removeFromSet({
                                        sKey:sKey,
                                        sOrderBy:hSettings.sOrderBy
                                    }, Value, cb);
                                }
                            ...
                            break;
                        case 'Increment':
                            Config.get('Redis').del(oSelf.getClass()+':'+oSelf.getKey()+':'+sProperty, function (err) {
                                callback(err,oSelf[sProperty]);
                            });
    ...
    async.series([
        ...
        ,function(callback) {
            if (oSelf.hDelta || oSelf.bNew || (hOpts && hOpts.bForce)) {
                async.parallel([
                    function(cb) {
                        if (!Config.get('Redis').hOpts.bSkip && oSelf.getSettings().sDb != 'MySql')
                            Config.get('Redis').saveObject(hOpts,oSelf,cb);
                        else
                            cb();
                    }
                    ,function(cb) {
                        if (!Config.get('MySql').hOpts.bSkip && oSelf.getSettings().sDb != 'Redis') {
                            Config.get('MySql').saveObject(hOpts,oSelf,cb);
                        } else
                            cb();
                    }
                ],callback);
...
p.setID = function(fnCallback) {
    ...
    if (!oSelf.getKey()) {
        ...
        if (!oSelf.getSettings().bAutoIncrementKey)
            Config.get('Redis').getNextID(oSelf,fnCallback);
        else
...
p.delete = function(fnCallback) {
    ...
    if (oSelf.getKey()) {
        async.parallel([
            function(callback) {
                Config.get('Redis').deleteObject(oSelf,callback);
            }
            ,function(callback) {
                Config.get('MySql').deleteObject(oSelf,callback);
            }
        ],function(err){
            oSelf.bRemoved = true;
            oSelf.publish();
...
p.handle = function(sCustomPath){
    ...
    var sAdapterPath = (Config.NORDIS_ENV_ROOT_DIR) ? Config.NORDIS_ENV_ROOT_DIR+Config.getClasses(oSelf.sClass).sAdapterPath   : Config.getClasses(oSelf.sClass).sAdapterPath;
    if (sCustomPath)
...
p.toTimestamp = function(value) {
    if (validator.isInt(value))
        ...
        if (date)
            ...
            Config.warn('Cannot do anything with '+value+'. Tried converting to timestamp.');
    }
};
