            ...
            if (fnCallback) {
                ...
                async.waterfall([
                    ...
                    ,function(oObj,callback) {
                        Config.MySql.loadObject(hOpts,oSelf,function(err,oClient){
                    ...
                    switch (hSettings.hExtras[sProperty].sType) {
                        ...
                        case 'Object':
                            ...
                            async.series([
                                ...
                                ,function(cback){
                                    if (hQuery && !oParent[sProperty].getKey())
                                        Config.MySql.loadObject({hQuery:hQuery},oParent[sProperty],function(err,oResult){
...
p.publish = function() {
    ...
    if (Config.hExtrasByClass[oSelf.sClass])
        async.forEachLimit(Config.hExtrasByClass[oSelf.sClass],1,function(hOpts,callback) {
            // Simulate the parent using the fnCreate in the extra.
            var hData = Config.hClasses[hOpts.sParent].hExtras[hOpts.sExtra].fnCreate(oSelf);
            if (hData) {
                var oParent = Base.lookup({sClass:hOpts.sParent,hData:hData});
                if (oSelf.bRemoved)
