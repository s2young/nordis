...
function Collection(hOpts,fnCallback) {
    ...
    oSelf.nClass = (hOpts && hOpts.nClass != undefined) ? hOpts.nClass : undefined;

    oSelf.nTotal = 0;
    oSelf._nIndex = -1;

    if (oSelf.sClass && AppConfig.hClasses[oSelf.sClass])
        oSelf.nClass = AppConfig.hClasses[oSelf.sClass].nClass;
    else if (oSelf.nClass && AppConfig.hClassMap[oSelf.nClass])
    ...
    if (hOpts) {
        if (hOpts.aObjects) {
            ...
            AppConfig.MySql.loadCollection(hOpts,oSelf,function(err){
                if (err)
                    fnCallback(err);
