...
function Base(hOpts,fnCallback){
    ...
    if (hOpts) {
        if (hOpts.sClass) {
            ...
            throw('Must supply either nClass or sClass to create an object. '+JSON.stringify(hOpts));
        }
...
p.setExtra = function(sProperty,Value,fnCallback) {
    ...
    if (!sProperty || Value==undefined || Value==null) {
        console.log('Both sProperty ('+sProperty+') and Value ('+Value+') are required on setExtra. ');
        if (fnCallback)
            fnCallback(null,oSelf);
    } else {
        //console.log('setExtra('+sProperty+','+JSON.stringify(Value));
        // Make sure we have a primary key id.
                            ...
                            async.waterfall([
                                function(callback) {
                                    if (Value instanceof Base) {
                                        ...
                                        var oVal = Base.lookup({sClass:hSettings.sClass});
...
/**
 * This method is used to create a collection comprised of objects from different sorted sets.
 * This is for the news.json endpoint.
 * @param hOpts
 * @param fnCallback
 */
p.mergeSets = function(hOpts,fnCallback) {
    var oSelf = this;
    oSelf.sSource = (oSelf.sSource) ? oSelf.sSource : App.sDefaultDb;
    App[oSelf.sSource].zmerge(hOpts,hOpts.cColl,fnCallback);
...
p.getTitle = function(){
    var oSelf = this;
    if (oSelf.hData.sAlias)
        return oSelf.hData.sAlias;
    else if (oSelf.hData.sTitle)
        return oSelf.hData.sTitle;
    else if (oSelf.hData.sName)
        return oSelf.hData.sName;
    else if (oSelf.hData.sName)
        return oSelf.hData.sName;
    else
        return '';
...
p.getShortTitle = function() {
    var oSelf = this;
    if (oSelf.hData.sTitle)
        return oSelf.hData.sTitle;
    else if (oSelf.hData.sName)
        return oSelf.hData.sName;
    else if (oSelf.hData.sName)
...
p.getHtml = function(sProp,nTruncate){
    var sHtml = '';
    if (this.get(sProp)) {
        var re = new RegExp(/\b(http[s]?:\/\/[^\s]*)/g);
        var aMatches = re.exec(sHtml);
        sHtml = this.get(sProp).replace(new RegExp(/\n/g),'<br/>');
        var sLastMatch = '';
        if (aMatches) {
            for (var i = 0; i < aMatches.length; i++) {
                if (sLastMatch != aMatches[i]) {
                    sHtml = sHtml.replace(aMatches[i],'<a href="'+aMatches[i]+'" target="_blank">'+aMatches[i]+'<\/a>');
                    sLastMatch = aMatches[i];
                }
            }
        }
    }

    if (nTruncate && sHtml.length > nTruncate)
        return sHtml.substring(0,nTruncate)+'...';
    else
        return sHtml;
};
