...
module.exports.lookup = function(hOpts,fnCallback) {
    ...
    if (!hOpts.hMetrics) {
        ...
        if (hOpts.sClass && (!Config.hClasses[hOpts.sClass] || !Config.hClasses[hOpts.sClass].hMetrics))
            ...
            var sErr = '';
            async.forEachOf(hOpts.hMetrics,function(hItem,sName,cb) {
                ...
                if (!hConf) {
                    ...
                    async.forEachOf(hOpts.hMetrics[sName],function(hItem,sGrain,cb2){
    ...
    async.parallel([
        ...
        function(callback) {
            if (aDbMetrics.length)
                async.eachSeries(aDbMetrics,function(hItem,cb){
                    ...
                    var processMySqlMetric = function(hIncrement,cb2) {
                        ...
                        if (hIncrement.month != nM) nM = hIncrement.month;
                        ...
                        async.parallel([
                            ...
                            if (err)
                                async.setImmediate(function(){cb2(err)});
                            else {
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        if (!hSettings.hGrains)
                            async.setImmediate(function(){cb('Metric does not have granularity configured. Must have an \'hGrains\' hash that can include boolean values for alltime,year,month,day and/or hour.')});
                        else {
                            var aIncrements = [];
                            ...
                            async.eachSeries(aIncrements,processMySqlMetric,function(){
                                async.setImmediate(cb);
                            });
                        }

                    } else
                        processMySqlMetric({year:null,day:null,month:null,hour:null,sGrain:'alltime'},null,function(){
                            async.setImmediate(cb);
                        });
        ...
        ,function(callback){
            if (aRedisMetrics.length)
                async.eachSeries(aRedisMetrics,function(hItem,cb){
                    ...
                    Redis.keys(aKeyParts.join('|')+'*',function(err,aKeys){
                        ...
                        if (aKeys && aKeys.length)
                            async.eachSeries(aKeys,function(sKey,cb2){
                                sKey = sKey.replace(/\{\d+\}/,'');
                                ...
                                async.setImmediate(function(){cb(err)});
                            });
                        else
                            async.setImmediate(function(){cb(err)});
        ...
        ,function(callback) {
            Redis.keys('METRIC*',function(err,aKeys){
                if (err)
                    ...
                    async.eachSeries(aKeys,function(sKey,cb){
                        Redis.del(sKey,function(err){
                            async.setImmediate(function(){cb(err)});
                        });
                    },callback);
