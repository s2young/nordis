 ...
 *  sFilter:'' - (optional) String filter that identifies a client in a multi-client environment.
...
module.exports.track = function(hOpts,fnCallback){
    ...
    var sFilter = (hOpts && hOpts.sFilter) ? hOpts.sFilter : '';
    ...
    if (!sMetric)
        ...
        var aKeys = [];
        if (hConf.hGrains.alltime) aKeys.push('METRIC|'+sFilter+'|'+sMetric);
        if (hConf.hGrains.year) aKeys.push('METRIC|'+sFilter+'|'+sMetric + '|' + dDate.year());
        if (hConf.hGrains.month) aKeys.push('METRIC|'+sFilter+'|'+sMetric + '|' + dDate.year() + '|' + dDate.month());
        if (hConf.hGrains.day) aKeys.push('METRIC|'+sFilter+'|'+sMetric + '|' + dDate.year() + '|' + dDate.month() + '|' + dDate.date());
        if (hConf.hGrains.hour) aKeys.push('METRIC|'+sFilter+'|'+sMetric + '|' + dDate.year() + '|' + dDate.month() + '|' + dDate.date() + '|' + dDate.hour());
...
module.exports.process = function(hOpts,fnCallback) {
    ...
    console.log('PROCESS METRICS: '+hOpts.dStart.toString()+' -> '+hOpts.dEnd.toString());
    var aDbMetrics = [];
    var aRedisMetrics = [];
    ...
    async.parallel([
                ...
                async.forEachLimit(aDbMetrics,1,function(hItem,cb){
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        if (!hSettings.hGrains)
                            ...
                            async.forEachLimit(aIncrements,1,processMySqlMetric,cb);
                        }
        ...
        ,function(callback){
            if (aRedisMetrics.length)
                async.forEachLimit(aRedisMetrics,1,function(hItem,cb){

                    var hSettings = hItem.hSettings;
                    var processRedisMetric = function(hIncrement,cb2) {
                        var aKeyParts = [];
                        async.parallel([
                            // Look up the related stat in the stats table.
                            function(cb3) {
                            ...
                            ,function(cb3) {
                                ...
                                if (hIncrement.year) aKeyParts.push(hIncrement.year);
                                if (hIncrement.month) aKeyParts.push(hIncrement.month);
                                if (hIncrement.day) aKeyParts.push(hIncrement.day);
                                if (hIncrement.hour) aKeyParts.push(hIncrement.hour);

                                Redis.hgetall(aKeyParts.join('|'),function(err,res){
                                    if (err||!res)
                                        cb3(err,[0,'']);
                                    else if (hSettings.bUniques)
                                        cb3(null,[Object.keys(res).length,JSON.stringify(res)]);
                                    else {
                                        var nTotal = 0;
                                        for (var sFilter in res) {
                                            if (res[sFilter]) nTotal += Number(res[sFilter]);
                                        }
                                        cb3(null,[nTotal,JSON.stringify(res)]);
                                    }
                                });
                            ...
                            if (err)
                                ...
                                var oStat = aResults[0];
                                var nCount = aResults[1][0];
                                var sMeta = aResults[1][1];

                                // User can pass hOpts.bOverride = true to force writing of all stats.  But in the case of
                                // Redis stats the keys are flushed from the Redis db after processing so you can't re-run the
                                // metrics again so those metrics that are saved to MySql are left alone unless the count increases.
                                if (nCount != undefined && (!oStat.getKey() || !oStat.get('nCount')) && !hOpts.bOverride) {
                                    // Update the oStat.count with the cColl.nTotal. This is an overwrite because the total stat is being pulled, not just an increment.
                                    var name = (hItem.sClass) ? hItem.sClass+'.'+hItem.sName : hItem.sName;
                                    oStat.setData({
                                        nCount:nCount||0
                                        ...
                                        ,month:hIncrement.month
                                        ,year:hIncrement.year
                                        ,sMeta:(sMeta instanceof Object) ? JSON.stringify(sMeta) : sMeta
                                    });
                                    if (hIncrement.year)
                                        oStat.set('date',moment.utc(hIncrement).valueOf());
                                    ...
                                    oStat.save(function(err){
                                        ...
                                        if (err)
                                            ...
                                            Redis.del(aKeyParts.join('|'),cb2);
                                    });
                                } else {
                                    console.log('Skipping stat '+oStat.getKey());
                                    cb2();
                                }
                            }
                        });
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        if (!hSettings.hGrains)
                            ...
                            async.forEachLimit(aIncrements,1,processRedisMetric,cb);
                        }

                    } else if (hSettings.hGrains.alltime)
                        processRedisMetric({year:null,day:null,month:null,hour:null,sGrain:'alltime'},cb);
                    else
                        cb();
