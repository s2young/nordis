...
module.exports.lookup = function(hOpts,fnCallback) {
    ...
    if (!hOpts.hMetrics) {
        ...
        if (hOpts.sClass && (!Config.hClasses[hOpts.sClass] || !Config.hClasses[hOpts.sClass].hMetrics))
            ...
            var sErr = '';
            async.forEachOf(Object.keys(hOpts.hMetrics),function(sName,ind,cb) {
                ...
                if (!hConf) {
                    ...
                    async.forEachOf(Object.keys(hOpts.hMetrics[sName]),function(sGrain,ind,cb2){
    ...
    async.parallel([
        ...
        function(callback) {
            if (aDbMetrics.length)
                async.forEachOfLimit(aDbMetrics,2,function(hItem,ind,cb){

                    var hSettings = hItem.hSettings;

                    var nM;
                    var processMySqlMetric = function(hIncrement,ind,cb2) {
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        if (!hSettings.hGrains)
                            ...
                            async.forEachOfLimit(aIncrements,10,processMySqlMetric,cb);
                        }

                    } else
                        processMySqlMetric({year:null,day:null,month:null,hour:null,sGrain:'alltime'},null,cb);
        ...
        ,function(callback){
            if (aRedisMetrics.length)
                async.forEachOfLimit(aRedisMetrics,2,function(hItem,ind,cb){
                    ...
                    Redis.keys(aKeyParts.join('|')+'*',function(err,aKeys){
                        ...
                        if (aKeys && aKeys.length)
                            async.forEachOfLimit(aKeys,100,function(sKey,ind,cb2){
                                sKey = sKey.replace(/\{\d+\}/,'');
        ...
        ,function(callback) {
            Redis.keys('METRIC*',function(err,aKeys){
                if (err)
                    ...
                    async.forEachOf(aKeys,function(sKey,ind,cb){
                        Redis.del(sKey,cb);
                    },callback);
                else
                    callback();
