                ...
                async.forEachLimit(aRedisMetrics,2,function(hItem,cb){
                        ...
                        async.parallel([
                            ...
                            ,function(cb3) {
                                ...
                                Redis.hgetall(aKeyParts.join('|'),function(err,res){
                                    if (res) Config.debug(aKeyParts.join('|')+': '+Object.keys(res).length);
                                    if (err||!res)
                                        cb3(err,[0,'']);
                            ...
                            if (err)
                                ...
                                if (nCount && (!oStat.getKey() || !oStat.get('nCount')) && !hOpts.bOverride) {
                                    // Update the oStat.count with the cColl.nTotal. This is an overwrite because the total stat is being pulled, not just an increment.
                                    var name = (hItem.sClass) ? hItem.sClass+'.'+hItem.sName : hItem.sName;
                                    Config.debug(hOpts.sFilter+':'+hItem.sClass);
                                    oStat.setData({
                                        nCount:nCount||0
                                        ,sFilter:hOpts.sFilter
                                        ,sName:name
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        if (!hSettings.hGrains)
                            ...
                            async.forEachLimit(aIncrements,10,processRedisMetric,cb);
                        }
