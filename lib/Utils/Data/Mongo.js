...
function Mongo() {}
var p = Mongo.prototype;
/**
 * Mongo singleton initialization method. Using the passed-in params, which can
 * be stored in your configuration file, we define the connection params for your
 * Mongo server. This method also creates a couple essential framework tables that powers sorted sets and foreign key lookups.
 * @param hOpts
 */
...
p.loadObject = function(hOpts,oObj,fnCallback) {
    ...
    if (bSkip)
        ...
        async.series([
            function(callback) {
                MongoClient.connect(oSelf.hOpts[sDbAlias].sUrl)
                    .then(function(db){
                        oClient = db;
                        callback();
                    },callback);
            ...
            ,function(callback) {
                var tblName = oObj.getSettings().sTable||oObj.sClass+'Tbl';
                var tbl = oClient.collection(tblName);
                tbl.findOne(hOpts.hQuery)
                    .then(function(res){
                        if (res) {
                            oObj.hData = res;
                            oObj.sSource = 'Mongo';
            ...
            if (oClient && oClient.close) oClient.close();
            fnCallback(err,oObj);
        })
...
/**
 * Populates a collection from Mongo.
 * @param hOpts
 * @param cColl
 * @param fnCallback
 */
        ...
        async.waterfall([
            ...
            ,function(aResults,callback) {
                if (aResults && aResults.length > 0) {
                    ...
                    if (cColl.nTotal) cColl.sSource = 'Mongo';
...
p.saveObject = function(hOpts,oObj,fnCallback) {
    var oSelf = this;
    if (oObj.getSettings().sSource && oObj.getSettings().sSource != 'Mongo') {
        fnCallback();
    } else {
        var oClient;
        var sDbAlias = (oSelf.hOpts[oObj.getSettings().sDbAlias]) ? oObj.getSettings().sDbAlias : 'default';
        ...
        async.series([
            ...
            ,function(callback) {
                ...
                if (oObj.hData['_id']) {
                    ...
                    oClient.collection(tblName).updateOne(oObj.hData)
                        .then(function(res){
                            callback(null,oObj);
                        },callback);
                } else if (oObj.bNew) {
                    console.log('insertOne');
                    oObj.hData['_id'] = oObj.getKey().toString();
                    oClient.collection(tblName).insertOne(oObj.hData)
                        .then(function(res){
                            callback(null,oObj);
                        },callback);
                } else {
                    var hQuery = {};
                    hQuery[oObj.getSettings().sKeyProperty] = oObj.getKey();
                    console.log('findOne');
                    oClient.findOne(hQuery)
                        .then(function(res){
                            oObj.hData = res;
                            console.log('updateOne');
                            oClient.collection(tblName).updateOne(oObj.hData)
...
/**
 * This method removes the item from Mongo using its nID value. It also removes any secondary lookup
    ...
    if (bSkip)
        ...
        async.series([
            ...
            ,function(callback) {
                ...
                var tbl = oClient.collection(tblName);
                var hQuery = {};
                hQuery[sKeyProperty] = oObj.getKey();
                console.log('deleteOne');
                tbl.deleteOne(hQuery)
                    .then(function(res){
                        callback();
                    },callback);
            }
...
p.deleteCollection = function(cColl,fnCallback){
    var oSelf = this;
    var sTable = cColl.getSettings().sTable||cColl.sClass+'Tbl';
    var sDbAlias = cColl.getSettings().sDbAlias;
    var bDone;
    ...
    oSelf.acquire(function(err,oClient){
        if (err) {
            ...
            async.series([
                function(callback){
                    async.each(cColl.aObjects,function(oObj,cb){
                        oObj = (oObj instanceof Base) ? oObj : Base.lookup({sClass:cColl.sClass,hData:oObj});
                ...
                ,function(callback) {
                    ...
                    aStatements.push('DELETE FROM '+sTable+' WHERE '+cColl.getSettings().sKeyProperty+' IN ('+aValues.join(',')+')');
                    aStatements.push('DELETE FROM _CrossReferenceTbl WHERE RefID IN (\''+aValues.join('\',\'')+'\')');

                    oClient.query(aStatements.join(';')+';',aValues,function(err){
                        if (err)
                            oSelf._handleDbError(err,cColl,sDbAlias,callback,'deleteCollection',[cColl,function(err){
                                if (!bDone) callback(err,cColl);
                            }]);
                        else if (!bDone)
                            callback(err,cColl);
...
p._handleDbError = function(err,oObj,sDbAlias,fnCallback,sFnRetry,aRetryArgs) {
    ...
    if (oObj.nRetries > 2)
        ...
        //console.log('_handleDbError: '+oObj.sErr);
        switch (oObj.sErr) {
...
p._checkTable = function(sDbAlias,sTable,fnCallback) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err) {
            ...
            oClient.query('SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = \''+sSchema+'\' AND TABLE_NAME = \''+sTable+'\';',function(err,aResults){
...
p._confirmColumns = function(sDbAlias,oObj,fnCallback){
    ...
    var confirmColumn = function(sColumn,cb) {
        ...
        switch (oObj.getSettings().hProperties[sColumn].sType) {
            case 'String':
                sType = 'TEXT NULL';
                if (oObj.getSettings().hProperties[sColumn].sMongoType) {
                    sType = oObj.getSettings().hProperties[sColumn].sMongoType;
                    if (oObj.getSettings().hProperties[sColumn].bIndex)
                        sType += '; ALTER TABLE '+sSchema+'`'+sTable+'` ADD INDEX `'+sTable+'_'+sColumn+'` (`'+sColumn+'` ASC)';
                }
                break;
        ...
        Config.silly('ALTER TABLE '+sSchema+'`'+sTable+'` ADD COLUMN `'+sColumn+'` '+sType+';');

        oSelf.acquire(function(err,oClient){
            if (err)
                cb(err);
            else
                oClient.query('ALTER TABLE '+sSchema+'`'+sTable+'` ADD COLUMN `'+sColumn+'` '+sType+';',function(err){
                    if (!err || (err && err.code && err.code.toString() == 'ER_DUP_FIELDNAME'))
                        cb();
                    else
                        cb(err);
                });
