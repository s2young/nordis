...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    ...
    async.waterfall([
        ...
        cColl.nIndex = (hOpts) ? hOpts.nIndex || 0 : (cColl.nIndex) ? cColl.nIndex : 0;
        cColl.nSize = (hOpts) ? hOpts.nSize || 0 : (cColl.nSize) ? cColl.nSize : 0;
        ...
        var sLimit = '';
        var sOrderBy = (hOpts && hOpts.sOrderBy) ? ' ORDER BY '+hOpts.sOrderBy : (cColl.sOrderBy) ? cColl.sOrderBy : '';
        var sGroupBy = (hOpts && hOpts.sGroupBy) ? ' GROUP BY '+hOpts.sGroupBy : (cColl.sGroupBy) ? cColl.sGroupBy : '';
        ...
        oClient.query(aStatements.join(';'),aValues,function(err,aResults){
            if (aResults && aResults.length > 0) {
                ...
                if (hOpts.bDebug) {
                    cColl.aStatements = aStatements;
                    cColl.aValues = aValues;
                }
                ...
                if (hOpts.bDebug) {
                    console.log(JSON.stringify(hOpts.hQuery)+'\n'+JSON.stringify(cColl));
                }
