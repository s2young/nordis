...
p.acquire = function(fnCallback,sDbAlias,sTrace) {
    ...
    if (!oSelf.dbPool[sDbAlias]) {
        ...
        oSelf.dbPool[sDbAlias].on('error',function(){
            console.log('ENDING POOL `'+sDbAlias+'`');
        ...
        if (hQuery.aStatements) {
            ...
            for (var sLookup in hQuery) {
                if (oObj.getSettings().hProperties[sLookup]) {
                    ...
                    if (sLookup != oObj.getSettings().sKeyProperty && (oObj.getSettings().hProperties[sLookup].bUnique||oObj.getSettings().hProperties[sLookup].bPrimary) && Object.keys(hQuery).length==1) {
            ...
            if (err) {
                oSelf._handleDbError(err,cColl,oClient,function(err){
                    if (err) {
                        ...
                        delete cColl.sErr;
                        oSelf.loadCollection(hOpts,cColl,fnCallback);
                    }
                });
            } else {
            ...
            if (err)
                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        ...
                        console.log(oObj.sClass+'.'+oObj.getSettings().aSecondaryLookupKeys[i]+'='+sSecondaryKey,oObj.getRedisKey(sSecondaryKey));
                        if (sSecondaryKey) {
                            aStatements.push('INSERT INTO _CrossReferenceTbl (sID,RefID) VALUES (?,?) ON DUPLICATE KEY UPDATE RefID=?');
                            aValues.push(oObj.getRedisKey(sSecondaryKey));
        ...
        if (err)
            ...
            for (var sProperty in hExtras) {
                if (hExtras[sProperty].sType == 'Object') {
                    aCrossRefQMarks.push('?');
                    aCrossRefValues.push(oObj.getRedisKey(null,sProperty)+':'+sProperty);
                } else if (hExtras[sProperty].sType == 'Collection') {
                    aSortedSetQMarks.push('?');
                    aSortedSetValues.push(oObj.getRedisKey(null,sProperty)+':'+sProperty);
                }
