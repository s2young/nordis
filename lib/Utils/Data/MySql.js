    ...
    async.waterfall([
        ...
        if (err)
            ...
            if (oObj.get('nID') && oObj.hDelta) {
                ...
                for (var sProp in oObj.hSettings().aProperties) {
...
p.confirmTable = function(oClient,oObj,fnCallback) {
    ...
    async.waterfall([
        ...
        function(bExists,callback){
            App.debug('CREATE TABLE  IF NOT EXISTS `'+oObj.sClass+'Tbl` (`nID` BIGINT NOT NULL, PRIMARY KEY (`nID`))');
...
p.confirmColumns = function(oClient,oObj,fnCallback){
    ...
    if (oBase.hSettings() && oBase.hSettings().aProperties && oBase.hSettings().aProperties.length) {
        ...
        async.forEachLimit(oBase.hSettings().aProperties,1,confirmColumn,fnCallback);
    } else
