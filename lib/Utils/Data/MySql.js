...
p._updateSortedSet = function(Key,Score,Value,oClient,fnCallback) {
    var oSelf = this;
    oClient.query('INSERT INTO _SortedSetTbl (sID,nScore,RefID) VALUES (?,?,?) ON DUPLICATE KEY UPDATE nScore=?;',[Key,Score,Value.toString(),Score],function(err){
        if (err)
            oSelf._handleDbError(err,oObj,oClient,fnCallback,'_updateSortedSet',[Key,Score,Value,oClient,fnCallback]);
        else
            fnCallback();
    });
};
        ...
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                if (!nScore && (oObj.get(sOrderBy)==null || oObj.get(sOrderBy)==undefined))
                    ...
                    oSelf._updateSortedSet(hOpts.sKey,nScore,oObj.getKey(),oClient,function(err){
                        oSelf.dispatchResult(err,oObj,fnCallback,oClient);
                    });
