...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool[sDbAlias] || nRetry) {
        if (oSelf.hOpts[sDbAlias]) console.log('CREATING POOL `'+sDbAlias+'` with max connections of '+oSelf.hOpts[sDbAlias].nMaxConnections,nRetry);
    ...
    oSelf.dbPool[sDbAlias].getConnection(function(err,oClient){
        if (err) console.log('HEY! ',err.toString());
        if (err && err.toString() == 'Pool is closed.' && !nRetry)
            oSelf.acquire(fnCallback,sDbAlias,1);
        else if (err) {
