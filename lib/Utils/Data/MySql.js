 ...
 * @param bEnd
 */
p.release = function(oClient,bEnd) {
    if (oClient) {
        delete this.hTrace[oClient.sID];
//        if (bEnd)
//            oClient.destroy();
//        else
            oClient.release();
 ...
 * @param bEnd
 */
p.dispatchResult = function(err,oClient,fnCallback,bDispose,bEnd) {
    if (err) Config.error(err);
    if (oClient && bDispose)  this.release(oClient,bEnd);
    if (fnCallback) fnCallback(err,oClient);
};
/**
    ...
    async.waterfall([
        ...
        function(callback){
            if (bSkip || (oClient && oClient.sDbAlias == sDbAlias))
                ...
                oSelf.acquire(callback,sDbAlias,'loadObject'+oObj.sClass);
            }
        }
        ...
        oSelf.dispatchResult(err,oClient,fnCallback,false);
    });
};
/**
...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    ...
    if (!hOpts.hQuery) {
        oSelf.dispatchResult(null,oClient,fnCallback,false);
    } else
        async.waterfall([
            function(callback){
            ...
            ,function(oResult,callback){
                ...
                var sWhere = '';
                if (hOpts.hQuery && hOpts.hQuery.sWhere != undefined) {
                ...
                var sTbl = Config.getClasses(cColl.sClass).sTable||cColl.sClass+'Tbl';
                ...
                oClient.query(aStatements.join(';'),aValues,function(err,res,field){
                    callback(err,res);
                });
            }
...
p.saveObject = function(hOpts,oObj,fnCallback,oClient) {
    ...
    async.waterfall([
        function(callback){
            if (!oClient)
                oSelf.acquire(callback,oObj.getSettings().sDbAlias,'saveObject'+oObj.sClass);
            else
                callback(null,oClient);
        }
    ],function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback,true);
        else {
            var aStatements = [];
            var aValues = [];
        ...
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                if (oObj.get(sOrderBy)==null || oObj.get(sOrderBy)==undefined) {
                    oSelf.dispatchResult('Score missing for '+oObj.sClass+'. Looking for \''+sOrderBy+'\' on key '+hOpts.sKey+' and found '+oObj.get(sOrderBy),oClient,fnCallback,true);
                } else {
                    oClient.query('INSERT INTO _SortedSetTbl (sID,nScore,RefID) VALUES (?,?,?) ON DUPLICATE KEY UPDATE nScore=?;',[hOpts.sKey,oObj.get(sOrderBy),oObj.getKey().toString(),oObj.get(sOrderBy)],function(err){
                        oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                    });
                }
            } else
                oSelf.dispatchResult('sKey missing.',oClient,fnCallback,true);
        }
    ...
    oSelf.acquire(function (err, oClient) {
        if (err)
            ...
            async.waterfall([
                function(callback){
                    if (hOpts.nFirstID)
                        oClient.query('SELECT S.nScore FROM _SortedSetTbl S WHERE S.sID IN (\''+hOpts.aKeys.join('\',\'')+'\') AND S.RefID=\''+hOpts.nFirstID.toString()+'\''+sOrderBy,null,function(err,aResults){
                ...
                function(nuttin,callback) {
                    ...
                    var sWhere = 'SELECT T.*,S.nScore,S.RefID AS RefID FROM '+cColl.sClass+'Tbl T RIGHT OUTER JOIN _SortedSetTbl S ON T.nID = S.RefID WHERE S.sID IN (\''+hOpts.aKeys.join('\',\'')+'\') '+sMin+sOrderBy+sLimit+';';
                ...
                if (err)
                    ...
                    if (aResults[1]) {
                        for (var i = 0; i < aResults[1].length; i++) {
                            if (!aResults[1][i] || !aResults[1][i].nID || aResults[1][i].bRemoved) {
                    ...
                    if (aMissingIds.length > 0) {
                        ...
                        if (aResults[1])
                            cColl.setData(aResults[1]);
                        oSelf.dispatchResult(null,oClient,fnCallback,true);
                    }
                }
            });
        }
