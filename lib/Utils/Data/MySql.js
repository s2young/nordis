...
p.confirmTable = function(oClient,oObj,fnCallback) {
    ...
    AppConfig.info('Confirm table: '+sTable);
    async.waterfall([
        function(callback) {
            oSelf._checkTable('_CrossReferenceTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    AppConfig.info('CREATE TABLE IF NOT EXISTS `_CrossReferenceTbl` (`sID` CHAR(140) NOT NULL, nRefID BIGINT NULL,PRIMARY KEY (`sID`))');
        ...
        function(bExists,callback) {
            oSelf._checkTable('_SortedSetTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    AppConfig.info('CREATE TABLE IF NOT EXISTS `_SortedSetTbl` (`sID` CHAR(40) NOT NULL, nScore BIGINT NULL, nRefID BIGINT NULL);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_sID` (`sID` ASC);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_nScore` (`nScore` ASC);ALTER TABLE `_SortedSetTbl` ADD UNIQUE INDEX `_SortedSetTbl_sID_nRefID` (`sID` ASC, `nRefID` ASC);');
        ...
        function(bExists,callback){
            ...
            AppConfig.info('CREATE TABLE  IF NOT EXISTS `'+sTable+'` ('+sKey+')');
            oClient.query('CREATE TABLE  IF NOT EXISTS `'+sTable+'` ('+sKey+')',function(err){
                if (!err || err.code.toString() == 'ER_TABLE_EXISTS_ERROR')
                    callback(null,true);
                else
                    callback(err);
...
p.confirmColumns = function(oClient,oObj,fnCallback){
    ...
    var confirmColumn = function(sColumn,cb) {
        ...
        AppConfig.silly('ALTER TABLE `'+oSelf.hOpts.sSchema+'`.`'+sTable+'` ADD COLUMN `'+sColumn+'` '+sType+';');
