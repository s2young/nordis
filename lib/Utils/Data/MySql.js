...
p.end = function(fnCallback){
    var oSelf = this;
    if (oSelf.dbPool)
        async.forEachOf(Object.keys(oSelf.dbPool),function(sDbAlias,n,cb){
            try {
...
var createPool = function(hOpts) {
    ...
    return mysql.createPool({
        connectionLimit : hOpts.nMaxConnections||1,
        ...
        ,function(callback) {
            if (oClient && !bSkip) {
                ...
                if (!hQuery || !hQuery.aStatements.length)
                    ...
                    oClient.query(hQuery.aStatements.join(';'),hQuery.aValues,function(err, rows, fields){
                        if (err) {
                            delete oObj.sSource;
        ...
        if (hQuery.aStatements) {
            ...
            for (var sLookup in hQuery) {
                if (oObj.getSettings().hProperties[sLookup]) {
                    ...
                    Config.fatal('Attempting to lookup using a property ('+sLookup+') that does not exist on this class ('+oObj.sClass+') -- '+JSON.stringify(hQuery));
                }
            }
...
p.deleteObject = function(oObj,fnCallback){
    ...
    oSelf.acquire(function(err,oClient){
        if (err) {
            ...
            Config.debug(aStatements);
            Config.debug(aValues);

            oClient.query(aStatements.join(';')+';',aValues,function(err){
                if (err)
                    oSelf._handleDbError(err,oObj,sDbAlias,fnCallback,'deleteObject',[oObj,function(err){
                        if (!bDone) fnCallback(err,oObj);
                    }]);
                else
                    if (!bDone) fnCallback(err,oObj);
