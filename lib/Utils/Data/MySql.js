...
p.acquire = function(fnCallback,sDbAlias) {
    ...
    this.dbPool[sDbAlias].getConnection(function(err,oClient){
 ...
 * @param oResult
 */
p.dispatchResult = function(err,oClient,fnCallback,oResult,oReturnClient) {
    if (err) AppConfig.error(err);
    if (oClient) oClient.release();
    if (oResult) delete oResult.sID;
    if (fnCallback) fnCallback(err,oReturnClient||oResult);
};
/**
...
p.loadObject = function(hOpts,oObj,fnCallback,oClient) {
    ...
    var bDebug = (hOpts && hOpts.bDebug) ? hOpts.bDebug : false;
    ...
    async.waterfall([
        ...
        ,function(oResult,callback) {
            ...
            if (oResult && !bSkip) {
                ...
                if (!hQuery || !hQuery.aStatements.length)
                    ...
                    AppConfig.silly(hQuery.aStatements,bDebug);
                    AppConfig.silly(hQuery.aValues,bDebug);
                    ...
                    async.parallel([
                        ...
                        if (err) {
                            ...
                            if (hQuery.sLookupField) {
                                oSelf.loadObjectBySecondary(hQuery,oObj,oClient,callback,bDebug);
                            } else
                                callback(err,oObj);
                        }
                    });
        ...
        oSelf.dispatchResult(err,null,fnCallback,oObj,oClient);
    });
};
/**
 ...
 * @param fnCallback
 */
p.loadObjectBySecondary = function(hQuery,oObj,oClient,fnCallback,bDebug){
    var oSelf = this;
    var sTable = oObj.getSettings().sTable||oObj.sClass+'Tbl';
    var sSql = 'SELECT * FROM '+sTable+' WHERE '+hQuery.sLookupField+'=?;';

    AppConfig.silly(sSql,bDebug);
    AppConfig.silly(hQuery.aLookupValue,bDebug);
        ...
        if (err) {
            ...
            oSelf._handleDbError(err,oObj,oClient,function(err){
                if (err)
                    ...
                    delete oObj.sErr;
                    oSelf.loadObjectBySecondary(hQuery,oObj,oClient,fnCallback,bDebug);
                }
            });
        } else if (aRes && aRes[0]) {
            oObj.hData = aRes[0];
...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    ...
    if (!hOpts.hQuery)
        oSelf.dispatchResult(null,null,fnCallback,cColl,oClient);
    else
        async.waterfall([
            function(callback){
            ...
            ,function(oResult,callback){
                ...
                AppConfig.silly(aStatements,bDebug);
                AppConfig.silly(aValues,bDebug);
            ...
            if (err)
                oSelf._handleDbError(err,cColl,oClient,function(err){
                    if (err)
                        oSelf.dispatchResult(err,null,fnCallback,cColl,oClient);
                    else {
                        delete cColl.sErr;
