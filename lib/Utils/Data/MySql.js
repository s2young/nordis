...
p.acquire = function(fnCallback,sDbAlias) {
    ...
    //
    // var nNow = new Date().getTime() + Math.random();
    // oSelf._Timers[nNow] = setTimeout(function(){
    //     Config.fatal('getConnection failed on '+process.env.sApp+'!');
    // },15000);
    //
    // //fnCallback(oSelf.dbPool[sDbAlias]);
    // oSelf.dbPool[sDbAlias].getConnection(function(err,oClient){
    //     clearTimeout(oSelf._Timers[nNow]);
    //     if (err) {
