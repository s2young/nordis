 ...
 * @param oClient
 */
p.dispatchResult = function(err,oObj,fnCallback,oClient,sStatement) {
    if (err) Config.error(err);
    if (oClient && oClient.release) oClient.release();
    if (fnCallback) fnCallback(err,oObj);

    if (sStatement && Config.logger.level.match(/^(debug|silly)$/)) {
        clearTimeout(this._Timers[sStatement]);
    }
};
/**
...
p.loadObject = function(hOpts,oObj,fnCallback) {
    ...
    async.series([
            ...
            if (oClient && !bSkip) {
                ...
                if (!hQuery || !hQuery.aStatements.length)
                    ...
                    if (Config.logger.level.match(/^(debug|silly)$/)) {
                        ...
                        oSelf._Timers[sStatement] = setTimeout(function(){
                            Config.fatal('Connection never closed! '+sStatement);
                        },10000);
                    }
        ...
        oSelf.dispatchResult(err,oObj,fnCallback,oClient,sStatement);
    });
};
/**
        ...
        async.waterfall([
            ...
            if (err)
                oSelf._handleDbError(err,cColl,sDbAlias,fnCallback,'loadCollection',[hOpts,cColl,function(err){
                    oSelf.dispatchResult(err,cColl,fnCallback,oClient,sStatement);
                }]);
             else
                oSelf.dispatchResult(null,cColl,fnCallback,oClient,sStatement);
        });
};
/**
