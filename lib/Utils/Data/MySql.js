...
p.end = function(fnCallback){
    var oSelf = this;
    if (oSelf.dbPool) {
        async.forEach(Object.keys(oSelf.dbPool),function(sDbAlias,cb){
            oSelf.dbPool[sDbAlias].end(cb);
        },fnCallback);
    } else if (fnCallback) {
        fnCallback();
    }
...
var createPool = function(hOpts) {
    ...
    console.log('hOpts.nMaxConnections: '+hOpts.nMaxConnections);
...
p.acquire = function(fnCallback,sDbAlias,sTrace) {
    ...
    oSelf.dbPool[sDbAlias].on('error',function(){
        try {
            oSelf.dbPool[sDbAlias].removeListener('error', function(){});
        } catch (er) {} finally {
    ...
    oSelf.dbPool[sDbAlias].getConnection(function(err,oClient){
        if (err) {
            ...
            oClient.on('error', function() {
                try {
                    oClient.removeListener('error', function(){});
                } catch (er) {} finally {
                    oClient.release();
                }
            });
