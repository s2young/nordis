...
p.loadObject = function(hOpts,oObj,fnCallback,oClient) {
    var oSelf = this;
    var bSkip = (oSelf.hOpts[oObj.getSettings().sDbAlias||'default'].bSkip||oObj.getKey());
...
p.loadObjectBySecondary = function(hQuery,oObj,oClient,fnCallback){
    ...
    oClient.query('SELECT * FROM '+sTable+' WHERE '+hQuery.sLookupField+'=?;',hQuery.aLookupValue,function(err,aRes){
        if (err)
            ...
            oClient.query('INSERT INTO _CrossReferenceTbl (sID,nRefID) VALUES (?,?) ON DUPLICATE KEY UPDATE nRefID=?',[oObj.getClass()+':'+oObj.get(hQuery.sLookupField),oObj.getKey(),oObj.getKey()],function(err){
                fnCallback(err,oObj);
            });
        } else
...
p.generateQuery = function(hQuery,oObj,sProperty) {
    ...
    if (sProperty && oObj[sProperty] instanceof Collection) {
        if (oObj[sProperty].aValues) {
            hResult.aStatements.push('SELECT * FROM '+sTable+' WHERE '+oObj.getSettings().sKeyProperty+' IN ('+oObj[sProperty].aValues.join(',')+')');
        ...
        if (aParams.length > 1) {
            ...
            hResult.aStatements.push('SELECT * FROM '+sTable+' WHERE '+oObj.getSettings().sKeyProperty+' = (SELECT nRefID FROM _CrossReferenceTbl WHERE sID = ?)');
...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    ...
    var sDbAlias = (AppConfig.hClasses[cColl.sClass].sDbAlias) ? AppConfig.hClasses[cColl.sClass].sDbAlias : 'default';
    ...
    if (!hOpts.hQuery)
        ...
        async.waterfall([
            ...
            AppConfig.debug(aStatements);
            AppConfig.debug(aValues);
    ...
    async.waterfall([
        ...
        if (err)
            ...
            if (oObj.getKey() && oObj.hDelta) {
                ...
                if (aNames.length)
                    aStatements.push('UPDATE '+sTable+' SET '+aNames.join(',')+' WHERE '+oObj.getSettings().sKeyProperty+' = '+oObj.getKey());
