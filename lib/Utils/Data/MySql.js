        ...
        ,function(callback) {
            if (oClient && !bSkip) {
                ...
                if (!hQuery || !hQuery.aStatements.length)
                    ...
                    oClient.query(hQuery.aStatements.join(';'),(hQuery.aValues && hQuery.aValues.length > 0) ? hQuery.aValues:null,function(err,aResults){
                        if (err) {
                            delete oObj.sSource;
                            oSelf._handleDbError(err,oObj,oClient,callback,'loadObject',[hOpts,oObj,callback]);

                        } else if (aResults && aResults[0]) {
                            Config.debug(aResults);
...
p.loadObjectBySecondary = function(hQuery,oObj,oClient,fnCallback){
    ...
    oClient.query(sSql,hQuery.aLookupValue,function(err,aRes){
        if (err)
            ...
            oObj.hData = aRes[0];
            oObj.sSource = 'MySql';
            oSelf._updateCrossRef(hQuery,oObj,oClient,fnCallback);
        } else
            fnCallback(null,oObj);
    });
};

p._updateCrossRef = function(hQuery,oObj,oClient,fnCallback) {
    var oSelf = this;
    ...
    oClient.query('INSERT INTO _CrossReferenceTbl (sID,RefID) VALUES (?,?) ON DUPLICATE KEY UPDATE RefID=?',[oObj.getRedisKey(oObj.get(hQuery.sLookupField)),oObj.getKey().toString(),oObj.getKey().toString()],function(err){
        if (err)
            oSelf._handleDbError(err,oObj,oClient,fnCallback,'_updateCrossRef',[hQuery,oObj,oClient,fnCallback]);
        else
            fnCallback(null,oObj);
    });
};
...
p.loadCollection = function(hOpts,cColl,fnCallback) {
    ...
    if (!hOpts.hQuery) {
        ...
        async.waterfall([
            ...
            if (err)
                oSelf._handleDbError(err,cColl,oClient,fnCallback,'loadCollection',[hOpts,cColl,function(err){
