...
p.loadObjectBySecondary = function(hQuery,oObj,oClient,fnCallback){
    ...
    oClient.query('SELECT * FROM '+oObj.sClass+'Tbl WHERE '+hQuery.sLookupField+'=?;',hQuery.aLookupValue,function(err,aRes){
        if (err)
            ...
            oClient.query('INSERT INTO _CrossReferenceTbl (sID,nRefID) VALUES (?,?) ON DUPLICATE KEY UPDATE nRefID=?',[oObj.nClass+':'+oObj.get(hQuery.sLookupField),oObj.getNumKey(),oObj.getNumKey()],function(err){
                fnCallback(err,oObj);
            });
        } else
...
p.generateQuery = function(hQuery,oObj,sProperty) {
    ...
    if (sProperty && oObj[sProperty] instanceof Collection) {
        if (oObj[sProperty].aValues) {
            hResult.aStatements.push('SELECT * FROM '+oObj.sClass+'Tbl WHERE '+oObj.getSettings().sNumericKey+' IN ('+oObj[sProperty].aValues.join(',')+')');
            delete oObj[sProperty].aValues;
        }
    } else if (hQuery[oObj.getSettings().sNumericKey]) {
        // Primary key lookup. We can get the whole object because we know where to look
        hResult.aStatements.push('SELECT * FROM '+oObj.sClass+'Tbl WHERE '+oObj.getSettings().sNumericKey+' = ?');
        hResult.aValues.push(hQuery[oObj.getSettings().sNumericKey]);
        ...
        for (var sLookup in hQuery) {
            ...
            if (!hResult.sLookupField) hResult.sLookupField = sLookup;
            if (!hResult.aLookupValue) hResult.aLookupValue = [hQuery[sLookup]];
        }
        // Query syntax depends on whether we're doing a single, secondary key lookup or a multi-parameter query.
        if (aParams.length > 1) {
            hResult.aStatements.push('SELECT * FROM '+oObj.sClass+'Tbl WHERE '+aParams.join(' AND '));
            ...
            hResult.aStatements.push('SELECT * FROM '+oObj.sClass+'Tbl WHERE '+oObj.getSettings().sNumericKey+' = (SELECT nRefID FROM _CrossReferenceTbl WHERE sID = ?)');
...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    var oSelf = this;
    if (!App) App = require('./../../AppConfig');
    var sNumericKey = App.hClasses[cColl.sClass].sNumericKey;
    ...
    async.waterfall([
        ...
        hOpts.sOrderBy = hOpts.sOrderBy||sNumericKey;
        ...
        if (hOpts.nFirstID) {
            ...
            if (hOpts.sOrderBy == sNumericKey)
                sStartingScore += hOpts.nFirstID;
            else
                sStartingScore += '(SELECT '+hOpts.sOrderBy+' FROM '+sTbl+' WHERE '+sNumericKey+'='+hOpts.nFirstID+')';
        }
                ...
                if (aResults && aResults.length > 0) {
                    ...
                    if (aResults[1] && aResults[1].length) {
                        if (hOpts.nSize && aResults[1][hOpts.nSize]) {
                            cColl.nNextID = aResults[1][hOpts.nSize][sNumericKey];
...
p.saveObject = function(hOpts,oObj,fnCallback,oClient) {
    ...
    async.waterfall([
        ...
        if (err)
            ...
            if (oObj.getNumKey() && oObj.hDelta) {
                for (var sProp in oObj.hDelta) {
                    switch (oObj.getSettings().hProperties[sProp]) {
                        case 'Number':
                ...
                if (aNames.length)
                    aStatements.push('UPDATE '+oObj.sClass+'Tbl SET '+aNames.join(',')+' WHERE '+oObj.getSettings().sNumericKey+' = '+oObj.getNumKey());

            } else {
                oObj.getSettings().aProperties.forEach(function(sProp,nIndex){
                    switch (oObj.getSettings().hProperties[sProp]) {
                        case 'Number':
...
p.deleteObject = function(oObj,fnCallback){
    ...
    oSelf.acquire(function(err,oClient){
        if (err) {
            ...
            aStatements.push('DELETE FROM '+oObj.sClass+'Tbl WHERE '+oObj.getSettings().sNumericKey+' = ?');
            aValues.push(oObj.getNumKey());
            ...
            for (var sProperty in hExtras) {
                if (hExtras[sProperty].sType == 'Object') {
                    aCrossRefQMarks.push('?');
                    aCrossRefValues.push(oObj.nClass+':'+oObj.getNumKey()+':'+sProperty);
                } else if (hExtras[sProperty].sType == 'Collection') {
                    aSortedSetQMarks.push('?');
                    aSortedSetValues.push(oObj.nClass+':'+oObj.getNumKey()+':'+sProperty);
                }
        ...
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                if (oObj.get(sOrderBy)==null || oObj.get(sOrderBy)==undefined)
                    ...
                    oClient.query('INSERT INTO _SortedSetTbl (sID,nScore,nRefID) VALUES (?,?,?) ON DUPLICATE KEY UPDATE nScore=?;',[hOpts.sKey,oObj.get(sOrderBy),oObj.getNumKey(),oObj.get(sOrderBy)],function(err,res){
                        oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                    });
                }
            } else
    ...
    async.waterfall([
        function(callback) {
            oSelf._checkTable('_CrossReferenceTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    App.debug('CREATE TABLE IF NOT EXISTS `_CrossReferenceTbl` (`sID` CHAR(140) NOT NULL, nRefID BIGINT NULL,PRIMARY KEY (`sID`))');
        ...
        function(bExists,callback) {
            oSelf._checkTable('_SortedSetTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    App.debug('CREATE TABLE IF NOT EXISTS `_SortedSetTbl` (`sID` CHAR(40) NOT NULL, nScore BIGINT NULL, nRefID BIGINT NULL);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_sID` (`sID` ASC);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_nScore` (`nScore` ASC);ALTER TABLE `_SortedSetTbl` ADD UNIQUE INDEX `_SortedSetTbl_sID_nRefID` (`sID` ASC, `nRefID` ASC);');
        ...
        function(bExists,callback){
            App.debug('CREATE TABLE  IF NOT EXISTS `'+oObj.sClass+'Tbl` (`'+oObj.getSettings().sNumericKey+'` BIGINT NOT NULL, PRIMARY KEY (`'+oObj.getSettings().sNumericKey+'`))');
            oClient.query('CREATE TABLE  IF NOT EXISTS `'+oObj.sClass+'Tbl` (`'+oObj.getSettings().sNumericKey+'` BIGINT NOT NULL, PRIMARY KEY (`'+oObj.getSettings().sNumericKey+'`))',function(err){
                if (!err || err.code.toString() == 'ER_TABLE_EXISTS_ERROR')
                    callback(null,true);
                else
                    callback(err);
...
p.confirmColumns = function(oClient,oObj,fnCallback){
    ...
    var confirmColumn = function(sColumn,cb) {
        ...
        App.debug('ALTER TABLE `'+oSelf.hSettings.sSchema+'`.`'+oObj.sClass+'Tbl` ADD COLUMN `'+sColumn+'` '+sType+';');
