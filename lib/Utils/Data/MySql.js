...
p.loadObject = function(hOpts,oObj,fnCallback,oClient) {
    ...
    async.waterfall([
        ...
        if (err)
            ...
            if (!App) App = require('./../../AppConfig');
            App.silly(hQuery.aStatements);
            App.silly(hQuery.aValues);
...
p.loadObjectBySecondary = function(hQuery,oObj,oClient,fnCallback){
    ...
    App.silly('SELECT * FROM '+sTable+' WHERE '+hQuery.sLookupField+'=?;');
    App.silly(hQuery.aLookupValue);
...
p.loadCollection = function(hOpts,cColl,fnCallback,oClient) {
    var oSelf = this;
    if (!App) App = require('./../../AppConfig');
    var sNumKeyProperty = App.hClasses[cColl.sClass].sNumKeyProperty;
    ...
    if (!hOpts.hQuery)
        ...
        async.waterfall([
            ...
            var sTbl = App.hClasses[cColl.sClass].sTable||cColl.sClass+'Tbl';
    ...
    async.waterfall([
        ...
        if (err)
            ...
            if (oObj.getNumKey() && oObj.hDelta) {
                ...
                if (aNames.length)
                    aStatements.push('UPDATE '+sTable+' SET '+aNames.join(',')+' WHERE '+oObj.getSettings().sNumKeyProperty+' = '+oObj.getNumKey());
        ...
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                if (oObj.get(sOrderBy)==null || oObj.get(sOrderBy)==undefined) {
                    ...
                    oClient.query('INSERT INTO _SortedSetTbl (sID,nScore,nRefID) VALUES (?,?,?) ON DUPLICATE KEY UPDATE nScore=?;',[hOpts.sKey,oObj.get(sOrderBy),oObj.getNumKey(),oObj.get(sOrderBy)],function(err){
                        oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                    });
                }
            } else
...
p._checkTable = function(sTable,oClient,fnCallback) {
    oClient.query('SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = \''+this.hOpts.sSchema+'\' AND TABLE_NAME = \''+sTable+'\';',function(err,aResults){
...
p.confirmTable = function(oClient,oObj,fnCallback) {
    ...
    App.warn('Confirm table: '+sTable);
    async.waterfall([
        function(callback) {
            oSelf._checkTable('_CrossReferenceTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    App.warn('CREATE TABLE IF NOT EXISTS `_CrossReferenceTbl` (`sID` CHAR(140) NOT NULL, nRefID BIGINT NULL,PRIMARY KEY (`sID`))');
                    oClient.query('CREATE TABLE IF NOT EXISTS `_CrossReferenceTbl` (`sID` CHAR(140) NOT NULL, nRefID BIGINT NULL,PRIMARY KEY (`sID`))',function(err){
        ...
        function(bExists,callback) {
            oSelf._checkTable('_SortedSetTbl',oClient,function(err,bExists){
                if (!err && !bExists) {
                    App.warn('CREATE TABLE IF NOT EXISTS `_SortedSetTbl` (`sID` CHAR(40) NOT NULL, nScore BIGINT NULL, nRefID BIGINT NULL);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_sID` (`sID` ASC);ALTER TABLE `_SortedSetTbl` ADD INDEX `_SortedSetTbl_nScore` (`nScore` ASC);ALTER TABLE `_SortedSetTbl` ADD UNIQUE INDEX `_SortedSetTbl_sID_nRefID` (`sID` ASC, `nRefID` ASC);');
        ...
        function(bExists,callback){
            App.warn('CREATE TABLE  IF NOT EXISTS `'+sTable+'` (`'+oObj.getSettings().sNumKeyProperty+'` BIGINT NOT NULL, PRIMARY KEY (`'+oObj.getSettings().sNumKeyProperty+'`))');
            oClient.query('CREATE TABLE  IF NOT EXISTS `'+sTable+'` (`'+oObj.getSettings().sNumKeyProperty+'` BIGINT NOT NULL, PRIMARY KEY (`'+oObj.getSettings().sNumKeyProperty+'`))',function(err){
                if (!err || err.code.toString() == 'ER_TABLE_EXISTS_ERROR')
                    callback(null,true);
                else
                    callback(err);
...
p.confirmColumns = function(oClient,oObj,fnCallback){
    ...
    var confirmColumn = function(sColumn,cb) {
        App.warn('Confirm column: '+sColumn);
        var sType;
            ...
            case 'String':
                ...
                if (oObj.getSettings().hProperties[sColumn].sMySqlType) {
                    ...
                    if (oObj.getSettings().hProperties[sColumn].bIndex)
                        sType += '; ALTER TABLE `'+oSelf.hOpts.sSchema+'`.`'+sTable+'` ADD INDEX `'+sTable+'_'+sColumn+'` (`'+sColumn+'` ASC)';
                }
                break;
        ...
        App.silly('ALTER TABLE `'+oSelf.hOpts.sSchema+'`.`'+sTable+'` ADD COLUMN `'+sColumn+'` '+sType+';');
        oClient.query('ALTER TABLE `'+oSelf.hOpts.sSchema+'`.`'+sTable+'` ADD COLUMN `'+sColumn+'` '+sType+';',function(err){
