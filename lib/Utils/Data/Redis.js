                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.hset(oClient.sHashKey+'KEYS',oObj.getRedisKey(oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])),oClient.sHashKey+oObj.getRedisKey());
                            // The object pointer must also be set to expire if the data it's pointing to is going to expire.
                            if (nTTL)
                                multi.expire(oClient.sHashKey+oObj.getRedisKey(oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])),nTTL);
                        }
                    }
                }
            ...
            oClient.hgetall(oClient.sHashKey+oObj.getRedisKey()+':MAP',function(err,hMap) {
                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        ...
                        if (sKey) multi.hdel(oClient.sHashKey+'KEYS',oObj.getRedisKey(sKey));
                    }
                }
                // And its extras.
