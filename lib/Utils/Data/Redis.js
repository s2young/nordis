...
p.dispatchResult = function(err,oClient,fnCallback,oResult) {
    ...
    if (oResult && oClient) Config.trace(oResult.sID,{Redis:{sID:oClient.sID,bReleased:true}});
...
p.loadObject = function(hOpts,oObj,fnCallback) {
    ...
    async.waterfall([
        ...
        if (hResult) {
            Config.trace(oObj.sID,{sSource:'Redis'});
            oObj.hData = hResult;
            oObj.sSource = 'Redis';
        }
        ...
        async.waterfall([
            function(callback){
                if (hOpts && (hOpts.nMin || hOpts.nMax))
                    Config.get('Redis').zcount(hOpts.sKey,hOpts.nMin||0,hOpts.nMax||726081834000,callback);
                else
                    Config.get('Redis').zcard(hOpts.sKey,callback,sDbAlias);
            }
                    ...
                    async.waterfall([
                        function(cb){
                            ...
                            if (!oObj.sFirstID)
                                ...
                                Config.get('Redis').dbPool[sDbAlias].zrevrank(hOpts.sKey,oObj.getClass()+':'+oObj.sFirstID,cb);
                            else
                                Config.get('Redis').dbPool[sDbAlias].zrank(hOpts.sKey,oObj.getClass()+':'+oObj.sFirstID,cb);
                        }
                        ,function(nStart,cb){
                            ...
                            var handleResult = function(err,aResult) {
                                ...
                                async.forEach(aResult,function(sItemKey,cback){
                                    Config.get('Redis').dbPool[sDbAlias].hgetall(sItemKey,function(err,res){
                                        if (err)
                                            cback(err);
                            ...
                            if (oObj.bReverse)
                                Config.get('Redis').dbPool[sDbAlias].zrevrange(hOpts.sKey,oObj.nStart,oObj.nEnd,handleResult);
                            else
                                Config.get('Redis').dbPool[sDbAlias].zrange(hOpts.sKey,oObj.nStart,oObj.nEnd,handleResult);
                        }
                    ],callback);
                }
            }
