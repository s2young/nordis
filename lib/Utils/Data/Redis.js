...
p.acquire = function(fnCallback,sDbAlias,bForce) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias])
        oSelf.createClient('dbPool',sDbAlias,fnCallback,bForce);
    else
...
p.loadObject = function(hOpts,oObj,fnCallback) {
    ...
    var sTenantProperty = oObj.getSettings().sTenantProperty;
                ...
                oSelf.acquire(function(err,oResult){
                    if (err||!oResult)
                        ...
                        if (hOpts.hQuery[sKeyProperty]) {
                            ...
                            oClient.get(oClient.sHashKey+oObj.getRedisKey(hOpts.hQuery[sKeyProperty],hOpts.hQuery[sTenantProperty]),function(err,hResult){
                                if (hResult) {
                            ...
                            if (hOpts.hQuery) {
                                for (var sLookup in hOpts.hQuery) {
                                    ...
                                    if (hOpts.hQuery[sLookup] && oObj.getSettings().hProperties && oObj.getSettings().hProperties[sLookup] && (oObj.getSettings().hProperties[sLookup].bPrimary || oObj.getSettings().hProperties[sLookup].bUnique)) {
                                        sKey = oObj.getRedisKey(hOpts.hQuery[sLookup]);
                                        break;
                                    }
                                }
                            }
                            ...
                            if (sKey) {
                                ...
                                oClient.hmget(oClient.sHashKey+'KEYS',sKey,function(err,res){
                                    if (err || !res || !res.length || !res[0])
                                        ...
                                        oClient.get(res[0],function(err,hResult){
                                            if (hResult) {
                                                oObj.hData = JSON.parse(hResult);
                                                oObj.sSource = 'Redis';
                                            }
                ...
                async.waterfall([
                    ...
                    ,function(res,callback){
                        ...
                        if (!oObj.nTotal)
                            ...
                            async.waterfall([
                                function(cb){
                                    if (hOpts.sFirstID) console.log(oClient.sHashKey+oObj.getRedisKey(oObj.sFirstID));
                                ...
                                ,function(nStart,cb){
                                    ...
                                    var handleResult = function(err,aResult) {
                                        ...
                                        if (oObj.nSize && aResult && aResult[oObj.nSize]) {
                                            ...
                                            console.log(aParts,oObj.nNextID);
                                            aResult.splice(-1,1);
                                        ...
                                        if (aResult) {
                                            ...
                                            oClient.mget(aResult,function(err,aItems){
                                                if (err || !aItems || !aItems.length)
                                                    ...
                                                    var bMissing;
                                                    async.forEachOf(aItems,function(hData,n,cb2){
    ...
    if (!oObj.getKey() || (hOpts && hOpts.sDestination == 'MySql') || oSelf.hOpts[oObj.getSettings().sDbAlias||'default'].bSkip || oObj.getSettings().sSource == 'MySql')
        ...
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                multi.set(sKey,JSON.stringify(oObj.hData));
                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.hset(oClient.sHashKey+'KEYS',oObj.getClass()+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]),oClient.sHashKey+oObj.getRedisKey());
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            oClient.hgetall(oClient.sHashKey+oObj.getRedisKey()+':MAP',function(err,hMap) {
                ...
                multi.del(oClient.sHashKey+oObj.getRedisKey());
                multi.hdel(oClient.sHashKey+'KEYS',oObj.getRedisKey());
                ...
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
                        multi.del(oClient.sHashKey+oObj.getRedisKey()+':'+sProp);
                    }
                }
                for (var sKey in hMap) {
                    multi.zrem(sKey,oClient.sHashKey+oObj.getRedisKey());
                }
                multi.del(oClient.sHashKey+oObj.getRedisKey()+':MAP');
                multi.exec(function(err){
                    oSelf.dispatchResult(err,oObj,fnCallback);
                });
            });
        ...
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                if (hOpts && hOpts.sKey) {
                    ...
                    var multi = oClient.multi();
                    multi.zrem(oClient.sHashKey+hOpts.sKey,[oClient.sHashKey+oObj.getRedisKey()]);
                    multi.zadd(oClient.sHashKey+hOpts.sKey,nScore,oClient.sHashKey+oObj.getRedisKey());

                    // Add reference to object map for clean-up later.
                    multi.hset(oClient.sHashKey+oObj.getRedisKey()+':MAP',oClient.sHashKey+hOpts.sKey,1);
                    if (nTTL)
