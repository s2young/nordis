...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias]) {
        ...
        oSelf.dbPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,{connect_timeout:10000})
            .on('end',function(){
                console.log('connection end. '+new Date().toString());
                delete oSelf.dbPool[sDbAlias];
            })
            .on('error',function(err){
                console.log('connection error. '+new Date().toString(),err);
                delete oSelf.dbPool[sDbAlias];
            });
...
p.saveObject = function(hOpts,oObj,fnCallback) {
    var oSelf = this;
    if (!oObj.getKey() || (hOpts && hOpts.sDestination == 'MySql') || oSelf.hOpts[oObj.getSettings().sDbAlias||'default'].bSkip)
