            ...
            if (err)
                ...
                if (hOpts && hOpts.sKey) {
                    ...
                    if (isNaN(nScore)) {
                        ...
                        multi.zrem(oClient.sHashKey+hOpts.sKey,[oClient.sHashKey+oObj.getRedisKey()],function(){});
                        multi.zadd(oClient.sHashKey+hOpts.sKey,nScore,oClient.sHashKey+oObj.getRedisKey(),function(){});
                        // Add reference to object map for clean-up later.
                        if (oClient.sHashKey+hOpts.sKey) multi.hset(oClient.sHashKey+oObj.getRedisKey()+':MAP',oClient.sHashKey+hOpts.sKey,1,function(){});

                        var nTTL = (hOpts.nTTL) ? hOpts.nTTL : oObj.getSettings().nTTL;
                        if (nTTL) multi.expire(oClient.sHashKey+hOpts.sKey,nTTL,function(){}); // in seconds
