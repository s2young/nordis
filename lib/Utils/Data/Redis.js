...
p.init = function(hOpts) {
    var oSelf = this;
    // TODO: Add support for multiple dbs and servers.
    oSelf.hOpts = hOpts;
};
...
var createSubPool = function(hOpts){
    if (!AppConfig) AppConfig = require('./../../AppConfig');
    return poolModule.Pool({
        name     : 'redis_sub',
        create   : function(callback) {
...
p.acquire = function(fnCallback) {
    if (!this.dbPool)
        this.dbPool = createPool(this.hOpts);
...
p.acquireSub = function(fnCallback) {
    if (!this.dbSubPool)
        this.dbSubPool = createPool(this.hOpts);
    this.dbSubPool.acquire(fnCallback);
};
p.releaseSub = function(oClient,oObj) {
...
p.dispatchResult = function(err,oClient,fnCallback,oResult) {
    ...
    if (oClient) {
        if (oResult)
            AppConfig.trace(oResult.sID,{Redis:{sID:oClient.sID,bReleased:true}});
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            oClient.hgetall(oObj.getClass()+':'+oObj.getKey()+':MAP',function(err,hMap) {
                ...
                var multi = oClient.multi();
                multi.del(oObj.getClass()+':'+oObj.getKey());
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.del(oObj.getClass()+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]));
                        }
                    }
                }
                if (hMap) {
                    for (var sKey in hMap) {
                        multi.zrem(sKey,oObj.getClass()+':'+oObj.getKey());
                    }
                }
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
                        multi.del(oObj.getClass()+':'+oObj.getKey()+':'+sProp);
                    }
                }
                // Delete the object's map too.
                multi.del(oObj.getClass()+':'+oObj.getKey()+':MAP');
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                var multi = oClient.multi();
                multi.zrem(hOpts.sKey,[oObj.getClass()+':'+oObj.getKey()]);
                multi.zadd(hOpts.sKey,oObj.get(sOrderBy),[oObj.getClass()+':'+oObj.getKey()]);
                // Add reference to object map for clean-up later.
                multi.hset(oObj.getClass()+':'+oObj.getKey()+':MAP',hOpts.sKey,1);
                if (nTTL)
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                multi.zrem(hOpts.sKey,[oObj.getClass()+':'+oObj.getKey()]);
                // Add reference to object map for clean-up later.
                multi.hdel(oObj.getClass()+':'+oObj.getKey()+':MAP',hOpts.sKey,1);
                multi.exec(function(err){
                    oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                });
            } else
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (Value instanceof Base) {
                var multi = oClient.multi();
                multi.hset(Value.getClass()+':'+Value.getKey()+':MAP',hOpts.sKey,Value.nClass+':'+Value.getKey());
                if (hOpts.nTTL)
                    multi.expire(Value.getClass()+':'+Value.getKey()+':MAP',hOpts.nTTL);
                multi.exec(function(err){
            ...
            if (!oSelf.bSeeded) {
                ...
                multi.exec(function(err,aResult){
                    if (!err && aResult && aResult[1])
                        oObj.hData[oObj.getSettings().sKeyProperty] = aResult[1];
