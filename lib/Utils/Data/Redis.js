...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias]) {
        ...
        oSelf.dbPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,{connect_timeout:10000})
            .on('end',function(){
        ...
        oSelf.dbPool[sDbAlias].sHashKey = (oSelf.hOpts[sDbAlias].nDb) ?  '{'+oSelf.hOpts[sDbAlias].nDb+'}' : '';
        oSelf[sDbAlias] = {sHashKey:oSelf.dbPool[sDbAlias].sHashKey};
...
p.acquirePub = function(fnCallback,sDbAlias) {
    ...
    if (!oSelf.dbPubPool || !oSelf.dbPubPool[sDbAlias]) {
        if (!oSelf.dbPubPool) oSelf.dbPubPool = {};

        if (!Config) {
            Base = require('./../../Base');
            Config = require('./../../AppConfig');
        }
        oSelf.dbPubPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,{});
        oSelf.dbPubPool[sDbAlias].sDbAlias = sDbAlias;
        oSelf.dbPubPool[sDbAlias].sHashKey = (oSelf.hOpts[sDbAlias].nDb) ?  '{'+oSelf.hOpts[sDbAlias].nDb+'}' : '';
        oSelf[sDbAlias] = {sHashKey:oSelf.dbPubPool[sDbAlias].sHashKey};
...
p.acquireSub = function(fnCallback,sDbAlias) {
    ...
    if (!oSelf.dbSubPool || !oSelf.dbSubPool[sDbAlias]) {
        ...
        oSelf.dbSubPool[sDbAlias].sHashKey = (oSelf.hOpts[sDbAlias].nDb) ?  '{'+oSelf.hOpts[sDbAlias].nDb+'}' : '';
        oSelf[sDbAlias] = {sHashKey:oSelf.dbSubPool[sDbAlias].sHashKey};
                            ...
                            async.waterfall([
                                function(cb){
                                    ...
                                    if (!oObj.sFirstID)
                                        ...
                                        oClient.zrevrank(oClient.sHashKey+hOpts.sKey,oObj.getClass()+':'+oObj.sFirstID,cb);
                                    else
                                        oClient.zrank(oClient.sHashKey+hOpts.sKey,oObj.getClass()+':'+oObj.sFirstID,cb);
                                }
                                ,function(nStart,cb){
                                    oObj.nStart = parseFloat(nStart);
                                    ...
                                    var handleResult = function(err,aResult) {
                                        ...
                                        if (aResult) {
                                            ...
                                            async.forEach(aResult,function(sItemKey,cback){
                                                oClient.hgetall(oClient.sHashKey+sItemKey,function(err,res){
                                                    if (err)
                                                        cback(err);
                                                    else if (!res) {
                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.set(oClient.sHashKey+oObj.getClass()+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]),oClient.sHashKey+oObj.getClass()+':'+oObj.getKey());
                ...
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
                        if (oObj.getSettings().hExtras[sProp].aKey && oObj.get(oObj.getSettings().hExtras[sProp].aKey[0])) {
                            ...
                            if (sClass) {
                                multi.hset(oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':MAP',oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':'+sProp,Config.getClasses(sClass).nClass+':'+oObj.get(oObj.getSettings().hExtras[sProp].aKey[0]));
                                if (nTTL)
                                    multi.expire(oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':'+sProp,nTTL);
                            }
                        }
                    }
                }
        ...
        if (err)
            ...
            oClient.hgetall(oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':MAP',function(err,hMap) {
                ...
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.del(oClient.sHashKey+oObj.getClass()+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]));
                        }
                    }
                }
                if (hMap) {
                    for (var sKey in hMap) {
                        multi.zrem(oClient.sHashKey+sKey,oClient.sHashKey+oObj.getClass()+':'+oObj.getKey());
                    }
                }
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
        ...
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                if (hOpts && hOpts.sKey) {
                    ...
                    var multi = oClient.multi();
                    multi.zrem(oClient.sHashKey+hOpts.sKey,[oObj.getClass()+':'+oObj.getKey()]);
                    multi.zadd(oClient.sHashKey+hOpts.sKey,nScore,[oObj.getClass()+':'+oObj.getKey()]);
                    // Add reference to object map for clean-up later.
                    multi.hset(oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':MAP',hOpts.sKey,1);
                    if (nTTL)
...
p.blpop = function(sKey1,sKey2,nTimeout,fnCallback,sDbAlias) {
    ...
    oClient.blpop(oClient.sHashKey+sKey1,oClient.sHashKey+sKey2,nTimeout,function(err,res){
        oClient.quit();
        fnCallback(err,res);
    });
};
...
p.blpop3 = function(sKey1,sKey2,sKey3,nTimeout,fnCallback,sDbAlias) {
    ...
    sDbAlias = (oSelf.hOpts[sDbAlias]) ? sDbAlias : 'default';
    var oClient = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,{});
    oClient.blpop(oClient.sHashKey+sKey1,oClient.sHashKey+sKey2,oClient.sHashKey+sKey3,nTimeout,function(err,res){
        oClient.quit();
        fnCallback(err,res);
    });
};
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            Key.forEach(function(sKey){
                oClient.hincrby(oClient.sHashKey+sKey,sName,nIncr);
            });
            oSelf.dispatchResult(err,null,fnCallback);
        } else {
...
p.del = function(sKey,fnCallback,sDbAlias) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            oClient.del(oClient.sHashKey+sKey,function(err,res){
                oSelf.dispatchResult(err,res,fnCallback);
            });
        }
    },sDbAlias);
