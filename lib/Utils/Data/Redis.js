...
p.acquire = function(fnCallback) {
    ...
    this.dbPool.acquire(function(err,oClient){
        if (!err && !oClient.selected_db)
            oClient.select(App.hOptions.Redis.nDb,function(err2){
                fnCallback(err||err2,oClient);
            });
        else
            fnCallback(err,oClient);
    });
};
...
p.acquireSub = function(fnCallback) {
    ...
    this.dbSubPool.acquire(function(err,oClient){
        if (!err && !oClient.selected_db)
            oClient.select(App.hOptions.Redis.nDb,function(err2){
                fnCallback(err,oClient);
            });
