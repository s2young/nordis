...
p.loadCollection = function(hOpts,oObj,fnCallback) {
    ...
    if (hOpts && hOpts.sSource == 'MySql')
        ...
        async.waterfall([
            function(callback){
                if (hOpts && hOpts.nMin)
                    AppConfig.Redis.zcount(hOpts.sKey,hOpts.nMin,726081834000,callback);
                else
                ...
                if (!nTotal)
                    ...
                    async.waterfall([
                        ...
                        ,function(nStart,cb){
                            ...
                            oObj.nEnd = (oObj.nSize) ? (oObj.nStart + oObj.nSize) : -1;
...
p.zcount = function(sKey,fnCallback,sDbAlias) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback);
        else {
            oClient.zcount(sKey,function(err,res){
                oSelf.dispatchResult(err,oClient,fnCallback,res);
            });
        }
    },sDbAlias);
