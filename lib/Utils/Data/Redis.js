...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias]) {
        ...
        if (oSelf.hOpts[sDbAlias].sPassword)
            oSelf.opts.auth_pass = oSelf.hOpts[sDbAlias].sPassword;

        console.log('acquire: '+oSelf.hOpts[sDbAlias].nPort+','+oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        oSelf.dbPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        ...
        oSelf.dbPool[sDbAlias].on('connect',function(){
...
p.acquirePub = function(fnCallback,sDbAlias) {
    ...
    if (!oSelf.dbPubPool || !oSelf.dbPubPool[sDbAlias]) {
        ...
        console.log('acquirePub: '+oSelf.hOpts[sDbAlias].nPort+','+oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        oSelf.dbPubPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        ...
        oSelf.dbPubPool[sDbAlias].on('connect',function(){
...
p.acquireSub = function(fnCallback,sDbAlias) {
    ...
    if (!oSelf.dbSubPool || !oSelf.dbSubPool[sDbAlias]) {
        ...
        console.log('acquireSub: '+oSelf.hOpts[sDbAlias].nPort+','+oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        oSelf.dbSubPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,oSelf.opts);
        ...
        oSelf.dbSubPool[sDbAlias].on('connect',function(){
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                // Add reference to object map for clean-up later.
                oClient.hdel(oClient.sHashKey+oObj.getClass()+':'+oObj.getKey()+':MAP',oClient.sHashKey+hOpts.sKey,1);
                oSelf.dispatchResult(err,oObj,fnCallback);
            } else
                oSelf.dispatchResult('sKey missing.',oObj,fnCallback);
        }
