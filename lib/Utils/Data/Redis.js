            ...
            if (oObj.hSettings().hExtras) {
                for (var sProp in oObj.hSettings().hExtras) {
                    if (oObj.hSettings().hExtras[sProp].aKey && oObj.get(oObj.hSettings().hExtras[sProp].aKey[0])) {
                        var nClass = (oObj.hSettings().hExtras[sProp].sClass) ? App.hClasses[oObj.hSettings().hExtras[sProp].sClass].nClass : oObj.hSettings().hExtras[sProp].getClass ? oObj.hSettings().hExtras[sProp].sClass(oObj,App) : null;
                        if (nClass) {
                            multi.hset(oObj.nClass+':'+oObj.get('nID')+':MAP',oObj.nClass+':'+oObj.get('nID')+':'+sProp,nClass+':'+oObj.get(oObj.hSettings().hExtras[sProp].aKey[0]));
                            if (nTTL)
