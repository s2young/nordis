...
p.saveObject = function(hOpts,oObj,fnCallback) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            var sKey = (hOpts && hOpts.sKey) ? hOpts.sKey : oObj.nClass+':'+oObj.getKey();
            ...
            if (oObj.getSettings().aSecondaryLookupKeys) {
                for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                    if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                        multi.set(oObj.nClass+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]),oObj.nClass+':'+oObj.getKey());
                ...
                for (var sProp in oObj.getSettings().hExtras) {
                    if (oObj.getSettings().hExtras[sProp].aKey && oObj.get(oObj.getSettings().hExtras[sProp].aKey[0])) {
                        ...
                        if (!sClass && oObj.getSettings().hExtras[sProp].fnGetClass)
                            sClass = oObj.getSettings().hExtras[sProp].fnGetClass(oObj,AppConfig);
                        if (sClass) {
                            multi.hset(oObj.nClass+':'+oObj.getKey()+':MAP',oObj.nClass+':'+oObj.getKey()+':'+sProp,AppConfig.hClasses[sClass].nClass+':'+oObj.get(oObj.getSettings().hExtras[sProp].aKey[0]));
                            if (nTTL)
                                multi.expire(oObj.nClass+':'+oObj.getKey()+':'+sProp,nTTL);
                        }
                    }
                }
            }
...
p.deleteObject = function(oObj,fnCallback){
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            oClient.hgetall(oObj.nClass+':'+oObj.getKey()+':MAP',function(err,hMap) {

                var multi = oClient.multi();
                multi.del(oObj.nClass+':'+oObj.getKey());
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                ...
                if (hMap) {
                    for (var sKey in hMap) {
                        multi.zrem(sKey,oObj.nClass+':'+oObj.getKey());
                    }
                }
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
                        multi.del(oObj.nClass+':'+oObj.getKey()+':'+sProp);
                    }
                }
                // Delete the object's map too.
                multi.del(oObj.nClass+':'+oObj.getKey()+':MAP');
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                var multi = oClient.multi();
                multi.zrem(hOpts.sKey,[oObj.nClass+':'+oObj.getKey()]);
                multi.zadd(hOpts.sKey,oObj.get(sOrderBy),[oObj.nClass+':'+oObj.getKey()]);
                // Add reference to object map for clean-up later.
                multi.hset(oObj.nClass+':'+oObj.getKey()+':MAP',hOpts.sKey,1);
                if (nTTL)
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                multi.zrem(hOpts.sKey,[oObj.nClass+':'+oObj.getKey()]);
                // Add reference to object map for clean-up later.
                multi.hdel(oObj.nClass+':'+oObj.getKey()+':MAP',hOpts.sKey,1);
                multi.exec(function(err){
                    oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                });
            } else
    ...
    if (hOpts && hOpts.oObj && hOpts.sProperty) {
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                var sKey = hOpts.oObj.nClass+':'+hOpts.oObj.getKey()+':'+hOpts.sProperty;
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (Value instanceof Base) {
                var multi = oClient.multi();
                multi.hset(Value.nClass+':'+Value.getKey()+':MAP',hOpts.sKey,Value.nClass+':'+Value.getKey());
                if (hOpts.nTTL)
                    multi.expire(Value.nClass+':'+Value.getKey()+':MAP',hOpts.nTTL);
                multi.exec(function(err){
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (!oSelf.bSeeded) {
                ...
                multi.setnx('nSeedID',AppConfig.nSeedID);
