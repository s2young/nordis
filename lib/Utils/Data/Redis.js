...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias]) {
        ...
        if (oSelf.hOpts[sDbAlias].sPassword)
            opts.auth_pass = oSelf.hOpts[sDbAlias].sPassword;

        console.log(oSelf.hOpts[sDbAlias].nPort+','+oSelf.hOpts[sDbAlias].sHost,opts);

        oSelf.dbPool[sDbAlias] = redis.createClient(oSelf.hOpts[sDbAlias].nPort,oSelf.hOpts[sDbAlias].sHost,opts);
