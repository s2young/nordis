                ...
                async.waterfall([
                    function(callback){
                        if (hOpts && (hOpts.nMin || hOpts.nMax)) {
                            if (oObj.bReverse)
                                oClient.zrevrangebyscore(oClient.sHashKey+hOpts.sKey,parseFloat(hOpts.nMin),parseFloat(hOpts.nMax),handleResult);
                            else
                                oClient.zrangebyscore(oClient.sHashKey+hOpts.sKey,(hOpts.nMin||0),(hOpts.nMax||726081834000),callback);
                        } else
                            oClient.zcard(oClient.sHashKey+hOpts.sKey,callback);
                    }
                    ,function(nTotal,callback){
                        oObj.nTotal = (nTotal instanceof Array) ? nTotal.length : nTotal;
                                                ...
                                                if (err || !aItems || !aItems.length)
                                                    ...
                                                    async.forEachOf(aItems,function(hData,n,cb2){
                                                        ...
                                                        if (!hData) {
                                                            ...
                                                            console.log('sFirstID:'+oItem.getKey());
                                                            cb2();
                                                        }
