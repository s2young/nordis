...
p.createClient = function(sPool,sDbAlias,fnCallback,bForce){
    ...
    if (oSelf.hOpts[sDbAlias].bSkip && !bForce && !sPool.match(/(dbPubPool|dbSubPool)/)) {
        fnCallback();
    } else if (sPool) {
                ...
                oSelf.acquire(function(err,oResult){
                    if (err||!oResult)
                        ...
                        if (hOpts.hQuery[sKeyProperty]) {
                            ...
                            oClient.get(oClient.sHashKey+oObj.getClass()+':'+hOpts.hQuery[sKeyProperty],function(err,hResult){
                                if (hResult) {
                ...
                async.waterfall([
                    function(callback){
                        if (hOpts && (hOpts.nMin || hOpts.nMax)) {
                            var nMin = (hOpts.nMin) ? parseFloat(hOpts.nMin) : 0;
                            var nMax = (hOpts.nMax) ? parseFloat(hOpts.nMax) : 726081834000;
                            if (oObj.bReverse)
                                oClient.zrevrangebyscore(oClient.sHashKey+hOpts.sKey,nMin,nMax,callback);
                            else
                                oClient.zrangebyscore(oClient.sHashKey+hOpts.sKey,nMin,nMax,callback);
                        } else
                            oClient.zcard(oClient.sHashKey+hOpts.sKey,callback);
                    }
                    ,function(nTotal,callback){
                        oObj.nTotal = (nTotal.length) ? nTotal.length : nTotal;

                        if (hOpts && hOpts.nMin)
                            console.log(oClient.sHashKey+hOpts.sKey+','+nMin+','+nMax+'; nTotal:'+oObj.nTotal);
