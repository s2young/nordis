                                        ...
                                        if (aResult)
                                            oClient.mget(aResult,function(err,aItems){
                                                if (err || !aItems || !aItems.length)
                                                    ...
                                                    async.forEachOf(aItems,function(hData,ind,cb2){
                                                        hData = (hData) ? JSON.parse(hData) : null;
                                                        if (!hData) {

                                                            oObj.nTotal--;
                                                            oClient.zrem(oClient.sHashKey+hOpts.sKey,aResult[ind],function(){
                                                            ...
                                                            if (hOpts && hOpts.hExtras)
                                                                oItem.loadExtras(hOpts.hExtras, function(){
                                                                    async.setImmediate(cb2);
                                                                });
                                                            else
                                                                async.setImmediate(cb2);
...
p.hincrby = function(Key,sName,nIncr,fnCallback,sDbAlias) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            async.forEachOf(Key,function(sKey,ind,cb){
