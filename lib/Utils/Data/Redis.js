...
p.acquire = function(fnCallback,sDbAlias,nRetry) {
    ...
    if (!oSelf.dbPool || !oSelf.dbPool[sDbAlias]) {
        ...
        if (oSelf.hOpts[sDbAlias].nDb && oSelf.dbPool[sDbAlias].selected_db != oSelf.hOpts[sDbAlias].nDb) {
            console.log('Redis Db: '+oSelf.hOpts[sDbAlias].nDb);
            oSelf.dbPool[sDbAlias].select(oSelf.hOpts[sDbAlias].nDb, function (res) {
                console.log(res);
...
p.acquirePub = function(fnCallback,sDbAlias) {
    ...
    if (!oSelf.dbPubPool || !oSelf.dbPubPool[sDbAlias]) {
        ...
        if (oSelf.hOpts[sDbAlias].nDb && oSelf.dbPubPool[sDbAlias].selected_db != oSelf.hOpts[sDbAlias].nDb)
            oSelf.dbPubPool[sDbAlias].select(oSelf.hOpts[sDbAlias].nDb,function(){
...
p.loadCollection = function(hOpts,oObj,fnCallback) {
    ...
    if (oSelf.hOpts[sDbAlias].bSkip || (hOpts && hOpts.sSource == 'MySql') || (hOpts.hQuery && hOpts.hQuery.sWhere))
        fnCallback();
    else {
        async.waterfall([
            function(callback){
                                    ...
                                    async.forEach(aResult,function(sItemKey,cback){
                                        Config.Redis.dbPool[sDbAlias].hgetall(sItemKey,function(err,res){
                                            if (err)
                                                ...
                                                Config.Redis.dbPool[sDbAlias].zrem(hOpts.sKey,sItemKey);
                                                nMissing++;
                                                cback();
        ...
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                if (hOpts && hOpts.sKey) {
                    ...
                    multi.zadd(hOpts.sKey,nScore,[oObj.getClass()+':'+oObj.getKey()]);
...
p.blpop3 = function(sKey1,sKey2,sKey3,nTimeout,fnCallback,sDbAlias) {
    ...
    oClient.blpop(sKey1,sKey2,sKey3,nTimeout,function(err,res){
        oClient.quit();
        fnCallback(err,res);
    });
};
