...
p.loadObject = function(hOpts,oObj,fnCallback) {
    var oSelf = this;
    var oClient;
    var sNumKeyProperty = oObj.getSettings().sNumKeyProperty;
    async.waterfall([
        function(callback) {
            oSelf.acquire(function(err,oResult){
                oClient = oResult;
                if (hOpts.hQuery[sNumKeyProperty]) {
                    // Primary key lookup. We can get the whole object because we know where to look
                    oClient.hgetall(oObj.nClass+':'+hOpts.hQuery[sNumKeyProperty],callback);
                } else {
                    var sKey;
                    ...
                    for (var sLookup in hOpts.hQuery) {
                        ...
                        if (sKey) {
                            ...
                            sKey = oObj.nClass+':'+hOpts.hQuery[sLookup];
                    }
                    if (sKey)
                        oClient.get(sKey,callback);
                    else
                        callback(null,null);
                }
            });
        ...
        ,function(Result,callback) {
            if (Result && Result[sNumKeyProperty])
...
p.saveObject = function(hOpts,oObj,fnCallback) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            var sKey = (hOpts && hOpts.sKey) ? hOpts.sKey : oObj.nClass+':'+oObj.getNumKey();
            var multi = oClient.multi();

            App.silly('Store to Redis: '+sKey);
            ...
            if (oObj.getSettings().aSecondaryLookupKeys) {
                for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                    if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                        multi.set(oObj.nClass+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]),oObj.nClass+':'+oObj.getNumKey());
                        // The object pointer must also be set to expire if the data it's pointing to is going to expire.
                        if (nTTL)
                            multi.expire(oObj.nClass+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]),nTTL);
                    }
                }
            }
            ...
            if (oObj.getSettings().hExtras) {
                for (var sProp in oObj.getSettings().hExtras) {
                    if (oObj.getSettings().hExtras[sProp].aKey && oObj.get(oObj.getSettings().hExtras[sProp].aKey[0])) {
                        ...
                        if (!sClass && oObj.getSettings().hExtras[sProp].fnGetClass)
                            sClass = oObj.getSettings().hExtras[sProp].fnGetClass(oObj,App);
                        if (sClass) {
                            multi.hset(oObj.nClass+':'+oObj.getNumKey()+':MAP',oObj.nClass+':'+oObj.getNumKey()+':'+sProp,App.hClasses[sClass].nClass+':'+oObj.get(oObj.getSettings().hExtras[sProp].aKey[0]));
                            if (nTTL)
                                multi.expire(oObj.nClass+':'+oObj.getNumKey()+':'+sProp,nTTL);
                        }
                    }
                }
            }
...
p.deleteObject = function(oObj,fnCallback){
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            oClient.hgetall(oObj.nClass+':'+oObj.getNumKey()+':MAP',function(err,hMap) {

                var multi = oClient.multi();
                multi.del(oObj.nClass+':'+oObj.getNumKey());
                if (oObj.getSettings().aSecondaryLookupKeys) {
                    for (var i = 0; i < oObj.getSettings().aSecondaryLookupKeys.length; i++) {
                        if (oObj.get(oObj.getSettings().aSecondaryLookupKeys[i])) {
                            multi.del(oObj.nClass+':'+oObj.get(oObj.getSettings().aSecondaryLookupKeys[i]));
                        }
                    }
                }
                if (hMap) {
                    for (var sKey in hMap) {
                        multi.zrem(sKey,oObj.nClass+':'+oObj.getNumKey());
                    }
                }
                if (oObj.getSettings().hExtras) {
                    for (var sProp in oObj.getSettings().hExtras) {
                        multi.del(oObj.nClass+':'+oObj.getNumKey()+':'+sProp);
                    }
                }
                // Delete the object's map too.
                multi.del(oObj.nClass+':'+oObj.getNumKey()+':MAP');
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                var multi = oClient.multi();
                multi.zrem(hOpts.sKey,[oObj.nClass+':'+oObj.getNumKey()]);
                multi.zadd(hOpts.sKey,oObj.get(sOrderBy),[oObj.nClass+':'+oObj.getNumKey()]);
                // Add reference to object map for clean-up later.
                multi.hset(oObj.nClass+':'+oObj.getNumKey()+':MAP',hOpts.sKey,1);
                if (nTTL)
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (hOpts && hOpts.sKey) {
                ...
                multi.zrem(hOpts.sKey,[oObj.nClass+':'+oObj.getNumKey()]);
                // Add reference to object map for clean-up later.
                multi.hdel(oObj.nClass+':'+oObj.getNumKey()+':MAP',hOpts.sKey,1);
                multi.exec(function(err){
                    oSelf.dispatchResult(err,oClient,fnCallback,oObj);
                });
            } else
    ...
    if (hOpts && hOpts.oObj && hOpts.sProperty) {
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                var sKey = hOpts.oObj.nClass+':'+hOpts.oObj.getNumKey()+':'+hOpts.sProperty;
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (Value instanceof Base) {
                var multi = oClient.multi();
                multi.hset(Value.nClass+':'+Value.getNumKey()+':MAP',hOpts.sKey,Value.nClass+':'+Value.getNumKey());
                if (hOpts.nTTL)
                    multi.expire(Value.nClass+':'+Value.getNumKey()+':MAP',hOpts.nTTL);
                multi.exec(function(err){
...
p.getNextID = function(oObj,fnCallback) {
    ...
    oSelf.acquire(function(err,oClient){
        if (err)
            ...
            if (!oSelf.bSeeded) {
                if (!App.nSeedID) {
                    App.warn('No nSeedID was set in the config, so we are going to just start at 1. If you want a different seed, set the nSeedID value in your config, stop your app, delete the nSeedID key from redis, and then restart your app.');
                    App.nSeedID = 1;
                }
                ...
                multi.exec(function(err,aResult){
                    if (!err && aResult && aResult[1])
                        oObj.hData[oObj.getSettings().sNumKeyProperty] = aResult[1];
                ...
                oClient.incrby('nSeedID',1,function(err,nID){
                    if (!err && nID)
                        oObj.hData[oObj.getSettings().sNumKeyProperty] = nID;
...
p.hincrby = function(Key,sName,nIncr,fnCallback) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback);
            ...
            Key.forEach(function(sKey){
                multi.hincrby(sKey,sName,nIncr);
            })
            multi.exec(function(err,res){
                oSelf.dispatchResult(err,oClient,fnCallback,res);
            });
...
p.incr = function(Key,fnCallback) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback);
...
p.delkeys = function(aKeys,fnCallback) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback);
...
p.hlen = function(sKey,fnCallback) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,oClient,fnCallback);
