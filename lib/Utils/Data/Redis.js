...
p.dispatchResult = function(err,oResult,fnCallback) {
    ...
    if (fnCallback && fnCallback instanceof Function) {
        try {
            ...
            console.log(er,(oResult instanceof Base) ? oResult.sClass+':'+oResult.getKey() : '');
        }
    }
};
/**
                                                ...
                                                if (err || !aItems || !aItems.length)
                                                    ...
                                                    async.forEachOfLimit(aItems,100,function(hData,ind,cb2){
                                                        ...
                                                        if (!hData) {
                                                            ...
                                                            oObj.nTotal--;
                                                            oClient.zrem(oClient.sHashKey+hOpts.sKey,aResult[ind],cb2);
            ...
            if (err)
                ...
                if (oObj.getSettings().aSecondaryLookupKeys)
                    oObj.getSettings().aSecondaryLookupKeys.forEach(function(sProp){
                        if (oObj.get(sProp) && oObj.getRedisKey()) oClient.hset(oClient.sHashKey+'KEYS',oObj.getClass()+':'+oObj.get(sProp),oObj.getRedisKey());
                    });
        ...
        oSelf.acquire(function(err,oClient){
            if (err)
                ...
                if (hOpts && hOpts.sKey) {
                    ...
                    if (isNaN(nScore)) {
                        Config.warn('Score for '+hOpts.sKey+' is NaN');
                        oSelf.dispatchResult(null,oObj,fnCallback);
                    } else {

                        oClient.zrem(oClient.sHashKey+hOpts.sKey,[oClient.sHashKey+oObj.getRedisKey()],function(){});
                        oClient.zadd(oClient.sHashKey+hOpts.sKey,nScore,oClient.sHashKey+oObj.getRedisKey(),function(){});
                        // Add reference to object map for clean-up later.
                        if (oClient.sHashKey+hOpts.sKey) oClient.hset(oClient.sHashKey+oObj.getRedisKey()+':MAP',oClient.sHashKey+hOpts.sKey,1,function(){});

                        var nTTL = (hOpts.nTTL) ? hOpts.nTTL : oObj.getSettings().nTTL;
                        if (nTTL) oClient.expire(oClient.sHashKey+hOpts.sKey,nTTL,function(){}); // in seconds
...
p.sub = function(sKey,fnCallback,sDbAlias) {
...
p.pushAndPublish = function(sKey,sValue,fnCallback,sDbAlias) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,null,fnCallback);
        else {
            Config.debug('pushAndPublish: '+oClient.sHashKey+sKey,oClient.sHashKey+'{PUB}'+sKey);
            oClient.rpush(oClient.sHashKey+sKey,sValue,function(err,res){
                oClient.publish(oClient.sHashKey+'{PUB}'+sKey,sValue,function(err,res){
                    oSelf.dispatchResult(err,res,fnCallback);
                });
            });
        }
...
p.lpop1 = function(sKey1,fnCallback,sDbAlias) {
    var oSelf = this;
    sDbAlias = (oSelf.hOpts[sDbAlias]) ? sDbAlias : 'default';
    oSelf.createClient(null,sDbAlias,function(err,oClient){
        if (err)
            ...
            oClient.lpop(oSelf[sDbAlias].sHashKey+sKey1,function(err,res){
                oClient.quit();
...
p.mget = function(aKeys,fnCallback,sDbAlias) {
    var oSelf = this;
    oSelf.acquire(function(err,oClient){
        if (err)
            oSelf.dispatchResult(err,null,fnCallback);
        else {
            aKeys.forEach(function(sKey,n){
                aKeys[n] = oClient.sHashKey+sKey;
            });
            oClient.mget(aKeys,function(err,res){
                oSelf.dispatchResult(err,res,fnCallback);
            });
        }
    },sDbAlias);
