...
var apiPreparser = function(req,res,next){
    ...
    if (req.path && req.path != '/') {
        Config.info('START: '+req.method+' '+req.path+' ------');
        if (Config.get('sLogLevel').match(/debug|silly/)) {
        ...
        preParse(req,res,function(){
            Config.info('END: '+req.method+' '+req.path+' ------');
            next();
        });
    } else
        next();
...
var preParse = function(req,res,next) {
    ...
    if (!req.hNordis.sException && !req.hNordis.hException) {
        if (Config.get('hEndpointKeywords')[aPaths[1]]) {
            var hMatch;
            for (var i = 0; i < Config.get('hEndpointKeywords')[aPaths[1]].length; i++) {
                var hItem = Config.get('hEndpointKeywords')[aPaths[1]][i];
                var aMatches;
                if (req.path == hItem.sEndpoint) {
                    hMatch = hItem;
                    break;
                } else {
                    aMatches = req.path.match(new RegExp('^' + hItem.sRegEx + '$'));
                    if (aMatches) {
                        hMatch = hItem;
            ...
            if (hMatch) {
                ...
                req.hNordis.hEndpoint = Config.getClasses(hMatch.sApiClass||hMatch.sClass).hApi.hEndpoints[hMatch.sEndpoint].hVerbs[req.method];
                ...
                if (req.hNordis.sClass == 'Stat') {
                    ...
                    Config.get('hStats')[hMatch.sStat].aGrains.forEach(function(sGrain){
                        if (req.query[sGrain] || hMatch.sGrain == 'all') {
                            req.hNordis.hExtras[hMatch.sStat].hExtras[sGrain] = (sGrain != 'alltime') ? {
                                nMin:(req.query.hExtras) ? req.query.hExtras[sGrain].nMin : req.query.nMin
                                ,nMax:(req.query.hExtras) ? req.query.hExtras[sGrain].nMax : req.query.nMax
                                ,bReverse:(req.query.hExtras) ? req.query.hExtras[sGrain].bReverse : req.query.bReverse
                                ,nSize:(req.query.hExtras) ? req.query.hExtras[sGrain].nSize : req.query.nSize
                            } : true;
                        }
                    });
                }
    ...
    if (!req.hNordis.sClass && !req.hNordis.hEndpoint)
        ...
        async.series([
            ...
            ,function(cb){
                if (req.hNordis.oResult)
                    Config.debug('FOUND req.hNordis.oResult: '+req.hNordis.oResult.sClass+' ('+req.hNordis.oResult.getKey()+')');
                ...
                if (!req.hNordis.hEndpoint) {
                    ...
                    Config.debug(req.path);
                    Config.debug(req.hNordis.hEndpoint);
                    cb();
                }
            }
            ...
            ,function(cb) {
                ...
                if (fnApiCallProcessor) {
                    fnApiCallProcessor(req,Config.inst||Config,function(err,result){
            ...
            ,function(cb) {
                ...
                if (fnApiCallOutput)
                    fnApiCallOutput(req,Config.inst||Config,function(err,hResult){
                        req.hNordis.hResult = hResult;
                        cb();
                    });
                else if (!req.hNordis.oResult)
...
var clientHelper = function(req,res,next) {
    if (req.path.match(/\/_nordis_/)) {
        ...
        var sVersion = Config.get('sConfVersion')||'';
        ...
        switch (req.path) {
            case '/_nordis_server/version':
                if (Config.get('NORDIS_ENV_ROOT_DIR')) {
                    var hNordisInstance = require(Config.get('NORDIS_ENV_ROOT_DIR')+'/node_modules/nordis/package.json');
...
var getClientJS = function(minified,callback,localStorage) {
    ...
    var hContext = {hClasses:{},hApiCalls:{},hKeys:{},sMostCommonPrimaryKey:Config.get('sMostCommonPrimaryKey')};
    hContext.sNordisHost = Config.get('sNordisHost') || '';
    ...
    for (var sClass in Config.getClasses()) {
        ...
        if (Config.getClasses(sClass).hApi && Config.getClasses(sClass).hApi.hEndpoints) {
            append(Config.getClasses(sClass).hApi.hEndpoints,true);
        }
        // Look in the root hApi for any endpoints that belong in this class.
        if (Config.get('hApi') && Config.get('hApi').hEndpoints)
            append(Config.get('hApi').hEndpoints);
