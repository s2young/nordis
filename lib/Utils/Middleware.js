...
var preParse = function(req,res,next) {
    ...
    if (!req.hNordis.sException && !req.hNordis.hException) {
        if (AppConfig.hEndpointKeywords[aPaths[1]]) {
            var hMatch;
            for (var i = 0; i < AppConfig.hEndpointKeywords[aPaths[1]].length; i++) {
                var hItem = AppConfig.hEndpointKeywords[aPaths[1]][i];
                var aMatches;
                if (req.path == hItem.sEndpoint) {
                    ...
                    if (aMatches) {
                        ...
                        hMatch.sClass = AppConfig.hClassMap[aPaths[1]];
                        if (hMatch.sClass=='Stat') {
                            hMatch.sStat = AppConfig.hClassMap[aPaths[2]];
            ...
            if (hMatch) {
                ...
                req.hNordis.hEndpoint = AppConfig.hClasses[hMatch.sApiClass||hMatch.sClass].hApi.hEndpoints[hMatch.sEndpoint].hVerbs[req.method];
                ...
                if (req.hNordis.sClass == 'Stat') {
                    ...
                    AppConfig.hStats[hMatch.sStat].aGrains.forEach(function(sGrain){
