...
var apiRender = function(req,res) {
    ...
    if (req.hNordis.hException) {
        ...
        Config.error(req.hNordis.hException);
        res.status(500);
        res.end(JSON.stringify(req.hNordis.hException));
    } else if (req.hNordis.sException) {
        Config.error(req.hNordis.sException);
        req.hNordis.hExceptions = {nType:0,sMessage:req.hNordis.sException};
        res.status(500);
        res.end(JSON.stringify(req.hNordis.hException));
    } else {
        console.log('hello',req.hNordis.hResult);
        res.statusCode = 200;
        if (req.hNordis.hResult && (req.hNordis.hResult instanceof Object))
            res.end(JSON.stringify(req.hNordis.hResult));
        else
            res.end(req.hNordis.hResult);
    }
};
/**
