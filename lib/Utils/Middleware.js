        ...
        switch (req.path) {
            ...
            case '/_nordis_client.js':
                ...
                if (process.env[current_version])
                    ...
                    sDirectives += "\n.factory('AppConfig',function(helpers){ return {hClasses:{"+aClasses.join(',')+"}} })";


                    if (aClassesWithCalls.length)
                        aClassesWithCalls.forEach(function(sClass){
                            sFactories += '\n.factory(\''+sClass+'\',function(AppConfig) {var '+sClass+' = {};for (var sEndpoint in AppConfig.hClasses.'+sClass+'.hApi) {'+sClass+'[sEndpoint] = AppConfig.hClasses.'+sClass+'.hApi[sEndpoint];}; return '+sClass+';})'
                        });

                    fs.readFile(__dirname+'/angular.nordis.js',function(err,clientjs){
                        clientjs = clientjs.toString()+sDirectives+sFactories;

//                        var result = UglifyJS.minify(clientjs, {fromString: true});
