 ...
 *  or the numeric-key (defined in sNumericKey property of the class config) of the object. You can pass 'new' for new object creations.
...
var apiPreparser = function(req,res,next) {
    ...
    if (!req.hNordis.sAction || !req.hNordis.sClass) {
        ...
        req.hNordis.hQuery = {};
        if (isNaN(aPath[2]) && App.hClasses[req.hNordis.sClass] && App.hClasses[req.hNordis.sClass].sStringKey) {
            req.hNordis.hQuery[App.hClasses[req.hNordis.sClass].sStringKey] = aPath[2];
        } else {
            req.hNordis.hQuery[App.hClasses[req.hNordis.sClass].sNumericKey] = aPath[2];
        }
