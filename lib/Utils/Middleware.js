...
var preParse = function(req,res,next) {
    ...
    if (!req.hNordis.sException && !req.hNordis.hException) {
        if (Config.hEndpointKeywords[aPaths[1]]) {
            ...
            if (hMatch && hMatch.sMetric) {
                req.hNordis.sClass = hMatch.sClass;
                req.hNordis.sMetric = hMatch.sMetric;
                req.hNordis.sEndpoint = hMatch.sEndpoint;
                req.hNordis.hEndpoint = (Config.getClasses(hMatch.sClass)) ? Config.getClasses(hMatch.sClass).hMetrics[hMatch.sMetric] : null;
    ...
    if (!req.hNordis.sClass && !req.hNordis.hEndpoint) {
        ...
        async.series([
            ...
            function(cb) {
                if (req.hNordis.oResult || req.hNordis.sMetric)
                    cb();
            ...
            ,function(cb){
                ...
                } else if (req.hNordis.sMetric) {

                    Metric.lookup({sClass:req.hNordis.sClass,nMin:req.query.nMin,nMax:req.query.nMax,sFilter:req.hNordis.sFilter,hMetrics:req.hNordis.hExtras},function(err,oStat){
                        req.hNordis.hResult = {};
                        req.hNordis.hResult[req.hNordis.sMetric] = {};
                        if (oStat && req.hNordis.hExtras && req.hNordis.hExtras[req.hNordis.sMetric]) {
                            var hExtras = req.hNordis.hExtras[req.hNordis.sMetric];
                            for (var sProp in hExtras) {
                                if (oStat[req.hNordis.sMetric][sProp] && oStat[req.hNordis.sMetric][sProp] instanceof Base)
                                    req.hNordis.hResult[req.hNordis.sMetric][sProp] = oStat[req.hNordis.sMetric][sProp].toHash(hExtras[sProp].hExtras);
                                else
                                    req.hNordis.hResult[req.hNordis.sMetric][sProp] = oStat[req.hNordis.sMetric][sProp];
                            }
                        }
                        cb(err);
                    });
