...
var preParse = function(req,res,fnCallback) {
    ...
    (function findPath(hMap,nIndex){
        ...
        if (aPaths[nIndex+1]) {
            ...
            if (AppConfig.hClasses[sClass].hApi && AppConfig.hClasses[sClass].hApi.hEndpoints && AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint]) {
                req.hNordis.hEndpoint = AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint];
            }
        }
                ...
                if (req.hNordis.oResult && req.hNordis.hExtras) {
                    ...
                    req.hNordis.oResult.loadExtras(req.hNordis.hExtras,function(err){
                        if (err)
                            ...
                            AppConfig.debug(req.hNordis.oResult);
