...
var apiPreparser = function(req,res,next){
    ...
    if (req.path && req.path != '/') {
        ...
        if (AppConfig.sLogLevel.match(/debug|silly/)) {
            if (req.query) AppConfig.debug(JSON.stringify(req.query));
            if (req.body) AppConfig.debug(JSON.stringify(req.body));
        }
...
var preParse = function(req,res,fnCallback) {
    ...
    if (!req.hNordis.sException && !req.hNordis.hException)
        (function findPath(hMap,nIndex){
            ...
            if (aPaths[nIndex]) {
                var sPath = aPaths[nIndex];
                var sProp;
                if (nIndex==1) req.hNordis.sClass = AppConfig.hClassMap[sPath.toLowerCase()];

                if (hMap[sPath] || hMap[sPath.toLowerCase()]) {
                    hMap = hMap[sPath] || hMap[sPath.toLowerCase()];
                    aEndpoint.push(sPath);
                    if (!aPaths[nIndex+1] && req.path != req.hNordis.sPath) {
                    ...
                    for (var sProp in hMap) {
                        if (sProp.match(/^\{/)) {
                            ...
                            if (sProp == '{grain}' && req.hNordis.sClass == 'Stat' && AppConfig.hClasses[aPaths[nIndex-1]]) {
                                var hExtras = {};
                                if (sPath != 'all' && sPath != 'custom') {
                                    if (AppConfig.hStats[sPath].aGrains.join(',').match(sPath)) {
                                        if (req.query && sPath != 'alltime')
                                            hExtras[sPath] = {nMin:req.query.nMin,nMax:req.query.nMax,bReverse:req.query.bReverse,nSize:req.query.nSize};
                                        else
                                            hExtras[sPath] = true;
                                    }
                                } else if (sPath == 'custom' && req.query) {
                                    for (var sGrain in req.query.hExtras) {
                                        if (AppConfig.hStats[aPaths[nIndex-1]].aGrains.join(',').match(sGrain))
                                            hExtras[sGrain] = (sGrain != 'alltime') ? {
                                                nMin:req.query.hExtras[sGrain].nMin||req.query.nMin
                                                ,nMax:req.query.hExtras[sGrain].nMax||req.query.nMax
                                                ,bReverse:req.query.hExtras[sGrain].bReverse||req.query.bReverse
                                                ,nSize:req.query.hExtras[sGrain].nSize||req.query.nSize
                                            } : true;
                                    }
                                } else if (sPath == 'all' && req.query) {
                                    AppConfig.hClasses[aPaths[nIndex-1]].aGrains.forEach(function(sGrain){
                                        hExtras[sGrain] = (sGrain != 'alltime') ? {
                                            nMin:(req.query.hExtras[sGrain] && req.query.hExtras[sGrain].nMin) ? req.query.hExtras[sGrain].nMin : req.query.nMin
                                            ,nMax:(req.query.hExtras[sGrain] && req.query.hExtras[sGrain].nMax) ? req.query.hExtras[sGrain].nMax : req.query.nMax
                                            ,bReverse:(req.query.hExtras[sGrain] && req.query.hExtras[sGrain].bReverse) ? req.query.hExtras[sGrain].bReverse : req.query.bReverse
                                            ,nSize:(req.query.hExtras[sGrain] && req.query.hExtras[sGrain].nSize) ? req.query.hExtras[sGrain].nSize : req.query.nSize
                                        } : true;
                                    });
                                }
                                req.hNordis.hExtras = {};
                                req.hNordis.hExtras[aPaths[nIndex-1]] = {};
                                req.hNordis.hExtras[aPaths[nIndex-1]].hExtras = hExtras;
                ...
                if (aPaths[nIndex+1]) {
                    ...
                    if (AppConfig.hClasses[sClass].hApi
                        && AppConfig.hClasses[sClass].hApi.hEndpoints
                        && AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint]
                        && AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint].hVerbs
                        && AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint].hVerbs[req.method]) {

                        req.hNordis.sPath = sEndpoint;
                        req.hNordis.hEndpoint = AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint].hVerbs[req.method];

                    } else if (AppConfig.hApi
                        && AppConfig.hApi.hEndpoints
                        && AppConfig.hApi.hEndpoints[sEndpoint]
                        && AppConfig.hApi.hEndpoints[sEndpoint].hVerbs
                        && AppConfig.hApi.hEndpoints[sEndpoint].hVerbs[req.method]) {
        ...
        async.series([
            ...
            ,function(cb){
                ...
                if (!req.hNordis.hEndpoint) {
                    ...
                    AppConfig.debug(req.hNordis.sEndpoint);
                    AppConfig.silly(req.hNordis.hEndpoint);
                    cb();
                }
            }
            ...
            ,function(cb) {
                ...
                var fnApiCallProcessor = (req.sApiVersion && req.hNordis.hEndpoint[req.sApiVersion] && req.hNordis.hEndpoint[req.sApiVersion].fnApiCallProcessor) ? req.hNordis.hEndpoint[req.sApiVersion].fnApiCallProcessor : req.hNordis.hEndpoint.fnApiCallProcessor;
                if (fnApiCallProcessor) {
                    fnApiCallProcessor(req,AppConfig,function(err,result){
            ...
            ,function(cb) {
                var fnApiCallOutput = (req.sApiVersion && req.hNordis.hEndpoint[req.sApiVersion] && req.hNordis.hEndpoint[req.sApiVersion].fnApiCallOutput) ? req.hNordis.hEndpoint[req.sApiVersion].fnApiCallOutput : req.hNordis.hEndpoint.fnApiCallOutput;
                if (fnApiCallOutput)
                    fnApiCallOutput(req,AppConfig,function(err,hResult){
                        req.hNordis.hResult = hResult;
                        cb();
                    });
                else if (!req.hNordis.oResult)
 ...
 * @param minified
 */
var getClientJS = function(minified,callback,localStorage) {
    var Template = require('./Template');

    // Build the data to pass into template compiler, which will output custom js for the client.
    var hContext = {hClasses:{},hApiCalls:{},hKeys:{},sMostCommonPrimaryKey:AppConfig.sMostCommonPrimaryKey};
    ...
    var sLocalStorage = (localStorage) ? '.localstorage' : '';
