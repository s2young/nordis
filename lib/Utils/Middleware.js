...
var apiPreparser = function(req,res,next) {
    ...
    req.hNordis.sAction = (aPath && aPath[3]) ? aPath[3].toLowerCase() : null;

    App.info(req.hNordis.sAction+': '+req.hNordis.sAction);
    ...
    if (!req.hNordis.sAction || !req.hNordis.sClass) {
        ...
        Base.lookup({sClass:req.hNordis.sClass,hQuery:{sID:req.hNordis.sID}},function(err,oObj){
            if (err)
                handleError(err);
            else {
                req.hNordis.oResult = oObj;

                if (req.hNordis.sAction.match(/(save|delete|details)\.json/)) {
                    ...
                    async.series([
                        ...
                        ,function(cb){
                            var sExtra = req.hNordis.sAction.replace('.json','');
