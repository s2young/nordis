...
var apiPreparser = function(req,res,next) {
    ...
    req.sClass = (aPath && aPath[1]) ? App.hClassMap[aPath[1]] : null
    ...
    req.sAction = (aPath && aPath[3]) ? aPath[3].replace('.json','') : null;
    if (!req.sAction || aPath.length != 4 || !req.sClass || !req.sAction.match(/(save|details|delete)/)) {
        next();
    } else {
        // The first item in the path should be in the hClassMap.
        req.sID = aPath[2];
        req.sAction = aPath[3].replace('.json','');
        Base.lookup({sClass:req.sClass,hQuery:{sID:req.sID}},function(err,oObj){
            if (err)
                ...
                async.series([
                    ...
                    ,function(cb){
                        if (req.oResult.hSettings() && req.oResult.hSettings().hExtras && req.oResult.hSettings().hExtras[req.sAction]) {
                            // We're digging into the oResult's object tree and switching to that as the root oResult for the request.
                            hExtras = {};
                            hExtras[req.sAction] = req.body;
                            req.oResult.loadExtras(hExtras,function(err){
                                hExtras = hExtras[req.sAction].hExtras;
                                req.oResult = req.oResult[req.sAction];
                                cb(err);
                            });
                        } else if (hExtras)
