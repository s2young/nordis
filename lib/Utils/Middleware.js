        ...
        switch(nIndex) {
            ...
            case 2:
                ...
                if (AppConfig.hClasses[req.hNordis.sClass]) {
                    ...
                    try {
                        hEndpoint = AppConfig.hClasses[req.hNordis.sClass].hApi.hEndpoints[req.hNordis.sPath].hVerbs[req.method];
            ...
            case 3:
                ...
                if (!hEndpoint && AppConfig.hClassMap[sItem.toLowerCase()]) {
                    aClasses.push(AppConfig.hClassMap[sItem.toLowerCase()]);
                    try {
                        hEndpoint = AppConfig.hClasses[AppConfig.hClassMap[sItem.toLowerCase()]].hApi.hEndpoints[req.hNordis.sPath].hVerbs[req.method];
                        req.hNordis.sExtra = sItem;
                    } catch(err2) {
                        hEndpoint = null;
                        req.hNordis.sExtra = null;
                ...
                if (!hEndpoint && AppConfig.hClasses[req.hNordis.sClass].hExtras[sItem]) {
                    aClasses.push(AppConfig.hClasses[req.hNordis.sClass].hExtras[sItem].sClass);
                    try {
                        hEndpoint = AppConfig.hClasses[AppConfig.hClasses[req.hNordis.sClass].hExtras[sItem].sClass].hApi.hEndpoints[req.hNordis.sPath].hVerbs[req.method];
                        req.hNordis.sExtra = sItem;
                    } catch (err3) {
                        hEndpoint = null;
                        req.hNordis.sExtra = null;
            ...
            ,function(cb) {
                if (hEndpoint.fnApiCallProcessor)
                    hEndpoint.fnApiCallProcessor(req,AppConfig,function(err,result){
                        ...
                        console.log('fnApiCallProcessor DONE');
