            ...
            for (var sProp in hMap) {
                if (sProp.match(/^\{/)) {
                    ...
                    if (sProp == '{grain}' && req.hNordis.sClass == 'Stat' && AppConfig.hClasses[aPaths[nIndex-1]]) {
                        // Handle app tenants, by injecting middleware before the api middleware and set oResult with the App set with the proper sid.
                        // Ex: req.hNordis.oResult = Base.lookup({sClass:'App',hData:{sid:'tenant_id_here'}});
                        if (!req.hNordis.oResult)
                            req.hNordis.oResult = Base.lookup({sClass:'App',hData:{sid:'app'}});

                        req.hNordis.oResult[aPaths[nIndex-1]] = Base.lookup({sClass:aPaths[nIndex-1],hData:AppConfig.hClasses[aPaths[nIndex-1]].hData});
                        req.hNordis.oResult = req.hNordis.oResult[aPaths[nIndex-1]];
                        req.hNordis.hExtras = {};
                        if (sPath != 'all' && sPath != 'custom') {
                            ...
                            for (var sGrain in req.query.hExtras) {
                                req.hNordis.hExtras[sGrain] = (sGrain != 'alltime') ? {
