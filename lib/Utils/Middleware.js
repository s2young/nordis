                ...
                switch (req.method) {
                    case 'POST':
                        if (req.hNordis.oResult)
                            req.hNordis.oResult.save(null,cb);
                        else
                            cb(App.getError(500));
                        break;
                    case 'DELETE':
...
var apiPreparser = function(req,res,next){
    async.series([
        ...
        ,function(callback) {
            if (App.fnMiddleware)
                App.fnMiddleware(req,res,callback,App,async);
            else
                callback();
        }
    ],next);
...
var preParse = function(req,res,fnCallback) {
    ...
    req.path.split('/').forEach(function(sItem,nIndex){
        switch(nIndex) {
            case 1:
                ...
                req.hNordis.sClass = (sItem && App.hClassMap[sItem]) ? App.hClassMap[sItem] : null;
                sConfClass = App.hClassMap[sItem];

                break;
            case 2:
                // This should be the id property (either string or numeric).
                req.hNordis.hQuery = {};
                if (App.hClasses[req.hNordis.sClass]) {
                    if (isNaN(sItem) && App.hClasses[req.hNordis.sClass] && App.hClasses[req.hNordis.sClass].sStrKeyProperty) {
                        req.hNordis.sLookupProperty = App.hClasses[req.hNordis.sClass].sStrKeyProperty;
                    } else {
                        req.hNordis.sLookupProperty = App.hClasses[req.hNordis.sClass].sNumKeyProperty;
                    }
                    ...
                    try {
                        hEndpoint = App.hClasses[sConfClass].hApi.hEndpoints[req.hNordis.sPath].hVerbs[req.method]
                    } catch(err) {}
                }
                break;
            case 3:
                ...
                sConfClass = App.hClassMap[req.hNordis.sClass.toLowerCase()+'.'+sItem];
    ...
    if (!req.hNordis.sClass) {
        ...
        if (!hEndpoint) {
            App.warn('API call not configured: '+req.path+'. Tried looking in configuration under \''+req.hNordis.sPath+'\' using method '+req.method+' inside class: '+sConfClass);
            passItOn();
        } else {

            async.waterfall([
                // Look up the root object.
                function(cb) {
                    Base.lookup(req.hNordis,cb);
                }
                // Check for fnValidate, through which you can write a completely custom handler for setting properties, checking security, and tracking stats.
                // Otherwise, just call setData on POST requests or pass through if not.
                ,function(oResult,cb) {
                    req.hNordis.oResult = oResult;
                    if (hEndpoint.fnValidate)
                        hEndpoint.fnValidate(req,App,function(err){
                ...
                ,function(oResult,cb) {
                    if (hEndpoint.fnApiOutput)
                        hEndpoint.fnApiOutput(req,App,cb);
                    else if (req.hNordis.sExtra)
                        cb(null,req.hNordis.oResult[req.hNordis.sExtra].toHash(req.hNordis.hExtras[req.hNordis.sExtra]));
                    else
