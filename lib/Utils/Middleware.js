            ...
            switch(nIndex) {
                ...
                case 3:
                    ...
                    if (req.hNordis.sClass) {
                        ...
                        if (!req.hNordis.hEndpoint && AppConfig.hClasses[req.hNordis.sClass] && AppConfig.hClasses[req.hNordis.sClass].hExtras && AppConfig.hClasses[req.hNordis.sClass].hExtras[sItem]) {
    ...
    if (req.path.match(/\/_nordis_/)) {
        ...
        switch (req.path) {
            ...
            case '/_nordis_client/version.js':
                res.end('define(function (require, exports, module) {exports.version = "'+AppConfig.version+'"});');
                break;
            case '/_nordis_client.js':
                // Return from memory unless the version has changed.
                var current_version = 'nordis_client_'+AppConfig.version;
                if (process.env[current_version])
                    res.end(process.env[current_version]);
                else {
                    var UglifyJS = require('uglify-js');

                    // Create custom directives using the conf.js, and expose the API endpoints exposed there.
                    // Start with model definitions.
                    var aClasses = [];
                    // Keep track of the classes that have exposed api calls, because we'll build factories.
                    var aClassesWithCalls = [];
                    var sDirectives = '';var sFactories = '';
                    for (var sClass in AppConfig.hClasses) {
                        var sConf = '\n'+sClass+':{';
                        sConf += '\nhProperties:'+JSON.stringify(AppConfig.hClasses[sClass].hProperties);
                        sConf += '\n,hApi:{';

                        // Now append any exposed api calls for the class.
                        var aCalls = [];
                            ...
                            for (var sEndpoint in AppConfig.hClasses[sClass].hApi.hEndpoints) {
                                if (AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint].hVerbs)
                                    for (var sVerb in AppConfig.hClasses[sClass].hApi.hEndpoints[sEndpoint].hVerbs) {
                                        ...
                                        if (hVerb.sAlias) {
                                            aCalls.push(hVerb.sAlias+':function(hQuery,hData,hExtras,callback){helpers.'+sVerb.toLowerCase()+'({sPath:"'+sEndpoint.replace(/\{id\}/,'"+hQuery.sid')+',hData:hData,hExtras:hExtras},function(err,res){delete res.txid;callback(err,res);})}');
                                        }
                                    }
                            }
                        }
                        sConf += aCalls.join(',')+'}}'
                        if (aCalls.length)
                            aClassesWithCalls.push(sClass);

                        aClasses.push(sConf);
                    }

                    sDirectives += ".directive('AppConfig',function(helpers){ return {hClasses:{"+aClasses.join(',')+"}} })";


                    if (aClassesWithCalls.length)
                        aClassesWithCalls.forEach(function(sClass){
                            sFactories += '.factory(\''+sClass+'\',function(AppConfig) {var '+sClass+' = {};for (var sEndpoint in AppConfig.hClasses.'+sClass+'.hApi) {'+sClass+'[sEndpoint] = AppConfig.hClasses.'+sClass+'.hApi[sEndpoint];}; return '+sClass+';})'
                        });

                    fs.readFile(__dirname+'/angular.nordis.js',function(err,clientjs){
                        //clientjs += sDirectives+sFactories;
                        console.log(clientjs.toString());
                        //var result = UglifyJS.minify(clientjs.toString(), {fromString: true});
