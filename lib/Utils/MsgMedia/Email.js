...
var Email = function(){
    ...
    oSelf.oIMAP = new Imap({
        username: App.hOptions.Email['Reply']['user'],
        password: App.hOptions.Email['Reply']['pass'],
        host: 'imap.gmail.com',
        port: 993,
        secure: true,
        connTimeout:100000,
...
p.prepareEmail = function(hOpts,fnCallback) {
    ...
    async.series([
        ...
        function(callback){
            hOpts.from = (hOpts.sFrom) ? hOpts.sFrom : hOpts.from;
            if (!hOpts.from && hOpts.oMsgResult) {
                var sEmail = (hOpts.oMsgResult.oMsg.get('nApiConsumerID')) ? App.getConsumer(hOpts.oMsgResult.oMsg.get('nApiConsumerID')).get('sEmail') : null;
                hOpts.from = App.getConsumer(hOpts.oMsgResult.oMsg.get('nApiConsumerID')).getTitle();

                if (hOpts.oMsgResult.oSender) {
                    if (!hOpts.oMsgResult.oMsg.oPlatform)
                        hOpts.oMsgResult.oMsg.oPlatform = hOpts.oMsgResult.oSender.getPlatform({
                            nMedium:App.nMedium_Email,
                            nStatus:App.nPlatformStatus_Active,
                            nApiConsumerID:hOpts.oMsgResult.oMsg.get('nApiConsumerID')
                        });
                    ...
                    if (hOpts.oMsgResult.oMsg.oPlatform)
                        sEmail = hOpts.oMsgResult.oMsg.oPlatform.get('sEmail');
                    else if (hOpts.oMsgResult.oSender.get('sEmail'))
                        sEmail = hOpts.oMsgResult.oSender.get('sEmail');

                    if (hOpts.oMsgResult.oSender.get('sName'))
                        hOpts.from = hOpts.oMsgResult.oSender.get('sName');
                }
                ...
                hOpts.oMsgResult.from = hOpts.from;
            }
            // LAST RESORT FROM
            if (!hOpts.from)
                hOpts.from = App.hOptions.Email.oQuickMail.from;
            callback();
        }
    ],function(err){
        fnCallback(err);
