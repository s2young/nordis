    ...
    async.series([
        ...
        ,function(callback) {
            for (var sStat in AppConfig.hStats) {
                if (AppConfig.hStats[sStat].fnValidate) {
                    aRedisStats.push(sStat);
                    hConfirmExtras[sStat] = {nSize:1};
                } else if (AppConfig.hStats[sStat].fnProcessQuery) {// Mysql-based stats provide a query to retrieve the stat with a passed-in date range.
                    aDbStats.push(sStat);
                    hConfirmExtras[sStat] = {nSize:1};
                }
            }
            ...
            if (!aRedisStats || !aRedisStats.length)
                ...
                async.forEach(aRedisStats,function(sStat,cb) {
                    async.waterfall([
                        ...
                        ,function(n,cback) {
                            var hQuery = {sid:sStat};
                            if (oApp.getKey()) hQuery.app_id = oApp.getKey();
                            Base.lookup({sClass:sStat,hQuery:hQuery},function(err,oSingleton){
                                if (err)
                                    cback(err);
                                else if (!oSingleton || !oSingleton.getKey()) {
                                    AppConfig.warn('No singleton found for '+sStat+' ('+(hQuery.app_id||'')+')');
                                    cback();
                                } else {
                                    oSingleton.set('updated',new Date().getTime());
                                    oSingleton.save({bForce:true},cback);
                                }
                            });
                        }
                    ],cb);
            ...
            if (aDbStats.length)
                async.forEach(aDbStats,function(sStat,cb){
                        ...
                        if (err)
                            ...
                            Base.lookup({sClass:sStat,hQuery:hQuery},function(err,oSingleton){
                                if (err)
                                    ...
                                    oSingleton.set('updated',new Date().getTime());
                                    oSingleton.save({bForce:true},cb);
                                }
                            });
                        }
                    };
                    ...
                    if (hOpts && hOpts.dStart && hOpts.dEnd) {
                        ...
                        async.forEach(aIncrements,processMySqlStat,updateSingletonAndFinish);
