...
module.exports.track = function(hOpts,fnCallback){
    ...
    else if (!Config.get('hStats')[sStat])
        returnError('Stat not configured: '+sStat);
    else if (!Config.get('hStats')[sStat].fnValidate)
        returnError('fnValidate not defined for stat: '+sStat);
    else
        Config.get('hStats')[sStat].fnValidate(Params, function (err, sKey) {
            if (err)
                returnError(err);
            else {
                ...
                if (aKeys.length) {
                    if (sKey)
                        Config.get('Redis').hincrby(aKeys, sKey, hOpts.nFakeCount||1, fnCallback, Config.get('hStats').sDbAlias);
                    else
                        Config.get('Redis').incrby(aKeys, hOpts.nFakeCount||1, fnCallback, Config.get('hStats').sDbAlias);
                }
            }
        });
};
...
module.exports.process = function(hOpts,fnCallback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            var hStats =  {};
            if (hOpts.sStat && Config.get('hStats')[hOpts.sStat])
                hStats[hOpts.sStat] = Config.get('hStats')[hOpts.sStat];
            else if (hOpts.hStats)
                hStats = hOpts.hStats;
            else
                hStats = Config.get('hStats');
            ...
            if (!hOpts.oApp) {
                Collection.lookup({sClass:'App',hQuery:{root:true},hExtras:hConfirmExtras},function(err,cColl){
                    if (err)
                        ...
                        console.log(Config.get('MySql').hTrace);
                        cApps = cColl;
                        callback();
                    }
                });
                ...
                async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                    ...
                    async.forEachLimit(aRedisStats,1,function(sStat,cb) {
                        ...
                        async.waterfall([
                            ...
                            // primary redis box.
                            function(cback) {
                                Config.get('Redis').keys(oApp.getKey()+'-'+sStat+',*',cback,Config.get('hStats').sDbAlias);
                            }
                                    ...
                                    async.forEach(aKeys,function(sKey,cback2){
                                        ...
                                        async.waterfall([
                                            function(cback3){
                                                ...
                                                Config.get('hStats')[sStat].fnValidate(null,function(err){
                                                    if (err)
                                                        Config.get('Redis').hgetall(sKey,cback3,Config.get('hStats').sDbAlias);
                                                    else
                                                        Config.get('Redis').get(sKey,cback3,Config.get('hStats').sDbAlias);
                                                });
                                            }
                                        ],function(err,res){
                                            if (err || !res)
                                                ...
                                                var processRedisStat = function(hOpts,callback) {
                                                    ...
                                                    if (Config.get('hStats')[hOpts.name] && Config.get('hStats')[hOpts.name].aGrains && !Config.get('hStats')[hOpts.name].aGrains.join(',').match(hOpts.sGrain)) {
                                                        cback2();
                                                    } else {
                                                        ...
                                                        Base.lookup({sClass:'Stat',hQuery:hQuery,sSource:'MySql'},function(err,oStat){
                                                            if (err)
                                                                ...
                                                                async.series([
                                                                    ...
                                                                    ,function(cb){
                                                                        Config.get('Redis').del(hOpts.sKey,cb,oStat.getSettings().sDbAlias);
                                                                    }
                                                                ],callback);
                                                            }
                                                        });
                                                ...
                                                if (res instanceof Object) {
                                                    ...
                                                    for (var sFilter in res) {
                                                        if (Config.get('hStats')[sStat].bFilters)
                                                    ...
                                                    processRedisStat({
                                                        ...
                                                        ,sGrain:sGrain
                                                        ,count:nCount
                                                        ,filters:(Config.get('hStats')[sStat].bFilters) ? res : null
                                                        ,name:sStat
                                                        ,year:aParts[1]||null
                                                        ,month:aParts[2]||null
                                                        ,day:aParts[3]||null
            ...
            if (aDbStats.length)
                async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                    ...
                    async.forEachLimit(aDbStats,1,function(sStat,cb){
                        ...
                        var hSettings = Config.get('hStats')[sStat];
                        ...
                        if (hOpts && hOpts.dStart && hOpts.dEnd) {
                            ...
                            while (dNow < dEnd) {
                                if (nDay != dNow.date() && Config.get('hStats')[sStat].aGrains.join(',').match('day')) {
                                    nDay = dNow.date();
                                    aIncrements.push({year: dNow.year(), month: dNow.month(), day: dNow.date(), hour: null});
                                } else if (nMonth != dNow.month() && Config.get('hStats')[sStat].aGrains.join(',').match('month')) {
                                    nMonth = dNow.month();
                                    aIncrements.push({year: dNow.year(), month: dNow.month(), day: null, hour: null});
                                } else if (nYear != dNow.year() && Config.get('hStats')[sStat].aGrains.join(',').match('year')){
                                    nYear = dNow.year();
                                    aIncrements.push({year: dNow.year(), month: null, day: null, hour: null});
                                } else if (Config.get('hStats')[sStat].aGrains.join(',').match('hour'))
...
module.exports.flush = function(hOpts,fnCallback){
    ...
    async.series([
        ...
        function(callback) {
            Config.get('MySql').execute('DELETE FROM StatTbl',null,callback,Config.get('hStats').sDbAlias);
        }
        ...
        ,function(callback){
            async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                Config.get('Redis').keys(hApp.id+'-*',function(err,aKeys){
                    if (err || !aKeys.length)
                        callback(err);
                    else
                        async.forEach(aKeys,function(sKey,cb){
                            Config.get('Redis').del(sKey,cb,Config.get('hStats').sDbAlias);
                        },cback);
                },Config.get('hStats').sDbAlias);
            },callback);
        }
    ],fnCallback);
};