...
module.exports.track = function(hOpts,fnCallback){
    ...
    else if (!Config.hStats[sStat])
        returnError('Stat not configured: '+sStat);
    else if (!Config.hStats[sStat].fnValidate)
        returnError('fnValidate not defined for stat: '+sStat);
    else
        Config.hStats[sStat].fnValidate(Params, function (err, sKey) {
            if (err)
                returnError(err);
            else {
                ...
                if (aKeys.length) {
                    if (sKey)
                        Config.Redis.hincrby(aKeys, sKey, hOpts.nFakeCount||1, fnCallback, Config.hStats.sDbAlias);
                    else
                        Config.Redis.incrby(aKeys, hOpts.nFakeCount||1, fnCallback, Config.hStats.sDbAlias);
                }
            }
        });
};
...
module.exports.process = function(hOpts,fnCallback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            var hStats =  {};
            if (hOpts.sStat && Config.hStats[hOpts.sStat])
                hStats[hOpts.sStat] = Config.hStats[hOpts.sStat];
            else if (hOpts.hStats)
                hStats = hOpts.hStats;
            else
                hStats = Config.hStats;
            ...
            if (!hOpts.oApp) {
                Collection.lookup({sClass:'App',hQuery:{root:true},hExtras:hConfirmExtras},function(err,cColl){
                    if (err)
                        ...
                        console.log(Config.MySql.hTrace);
                        cApps = cColl;
                        callback();
                    }
                });
            ...
            if (!aRedisStats || !aRedisStats.length)
                ...
                async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                    ...
                    async.forEachLimit(aRedisStats,1,function(sStat,cb) {
                        Config.info('Processing redis stat: '+sStat+'; App: '+oApp.getKey());
                        async.waterfall([
                            ...
                            // primary redis box.
                            function(cback) {
                                Config.Redis.keys(oApp.getKey()+'-'+sStat+',*',cback,Config.hStats.sDbAlias);
                            }
                                    ...
                                    async.forEach(aKeys,function(sKey,cback2){
                                        ...
                                        async.waterfall([
                                            function(cback3){
                                                ...
                                                Config.hStats[sStat].fnValidate(null,function(err){
                                                    if (err)
                                                        Config.Redis.hgetall(sKey,cback3,Config.hStats.sDbAlias);
                                                    else
                                                        Config.Redis.get(sKey,cback3,Config.hStats.sDbAlias);
                                                });
                                            }
                                        ],function(err,res){
                                            if (err || !res)
                                                ...
                                                var processRedisStat = function(hOpts,callback) {
                                                    ...
                                                    if (Config.hStats[hOpts.name] && Config.hStats[hOpts.name].aGrains && !Config.hStats[hOpts.name].aGrains.join(',').match(hOpts.sGrain)) {
                                                        cback2();
                                                    } else {
                                                ...
                                                if (res instanceof Object) {
                                                    ...
                                                    for (var sFilter in res) {
                                                        if (Config.hStats[sStat].bFilters)
                                                    ...
                                                    processRedisStat({
                                                        ...
                                                        ,sGrain:sGrain
                                                        ,count:nCount
                                                        ,filters:(Config.hStats[sStat].bFilters) ? res : null
                                                        ,name:sStat
                                                        ,year:aParts[1]||null
                                                        ,month:aParts[2]||null
                                                        ,day:aParts[3]||null
            ...
            if (aDbStats.length)
                async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                    ...
                    async.forEachLimit(aDbStats,1,function(sStat,cb){
                        Config.info('Processing mysql stat: '+sStat+'; App: '+oApp.getKey());

                        var hSettings = Config.hStats[sStat];
                        var processMySqlStat = function(hOpts,cb2) {
                            ...
                            async.parallel([
                                ...
                                ,function(cb3) {
                                    ...
                                    if (hOpts.year) {
                                        var dStart = moment.utc({year:hOpts.year,month:hOpts.month,day:hOpts.day,hour:hOpts.hour});
                                        var dEnd;

                                        if (hOpts.hour)
                                            dEnd = dStart.clone().add('hours',1);
                                        else if (hOpts.day)
                                            dEnd = dStart.clone().add('days',1);
                                        else if (hOpts.month) {
                                            dEnd = dStart.clone().add('months',1);
                                        } else if (hOpts.year)
                                            dEnd = dStart.clone().add('years',1);

                                        Config.info('---- '+dStart.toString()+' -> '+dEnd.toString());
                                        hSubOpts.nMin = dStart.valueOf();
                                        hSubOpts.nMax = dEnd.valueOf();
                                    }
                                    Collection.lookup({sClass:hSettings.sClass,sSource:'MySql',nSize:1,hQuery:hSettings.fnProcessQuery(hSubOpts,Config)},cb3);
                                }
                                ...
                                if (err)
                                    ...
                                    if (cColl.nTotal)
                                        Config.info(oStat.hData);

                                    if (hOpts.year) {
                                        oStat.set('date',moment.utc(hOpts).valueOf());
                                    }
                        ...
                        if (hOpts && hOpts.dStart && hOpts.dEnd) {
                            ...
                            var dNow = moment.utc(hOpts.dStart);
                            var dEnd = moment.utc(hOpts.dEnd);
                            ...
                            Config.info('Processing granularity: '+sGrain);
                            while (dNow < dEnd) {
                                if (nDay != dNow.date() && Config.hStats[sStat].aGrains.join(',').match('day')) {
                                    nDay = dNow.date();
                                    aIncrements.push({year: dNow.year(), month: dNow.month(), day: dNow.date(), hour: null});
                                } else if (nMonth != dNow.month() && Config.hStats[sStat].aGrains.join(',').match('month')) {
                                    nMonth = dNow.month();
                                    aIncrements.push({year: dNow.year(), month: dNow.month(), day: null, hour: null});
                                } else if (nYear != dNow.year() && Config.hStats[sStat].aGrains.join(',').match('year')){
                                    nYear = dNow.year();
                                    aIncrements.push({year: dNow.year(), month: null, day: null, hour: null});
                                } else if (Config.hStats[sStat].aGrains.join(',').match('hour'))
...
module.exports.flush = function(hOpts,fnCallback){
    ...
    async.series([
        ...
        function(callback) {
            Config.MySql.execute('DELETE FROM StatTbl',null,callback,Config.hStats.sDbAlias);
        }
        ...
        ,function(callback){
            async.forEachLimit(cApps.aObjects,1,function(hApp,cback){
                Config.Redis.keys(hApp.id+'-*',function(err,aKeys){
                    if (err || !aKeys.length)
                        callback(err);
                    else
                        async.forEach(aKeys,function(sKey,cb){
                            Config.Redis.del(sKey,cb,Config.hStats.sDbAlias);
                        },cback);
                },Config.hStats.sDbAlias);
            },callback);
        }
    ],fnCallback);
};