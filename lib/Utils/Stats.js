...
module.exports.track = function(hOpts,fnCallback){
    ...
    if (!sStat)
        ...
        async.waterfall([
            function(callback) {
                Base.loadAppSingleton(app_id,callback);
            }
...
module.exports.flush = function(hOpts,fnCallback){
    ...
    async.series([
        // Create collection of apps to flush stats for, and pull the stats foreach.
        function(callback) {
            if (hOpts && hOpts.oApp) {
                cApps.add(hOpts.oApp);
                callback();
            } else {
                for (var sExtra in AppConfig.hClasses.App.hExtras) {
                    hExtras[sExtra] = true;
                }
                Collection.lookup({sClass:'App',hQuery:{root:true},hExtras:hExtras},function(err,cColl){
                    if (err)
                        callback(err);
                    else {
                        cApps = cColl;
        ...
        ,function(callback) {
            ...
            if (!cApps.nTotal)
                callback('No apps/tenants found.');
            else
                async.forEach(cApps.aObjects,function(hItem,cb){
                    var aExtras = [];
                    var oApp = Base.lookup({sClass:'App',hData:hItem.hData});
                    for (var sKey in hExtras) {
                        oApp[sKey] = hItem[sKey];
                        aExtras.push(sKey);
                    }
