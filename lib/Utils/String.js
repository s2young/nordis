...
module.exports.decodeHtml = function(sHtml) {
...
module.exports.trim = function(sText) {
    if (sText)
        return sText.toString().replace(/^\s+|\s+$/g,"");
    else
        return sText;
};
    ...
    if (sHtml) {
        ...
        if (matches && matches.length)
            matches.forEach(function(item){
                ...
                sHtml = sHtml.replace(new RegExp(item,"gi"),replace);
            });
        sHtml = module.exports.decodeHtml(sHtml);
    }
    return sHtml;
};
...
module.exports.getQMarks = function(aVals) {
    aVals.join('?,?').replace(/\d/g,'?').replace(/(\?+)/g,'?')
};

module.exports.cleanTags = function(aTags) {
    var hTags = {};
    var aFiltered = [];
    if (aTags && aTags.length)
        aTags.forEach(function(sTag){
            sTag = module.exports.trim(sTag);
            if (sTag && !hTags[sTag]) {
                hTags[sTag] = 1;
                aFiltered.push(sTag);
            }
