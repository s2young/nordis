        ...
        async.waterfall([
            function(callback) {
                ...
                if (hContext.oMsg && hContext.oMsg.get('nApiConsumerID'))
                    callback(null,process.env.GOBA_ENV_ROOT_NODE_DIR+'/templates/engage');
                else
...
p.buildDotHelpers = function() {
    ...
    oSelf.defs.loadfile = function(sPath) {
        ...
        if (!bFound)
            App.error('Template not found. Tried app-specific path ('+process.env.sViewPath+sPath+') and root path ('+process.env.GOBA_ENV_ROOT_NODE_DIR+'/templates'+sPath+').');
        return '';
    };
};
