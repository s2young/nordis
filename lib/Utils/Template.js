var fs      = require('fs'),
    async   = require('async'),
    doT     = require('dot'),
    Config  = require('./../Base').prototype.Config.inst || require('./../Base').prototype.Config;
...
p.buildDotHelpers = function() {
    ...
    oSelf.defs.loadfile = function(sPath) {
        ...
        if (!bFound)
            Config.error('Template not found. Tried app-specific path ('+process.env.sViewPath+sPath+') and root path ('+process.env.NORDIS_ENV_ROOT_DIR+sPath+') and nordis project path ('+project_path+').');
        return '';
    };
};
...
var compile = function(sPath,hData,fnCallback){
    ...
    async.series([
        function(cb){
            ...
            if (self.hTemplates[sPath] && self.hTemplates[sPath] instanceof Function && Config.getEnv('NORDIS_ENV') != 'local') {
                cb();
