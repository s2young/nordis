...
module.exports = {
    ...
    ,tearDown:function(callback) {
        async.series([
            ...
            ,function(cb){
                new Collection({sClass:'User',hQuery:{sWhere:App.hClasses.User.sNumKeyProperty+' IS NOT NULL'}},function(err,cColl){
                    if (err)
                        cb(err);
                    else
                        cColl.delete(cb);
    ...
    ,getCollectionInFivePages:function(test){
        ...
        async.waterfall([
            ...
            ,function(o,cb){
                //App.log('Ranked items: '+(nTestSize-1)+' - '+(nTestSize-(nTestSize/5)));
            ...
            ,function(o,cb){
                //App.log('Ranked items: '+(nTestSize-(nTestSize/5)-1)+' - '+(nTestSize-((nTestSize/5)*2)));
                // Confirm paging is correct by testing the rank of the first and last items.
            ...
            ,function(o,cb){
                //App.log('Ranked items: '+(nTestSize-((nTestSize/5)*2)-1)+' - '+(nTestSize-((nTestSize/5)*3)));
                // Confirm paging is correct by testing the rank of the first and last items.
            ...
            ,function(o,cb){
                //App.log('Ranked items: '+(nTestSize-((nTestSize/5)*3)-1)+' - '+(nTestSize-((nTestSize/5)*4)));
                // Confirm paging is correct by testing the rank of the first and last items.
